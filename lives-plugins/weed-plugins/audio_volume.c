...
static weed_error_t  avol_process(weed_plant_t *inst, weed_timecode_t timestamp) {
  ...
  float **src;
  float **odst = (float **)weed_get_voidptr_array(out_channel, WEED_LEAF_AUDIO_DATA, NULL);
  ...
  int *swapchans = weed_get_boolean_array(in_params[2], WEED_LEAF_VALUE, NULL);
  int *swappan = weed_get_boolean_array(in_params[3], WEED_LEAF_VALUE, NULL);
