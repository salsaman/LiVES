...
void on_cut_activate(LiVESMenuItem *menuitem, livespointer user_data) {
  ...
  if (menuitem != NULL) {
    ...
    if (mainw->ccpd_with_sound) chk_mask |= WARN_MASK_LAYOUT_DELETE_AUDIO | WARN_MASK_LAYOUT_SHIFT_AUDIO | WARN_MASK_LAYOUT_ALTER_AUDIO;
...
boolean prevclip_callback(LiVESAccelGroup *group, LiVESWidgetObject *obj, uint32_t keyval, LiVESXModifierType mod, livespointer user_data) {
  ...
  if (!CURRENT_CLIP_IS_VALID || mainw->preview || (mainw->is_processing && cfile->is_loaded) || mainw->cliplist == NULL) return TRUE;
  ...
  if (preview_type == LIVES_PREVIEW_TYPE_VIDEO_AUDIO || preview_type == LIVES_PREVIEW_TYPE_IMAGE_ONLY) {
    ...
    if (height > 0 && width > 0) {
      ...
      if (error == NULL) {
        ...
        lives_alignment_set(mainw->fs_playalign, 0.5, 0.5, (float)width / (float)fwidth, (float)height / (float)fheight);
...
EXPOSE_FN_DECL(expose_vid_event, widget, user_data) {
  ...
  if (mainw->video_drawable != NULL) {
    ...
    if (width != 0 && width != lives_widget_get_allocation_width(mainw->video_draw)) {
      lives_painter_surface_destroy(mainw->video_drawable);
...
static void redraw_laudio(lives_painter_t *cr, int ex, int ey, int ew, int eh) {
  ...
  if (mainw->laudio_drawable != NULL) {
    ...
    if (width != 0 && width != lives_widget_get_allocation_width(mainw->laudio_draw)) {
      lives_painter_surface_destroy(mainw->laudio_drawable);
...
static void redraw_raudio(lives_painter_t *cr, int ex, int ey, int ew, int eh) {
  ...
  if (mainw->raudio_drawable != NULL) {
    ...
    if (width != 0 && width != lives_widget_get_allocation_width(mainw->raudio_draw)) {
      lives_painter_surface_destroy(mainw->raudio_drawable);
