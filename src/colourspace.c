...
static void convert_yuv420p_to_rgb_frame(uint8_t **src, const int width, int height,
    int y_delta, int *LIVES_RESTRICT istrides,
    const int orowstride, uint8_t *LIVES_RESTRICT dest,
    boolean add_alpha,
  ...
  const int irow = istrides[0];
  uint8_t *LIVES_RESTRICT s_y = src[0], * LIVES_RESTRICT s_u = src[1], * LIVES_RESTRICT s_v = src[2];
  const int opsize = add_alpha ? 4 : 3;
...
int *calc_rowstrides(int width, int pal, weed_layer_t *layer, int *nplanes) {
  ...
  if (layer) {
    if (weed_plant_has_leaf(layer, LIVES_LEAF_NEW_ROWSTRIDES))
      constrs =  weed_layer_get_rowstrides(layer, NULL);
    else if (weed_plant_has_leaf(layer, WEED_LEAF_ROWSTRIDES)
  ...
  if (constrs) {
    for (int i = 0; i < npl; i++) {
      if (constrs[i] / rs[i] >= .75 && constrs[i] / rs[i] <= 1.25)
        rs[i] = constrs[i];
    }
  }
  return rs;
...
LIVES_GLOBAL_INLINE size_t lives_frame_calc_bytesize(int width, int height, int pal, boolean inc_rowstrides,
  ...
  if (inc_rowstrides) {
    if (!rows) rows = calc_rowstrides(width, pal, NULL, &nplanes);
    if (!rows) return 0;
  }
  ...
  if (rows != rowstrides) lives_free(rows);

  return tot;
}


LIVES_GLOBAL_INLINE void add_internal_copylists(weed_layer_t *layer) {
  if (!layer || weed_plant_has_leaf(layer, LIVES_LEAF_COPYLIST)) return;
  int nplanes;
  ...
  if (nplanes > 1) {
    ...
    **************************/
    LIVES_CALLOC_TYPE(lives_sync_list_t *, copylists, nplanes);
    lives_sync_list_t *copylist = NULL;
    for (int i = 0; i < nplanes; i++)
      copylists[i] = copylist = lives_sync_list_push(copylist, (void *)layer);
    lives_sync_list_set_priv(copylists[0], (void *)pd_array[0]);
...
boolean create_empty_pixel_data(weed_layer_t *layer, boolean black_fill) {
...
#ifdef USE_BIGBLOCKS
    if ((pixel_data = malloc_bigblock(framesize)))
      weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
    else
#endif
  ...
  case WEED_PALETTE_YVU420P:
    ...
    if (1) {
#ifdef USE_BIGBLOCKS
      if ((memblock = malloc_bigblock(framesize + framesize2 + framesize3)))
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
      else
#endif
  ...
  case WEED_PALETTE_YUV444P:
    ...
    if (1) {
#ifdef USE_BIGBLOCKS
      if ((memblock = malloc_bigblock(framesize * 3)))
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
      else
#endif
  ...
  case WEED_PALETTE_YUVA4444P:
    ...
    if (1) {
#ifdef USE_BIGBLOCKS
      if ((memblock = malloc_bigblock(framesize * 4)))
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
      else
#endif
  ...
  case WEED_PALETTE_RGBFLOAT:
#ifdef USE_BIGBLOCKS
    if ((pixel_data = malloc_bigblock(rowstride * height)))
      weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
    else
#endif
  ...
  case WEED_PALETTE_AFLOAT:
#ifdef USE_BIGBLOCKS
    if ((pixel_data = malloc_bigblock(width * height)))
      weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, TRUE);
    else
#endif
...
static void swap_layer_planes(weed_layer_t *layer, int l1, int l2) {
  if (!layer) return;

  int nplanes;
  void **pd_array;
  lives_sync_list_t **copylists, *ctmp;
  int *rowstrides, rtmp;
  uint8_t *tmp;

  pd_array = weed_layer_get_pixel_data_planar(layer, &nplanes);

  if (!pd_array || l1 == l2 || l1 < 0 || l2 < 0 || l1 >= nplanes || l2 >= nplanes) {
  ...
  rtmp = rowstrides[l1];
  rowstrides[l1] = rowstrides[l2];
  rowstrides[l2] = rtmp;
  weed_layer_set_rowstrides(layer, rowstrides, nplanes);
  lives_free(rowstrides);

  copylists = lives_layer_get_copylist_array(layer, NULL);
  if (copylists) {
    ctmp = copylists[l1];
    copylists[l2] = copylists[l1];
    copylists[l1] = ctmp;
    weed_set_voidptr_array(layer, LIVES_LEAF_COPYLIST, nplanes, (void **)copylists);
    lives_free(copylists);
  }
...
boolean convert_layer_palette_full(weed_layer_t *layer, int outpl, int oclamping, int osampling
        ...
        if (!weed_palette_has_alpha_first(outpl)) {
          if (!weed_palette_has_alpha_last(outpl)) {
            ...
            if (can_inplace) {
              ...
              if (!create_empty_pixel_data(layer, FALSE)) goto memfail;
  ...
  case WEED_PALETTE_YUVA4444P:
    ...
    case WEED_PALETTE_YUV422P:
      ...
      weed_leaf_set_flags(layer, WEED_LEAF_ROWSTRIDES, rflags);
      gudest_array = (uint8_t **)weed_layer_get_pixel_data_planar(layer, NULL);
      weed_layer_copy_single_plane(layer, orig_layer, 0);
      ostrides = weed_layer_get_rowstrides(layer, NULL);
      convert_halve_chroma(gusrc_array, width, height, istrides, ostrides, gudest_array, iclamping);
      break;
    case WEED_PALETTE_RGB24:
...
conv_done:
  ...
  /// if V plane is before U, swap the pointers
  if (get_advanced_palette(outpl)->chantype[1] == WEED_VCHAN_V) swap_layer_planes(layer, 1, 2);

  lives_freep((void **)&istrides);
...
static void *gamma_convert_layer_thread(void *data) {
  ...
  const int height = ccparams->vsize;
...
lives_result_t pixbuf_to_layer(weed_layer_t *layer, LiVESPixbuf * pixbuf) {
  ...
  if (rowstride == get_last_pixbuf_rowstride_value(width, nchannels)) {
    ...
    // but since we cannot add pixbuf directly to the layer copylist, we create a proxy layer
    // and add that. Proxy_layer will only be removed from copylist when it is last member
    // at that point, it will free itself and remove the extra ref on pixbuf
    // the extra ref is there to prevent pixbuf from being unreffed and leaving proxy_layer hanging
    px_layer = weed_layer_new(WEED_LAYER_TYPE_VIDEO);
    ...
    pflags = weed_leaf_get_flags(px_layer, WEED_LEAF_PIXEL_DATA);
...
boolean resize_layer_full(weed_layer_t *layer, int width, int height,
  ...
  if (weed_palette_is_yuv(palette)) iclamping = weed_layer_get_yuv_clamping(layer);
  ...
  if (iwidth > 1 && iheight > 1) {
    ...
    for (int sl = 0; sl < nthrds; sl++) {
      ...
      swparams[sl].psize = pixel_size(opal_hint);
...
boolean letterbox_layer(weed_layer_t *layer, int nwidth, int nheight, int width, int height,
  ...
  if (using == -1) {
    ...
    ///////////////////////
    if (!create_empty_pixel_data(layer, TRUE)) goto memfail2;
    //////////////////////////
...
memfail2:
  ...
  weed_layer_copy(layer, old_layer);
  weed_layer_unref(old_layer);
  if (new_pixel_data) lives_free(new_pixel_data);
  return FALSE;
}
