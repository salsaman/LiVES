...
static void convert_yuv888_to_rgb_frame(guchar *src, gint hsize, gint vsize, gint irowstride, gint orowstride, guchar *dest, gboolean add_alpha, gboolean clamped) {
  register int x,y;
  size_t offs=3;
...
static void convert_yuva8888_to_rgba_frame(guchar *src, gint hsize, gint vsize, gint irowstride, gint orowstride, guchar *dest, gboolean del_alpha, gboolean clamped) {
  register int x,y;
...
static void convert_yuv888_to_bgr_frame(guchar *src, gint hsize, gint vsize, gint irowstride, gint orowstride, guchar *dest, gboolean add_alpha, gboolean clamped) {
  register int x,y;
  size_t offs=3;
...
static void convert_yuva8888_to_bgra_frame(guchar *src, gint hsize, gint vsize, gint irowstride, gint orowstride, guchar *dest, gboolean del_alpha, gboolean clamped) {
  register int x,y;
...
void convert_rgb_to_uyvy_frame(guchar *rgbdata, gint hsize, gint vsize, gint rowstride, uyvy_macropixel *u, gboolean has_alpha, gboolean clamped, int thread_id) {
  ...
  if (thread_id==-1&&prefs->nfx_threads>1) {
	...
	if (dheight*(i+1)>(vsize-4)) dheight=vsize-(dheight*i);
	if (dheight<4) break;
...
static void convert_uyvy_to_rgb_frame(uyvy_macropixel *src, int width, int height, int orowstride, guchar *dest, gboolean add_alpha, gboolean clamped, int thread_id) {
  ...
  if (thread_id==-1&&prefs->nfx_threads>1) {
	...
	if (dheight*(i+1)>(height-4)) dheight=height-(dheight*i);
	if (dheight<4) break;
...
static void convert_yuv888_to_yuv420_frame(guchar *yuv8, int width, int height, int irowstride, guchar **yuv4, gboolean src_alpha, gboolean clamped) {
...
static void convert_yuv888_to_yuv422_frame(guchar *yuv8, int width, int height, int irowstride, guchar **yuv4, gboolean has_alpha, gboolean clamped) {
...
static void convert_yuv888_to_uyvy_frame(guchar *yuv, int width, int height, int irowstride, uyvy_macropixel *uyvy, gboolean has_alpha, gboolean clamped) {
  int size=width*height;
...
static void convert_yuv888_to_yuyv_frame(guchar *yuv, int width, int height, int irowstride, yuyv_macropixel *yuyv, gboolean has_alpha, gboolean clamped) {
  int size=width*height;
...
static void convert_splitplanes_frame(guchar *src, int width, int height, int irowstride, guchar **dest, gboolean src_alpha, gboolean dest_alpha) {
...
void create_empty_pixel_data(weed_plant_t *layer, gboolean black_fill) {
  ...
  case WEED_PALETTE_YUV888:
    if (!black_fill) {
      ...
      black[0]=y_black;
      black[1]=black[2]=128;
      pixel_data=(guchar *)g_malloc(width*height*3);
      ptr=pixel_data;
      for (i=0;i<height;i++) {
	for (j=0;j<width;j++) {
	  w_memcpy(ptr,black,3);
	  ptr+=3;
	}
      }
    }
  ...
  case WEED_PALETTE_YUYV8888:
    if (!black_fill) {
      ...
      black[1]=black[3]=128;
      pixel_data=(guchar *)g_malloc(width*height*4);
      ptr=pixel_data;
      for (i=0;i<height;i++) {
	for (j=0;j<width;j++) {
	  w_memcpy(ptr,black,4);
  ...
  case WEED_PALETTE_YVU420P:
    ...
    if (!black_fill) {
      ...
      pd_array[0]=(guchar *)g_malloc(width*height);
      memset(pd_array[0],y_black,width*height);
      pd_array[1]=(guchar *)g_malloc(width*height/4);
      memset(pd_array[1],128,width*height/4);
      pd_array[2]=(guchar *)g_malloc(width*height/4);
  ...
  case WEED_PALETTE_YUV422P:
    ...
    if (!black_fill) {
      ...
      pd_array[0]=(guchar *)g_malloc(width*height);
      memset(pd_array[0],y_black,width*height);
      pd_array[1]=(guchar *)g_malloc(width*height/2);
      memset(pd_array[1],128,width*height/2);
      pd_array[2]=(guchar *)g_malloc(width*height/2);
  ...
  case WEED_PALETTE_YUV444P:
    ...
    if (!black_fill) {
      ...
      pd_array[0]=(guchar *)g_malloc(width*height);
      memset(pd_array[0],y_black,width*height);
      pd_array[1]=(guchar *)g_malloc(width*height);
      memset(pd_array[1],128,width*height);
      pd_array[2]=(guchar *)g_malloc(width*height);
  ...
  case WEED_PALETTE_YUVA4444P:
    ...
    if (!black_fill) {
      ...
      pd_array[2]=(guchar *)g_malloc(width*height);
      memset(pd_array[2],128,width*height);
      pd_array[3]=(guchar *)g_malloc(width*height);
  ...
  case WEED_PALETTE_RGBAFLOAT:
    ...
    if (!black_fill) pixel_data=(guchar *)calloc(width*height*4,sizeof(float));
      ...
      pixel_data=(guchar *)g_malloc(width*height*sizf);
      ptr=pixel_data;
      for (i=0;i<height;i++) {
	for (j=0;j<width;j++) {
	  w_memcpy(ptr,blackf,sizf);
	  ptr+=sizf;
	}
      }
    }
  ...
  case WEED_PALETTE_A8:
    rowstride=width;
    if (!black_fill) pixel_data=(guchar *)calloc(width*height,1);
    else {
      pixel_data=(guchar *)g_malloc(width*height);
  ...
  case WEED_PALETTE_A1:
    rowstride=(width+7)>>3;
    if (!black_fill) pixel_data=(guchar *)calloc(rowstride*height,1);
    else {
      pixel_data=(guchar *)g_malloc(rowstride*height);
...
static struct SwsContext *swscale_find_context(int iwidth, int iheight,
  ...
  for (i=0;i<N_SWS_CTX;i++) {
	...
	swscale_ctx[i].flags==flags) {
      // move to front
      w_memcpy(&tmpctx,&swscale_ctx[i],sizeof(struct _swscale_ctx));
      for (;i>0;i--) {
	w_memcpy(&swscale_ctx[i],&swscale_ctx[i-1],sizeof(struct _swscale_ctx));
      }
      w_memcpy(&swscale_ctx[0],&tmpctx,sizeof(struct _swscale_ctx));
...
static void swscale_add_context(int iwidth, int iheight, int width, int height, int pixfmt, int flags, struct SwsContext *ctx) {
  // add at head of list
  int i;

  for (i=1;i<N_SWS_CTX;i++) {
    // make space by shifting up
    if (swscale_ctx[i-1].ctx==NULL) break;

    // free last in list
    if (i==N_SWS_CTX-1 && swscale_ctx[i].ctx!=NULL) 
