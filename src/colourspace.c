...
static inline int get_gamma_idx(int gamma_type) {
...
LIVES_INLINE void rgb2uyvy_with_gamma(uint8_t r0, uint8_t g0, uint8_t b0, uint8_t r1, uint8_t g1, uint8_t b1,
                                      uyvy_macropixel *uyvy, uint8_t *LIVES_RESTRICT lut) {
  short a;
...
LIVES_INLINE void rgb2yuyv_with_gamma(uint8_t r0, uint8_t g0, uint8_t b0, uint8_t r1, uint8_t g1, uint8_t b1,
                                      yuyv_macropixel *yuyv, uint8_t *LIVES_RESTRICT lut) {
  short a;
...
LIVES_GLOBAL_INLINE boolean weed_palette_is_resizable(int pal, int clamped, boolean in_out) {
  // in_out is TRUE for input, FALSE for output
  // we have various possibilities
  // - palette can be resizeable and convertable
  int res = weed_palette_conv_resizable(pal, pal);
  if (in_out && !(res & 1)) return TRUE;
  if (!in_out && !(res & 2)) return TRUE;
  return FALSE;
}

//#define NO_SWS_CSCONV
LIVES_GLOBAL_INLINE boolean weed_palette_check_resizable(int pal, int clamped, boolean in_out) {
  // we can call this when converting rgb <---> yuv
  // if such converssions are blocked, then even if the palette is resizbale, if it is rgb
  // we prentend it is not. This will force the palette conversion to be done before or after the resize
  // this may be important for maintiang consistent conversion factors
  int res = weed_palette_conv_resizable(pal, pal);
#ifdef USE_SWSCALE
#ifdef NO_SWS_CSCONV
  if (weed_palette_is_rgb(pal)) return FALSE;
#endif
#endif
  if (in_out && !(res & 1)) return TRUE;
  if (!in_out && !(res & 2)) return TRUE;
...
static void convert_yuv420p_to_rgb_frame(uint8_t **LIVES_RESTRICT src, int width, int height,
  ...
  if (!is_422) {
    for (int i = 0; i < height; i += 2) {
      ...
      boolean skpnext = FALSE;
      //
      if (is_bottom && i == height - 1) skpnext = TRUE;
      ...
      for (int j = 0; j < width; j += 2) {
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y1, u1, v1, &dest[orowstride * i + j * opsize],
                              &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2], gamma_lut);

          xyuv2rgb_with_gamma(y3, u2, v2, &dest[orowstride * (i + 1) + j * opsize],
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y2, u3, v3, &dest[orowstride * i + (j + 1) * opsize],
                              &dest[orowstride * i + (j + 1) * opsize + 1],
                              &dest[orowstride * i + (j + 1) * opsize + 2], gamma_lut);

          xyuv2rgb_with_gamma(y4, u4, v4, &dest[orowstride * (i + 1) + (j + 1) * opsize],
    ...
    for (int i = 0; i < height; i++) {
      ...
      for (int j = 0; j < width; j++) {
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y1, u, v, &dest[orowstride * i + j * opsize],
                              &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2], gamma_lut);
        } else {
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y2, u, v, &dest[orowstride * i + j * opsize],
                              &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2], gamma_lut);
        } else {
  ...
  if (!is_422) {
    for (int i = 0; i < height; i += 2) {
      ...
      for (int j = 0; j < width; j += 2) {
        ...
        u2 = avg_chromaf(this_u1, next_u1);
        v2 = avg_chromaf(this_v1, next_v1);
        ...
        if (add_alpha)
          dest[orowstride * i + j * opsize + 3] = dest[orowstride * (i + 1) + j * opsize + 3] = 255;
        ...
        if (j > 0) {
          // we assume central sampling (mpeg) so we have y1-uv1-y2-y3-uv2-y4-y5-uv3-y6-y7- etc
          // then y1 gets 100% uv1, y2 gets 75% uv1 and 25% uv2, y3 gets 25% uv1, 75% uv2 etc
          u3 = avg_chromaf(avg_chromaf(this_u2, last_u2), u1);
          v3 = avg_chromaf(avg_chromaf(this_v2, last_v2), v1);
        }
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y2, u3, v3, &dest[orowstride * (i + 1) + j * opsize],
                              &dest[orowstride * (i + 1) + j * opsize + 1],
                              &dest[orowstride * (i + 1) * j * opsize + 2], gamma_lut);
    ...
    for (int i = 0; i < height; i++) {
      ...
      for (int j = 0; j < width; j++) {
        ...
        if (gamma_lut) {
          xyuv2bgr_with_gamma(y1, u, v, &dest[orowstride * i + j * opsize],
                              &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2], gamma_lut);
        } else {
        ...
        if (gamma_lut) {
          xyuv2bgr_with_gamma(y2, u, v, &dest[orowstride * i + j * opsize],
                              &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2], gamma_lut);
        } else {
...
static void convert_yuv420p_to_argb_frame(uint8_t **LIVES_RESTRICT src, int width, int height,
    boolean is_bottom, int *istrides, int orowstride, uint8_t *LIVES_RESTRICT dest, boolean is_422,
    int sampling, int clamping, int subspace,
    int gamma, int tgt_gamma, uint8_t *LIVES_RESTRICT gamma_lut,
    int thread_id) {
  int i;
      ...
      for (i = prefs->nfx_threads; i--;) {
        ...
        if ((src[0] + dheight * i * istrides[0]) < end) {
          ...
          if (i == 0) convert_yuv420p_to_argb_frame_thread(&ccparams[i]);
            ...
            lives_thread_create(&threads[i], LIVES_THRDATTR_PRIORITY, convert_yuv420p_to_argb_frame_thread, &ccparams[i]);
            nthreads++;
          }
        }
      }
  ...
  if (!is_422) {
    for (int i = 0; i < height; i += 2) {
      ...
      for (int j = 0; j < width; j += 2) {
        ...
        dest[orowstride * i + j * opsize] = 255;

        if (gamma_lut) {
          xyuv2rgb_with_gamma(y1, u1, v1, &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2],
                              &dest[orowstride * i + j * opsize + 3], gamma_lut);

          xyuv2rgb_with_gamma(y3, u2, v2, &dest[orowstride * (i + 1) + j * opsize + 1],
                              &dest[orowstride * (i + 1) + j * opsize + 2],
                              &dest[orowstride * (i + 1) + j * opsize + 3], gamma_lut);
        } else {
          xyuv2rgb(y1, u1, v1, &dest[orowstride * i + j * opsize + 1],
                   &dest[orowstride * i + j * opsize + 2],
                   &dest[orowstride * i + j * opsize + 3]);

          xyuv2rgb(y3, u2, v2, &dest[orowstride * (i + 1)  + j * opsize + 1],
                   &dest[orowstride * (i + 1) + j * opsize + 2],
                   &dest[orowstride * (i + 1) + j * opsize + 3]);
        }
        ...
        dest[orowstride * (i + 1) + (j + 1) * opsize] = 255;

        if (gamma_lut) {
          xyuv2rgb_with_gamma(y2, u3, v3, &dest[orowstride * i + (j + 1) * opsize + 1],
                              &dest[orowstride * i + (j + 1) * opsize + 2],
                              &dest[orowstride * i + (j + 1) * opsize + 3], gamma_lut);

          xyuv2rgb_with_gamma(y4, u4, v4, &dest[orowstride * (i + 1) + (j + 1) * opsize + 1],
                              &dest[orowstride * (i + 1) + (j + 1) * opsize + 2],
                              &dest[orowstride * (i + 1) + (j + 1) * opsize + 3], gamma_lut);
        } else {
          xyuv2rgb(y2, u3, v3, &dest[orowstride * i + (j + 1) * opsize + 1],
                   &dest[orowstride * i + (j + 1) * opsize + 2],
                   &dest[orowstride * i + (j + 1) * opsize + 3]);

          xyuv2rgb(y4, u4, v4, &dest[orowstride * (i + 1) + (j + 1) * opsize + 1],
                   &dest[orowstride * (i + 1) + (j + 1) * opsize + 2],
                   &dest[orowstride * (i + 1) + (j + 1) * opsize + 3]);
        }
      }
      r2++;
    }
    ...
    for (int i = 0; i < height; i++) {
      ...
      for (int j = 0; j < width; j++) {
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y1, u, v, &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2],
                              &dest[orowstride * i + j * opsize + 3], gamma_lut);
        } else {
          xyuv2rgb(y1, u, v, &dest[orowstride * i + j * opsize + 1],
                   &dest[orowstride * i + j * opsize + 2],
                   &dest[orowstride * i + j * opsize + 3]);
        }
        // second RGB pixel
        ...
        if (gamma_lut) {
          xyuv2rgb_with_gamma(y2, u, v, &dest[orowstride * i + j * opsize + 1],
                              &dest[orowstride * i + j * opsize + 2],
                              &dest[orowstride * i + j * opsize + 3], gamma_lut);
        } else {
          xyuv2rgb(y2, u, v, &dest[orowstride * i + j * opsize + 1],
                   &dest[orowstride * i + j * opsize + 2],
                   &dest[orowstride * i + j * opsize + 3]);
        }
        last_u = this_u;
...
static void convert_rgb_to_uyvy_frame(uint8_t *LIVES_RESTRICT rgbdata, int hsize, int vsize, int rowstride, int orowstride,
                                      uyvy_macropixel *u, boolean has_alpha, int clamping, uint8_t *LIVES_RESTRICT gamma_lut,
                                      int thread_id) {
...
static void convert_rgb_to_yuyv_frame(uint8_t *LIVES_RESTRICT rgbdata, int hsize, int vsize, int rowstride, int orowstride,
                                      yuyv_macropixel *u, boolean has_alpha, int clamping, uint8_t *LIVES_RESTRICT gamma_lut, int thread_id) {
  // for odd sized widths, cut the rightmost pixel
  int hs3, ipsize = 3, ipsize2;
...
static void convert_bgr_to_uyvy_frame(uint8_t *LIVES_RESTRICT rgbdata, int hsize, int vsize, int rowstride, int orowstride,
                                      uyvy_macropixel *u, boolean has_alpha, int clamping, uint8_t *LIVES_RESTRICT gamma_lut, int thread_id) {
  // for odd sized widths, cut the rightmost pixel
  int hs3, ipsize = 3, ipsize2;
  uint8_t *end = rgbdata + rowstride * vsize;
  int i;
...
static void convert_argb_to_uyvy_frame(uint8_t *LIVES_RESTRICT rgbdata, int hsize, int vsize, int rowstride, int orowstride,
                                       uyvy_macropixel *u, int clamping, uint8_t *LIVES_RESTRICT gamma_lut, int thread_id) {
  // for odd sized widths, cut the rightmost pixel
...
static void convert_argb_to_yuyv_frame(uint8_t *LIVES_RESTRICT rgbdata, int hsize, int vsize, int rowstride, int orowstride,
                                       yuyv_macropixel *u, int clamping, uint8_t *LIVES_RESTRICT gamma_lut, int thread_id) {
  // for odd sized widths, cut the rightmost pixel
  int hs3, ipsize = 4, ipsize2;
  uint8_t *end;
  int i;
...
static void convert_swap4_frame(uint8_t *LIVES_RESTRICT src, int width, int height, int irowstride,
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j += 4) {
      tmp[0] = gamma_lut[src[i * irowstride + j + 3]]; // r / b
...
static void convert_swap3addpost_frame(uint8_t *LIVES_RESTRICT src, int width, int height, int irowstride, int orowstride,
    ...
    for (i = prefs->nfx_threads; i--;) {
      ...
      if ((src + dheight * i * irowstride) < end) {
        ...
        if (i == 0) convert_swap3addpost_frame_thread(&ccparams[i]);
          ...
          lives_thread_create(&threads[i], LIVES_THRDATTR_PRIORITY, convert_swap3addpost_frame_thread, &ccparams[i]);
          nthreads++;
        }
      }
    }
  ...
  if (!gamma_lut) {
    for (j = 0; j < height; j++) {
      for (i = 0; i < width; i++) {
        dest[orowstride * j + i * 5] = src[irowstride * j + i * 3 + 2]; // red
  ...
  for (j = 0; j < height; j++) {
    for (i = 0; i < width; i++) {
      dest[orowstride * j + i * 4] = gamma_lut[src[irowstride * j + i * 3 + 2]]; // red
      dest[orowstride * j + i * 4 + 1] = gamma_lut[src[irowstride * j + i * 3 + 1]]; // green
      dest[orowstride * j + i * 4 + 2] = gamma_lut[src[irowstride * j + i * 3]]; // blue
      dest[orowstride * j + i * 4 + 3] = 255; // alpha
    }
  }
}
...
static void convert_addpost_frame(uint8_t *LIVES_RESTRICT src, int width, int height, int irowstride, int orowstride,
  ...
  if (!gamma_lut) {
    ...
    for (k = 0; k < height; k++) {
      for (i = 0; i < width; i++) {
        dest[orowstride * k + i * 4] = gamma_lut[src[irowstride * k + i * 3]];
        dest[orowstride * k + i * 4 + 1] = gamma_lut[src[irowstride * k + i * 3 + 1]];
        dest[orowstride * k + i * 4 + 2] = gamma_lut[src[irowstride * k + i * 3 + 2]];
        dest[orowstride * k + i * 4 + 3] = 255; // alpha
      }
    }
  }
...
static void convert_swap3delpost_frame(uint8_t *LIVES_RESTRICT src, int width, int height, int irowstride, int orowstride,
  ...
  for (int k = 0; k < height; k++) {
    for (i = 0; i < width; i++) {
      dest[orowstride * k + i * 3] = gamma_lut[src[irowstride * k + i * 4 + 2]]; // red
      dest[orowstride * k + i * 3 + 1] = gamma_lut[src[irowstride * k + i * 4 + 1]]; // green
      dest[orowstride * k + i * 3 + 2] = gamma_lut[src[irowstride * k + i * 4]]; // blue
    }
  }
}
...
static void convert_swap3delpre_frame(uint8_t *LIVES_RESTRICT src, int width, int height, int irowstride, int orowstride,
  ...
  if ((irowstride == width * 4) && (orowstride == width * 3)) {
    // quick version
    for (; src < end; src += 4) {
      *(dest++) = gamma_lut[src[3]]; // red
      *(dest++) = gamma_lut[src[2]]; // green
      *(dest++) = gamma_lut[src[1]]; // blue
    }
  } else {
    int width4 = width * 4;
    orowstride -= width * 3;
    for (; src < end; src += irowstride) {
      for (i = 0; i < width4; i += 4) {
        *(dest++) = gamma_lut[src[i + 3]]; // red
        *(dest++) = gamma_lut[src[i + 2]]; // green
        *(dest++) = gamma_lut[src[i + 1]]; // blue
      }
      dest += orowstride;
    }
  }
...
int *calc_rowstrides(int width, int pal, weed_layer_t *layer, int *nplanes) {
  ...
  rs[0] = ALIGN_CEIL(rs[0], rowstride_alignment);
...
boolean create_empty_pixel_data(weed_layer_t *layer, boolean black_fill, boolean may_contig) {
  ...
  if ((!copylist || !copylist->list->next) && weed_plant_has_leaf(layer, LIVES_LEAF_BBLOCKALLOC)) {
    // if we have big blog pdata, it should be large enough for whatever frame type
    // so just reuse it
    if (LIVES_IS_PLAYING) abort();
...
#ifdef USE_BIGBLOCKS
    if ((pixel_data = realloced) || (pixel_data = calloc_bigblock(framesize)))
...
#ifdef USE_BIGBLOCKS
      if ((memblock = realloced) || (memblock = calloc_bigblock(framesize + framesize2 + framesize3))) {
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, WEED_TRUE);
      } else
#endif
  ...
  case WEED_PALETTE_YUV444P:
    ...
    if (!may_contig) {
      ...
      weed_set_boolean_value(layer, LIVES_LEAF_PIXEL_DATA_CONTIGUOUS, WEED_TRUE);
#ifdef USE_BIGBLOCKS
      if ((memblock = realloced) || (memblock = calloc_bigblock(framesize * 3))) {
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, WEED_TRUE);
      } else
#endif
  ...
  case WEED_PALETTE_YUVA4444P:
    ...
    if (!may_contig) {
      ...
      weed_set_boolean_value(layer, LIVES_LEAF_PIXEL_DATA_CONTIGUOUS, WEED_TRUE);
#ifdef USE_BIGBLOCKS
      if ((memblock = realloced) || (memblock = calloc_bigblock(framesize * 4))) {
        weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, WEED_TRUE);
      } else
#endif
  ...
  case WEED_PALETTE_RGBFLOAT:
#ifdef USE_BIGBLOCKS
    if ((pixel_data = realloced) || (pixel_data = calloc_bigblock(rowstride * height))) {
      weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, WEED_TRUE);
    } else
#endif
  ...
  case WEED_PALETTE_AFLOAT:
#ifdef USE_BIGBLOCKS
    if ((pixel_data = realloced) || (pixel_data = calloc_bigblock(width * height))) {
      weed_set_boolean_value(layer, LIVES_LEAF_BBLOCKALLOC, WEED_TRUE);
    } else
#endif
      ...
      if (!weed_palette_has_alpha_last(inpl)) {
        if (!weed_palette_has_alpha_first(outpl)) {
          ...
          if (weed_palettes_rbswapped(inpl, outpl)) {
            ...
            orowstride = weed_layer_get_rowstride(layer);
            gudest = weed_layer_get_pixel_data(layer);
            convert_addpre_frame(gusrc, width, height, irowstride, orowstride, gudest, gamma_lut, -USE_THREADS);
          }
        }
      } else {
        /// inpl has post
      ...
      if (!weed_palette_has_alpha_first(outpl)) {
        if (!weed_palette_has_alpha_last(outpl)) {
          if (weed_palettes_rbswapped(inpl, outpl)) {
            ...
            orowstride = weed_layer_get_rowstride(layer);
            gudest = weed_layer_get_pixel_data(layer);
            convert_delpre_frame(gusrc, width, height, irowstride, orowstride, gudest, gamma_lut, -USE_THREADS);
          }
        } else {
          /// outpl has post
          if (weed_palettes_rbswapped(inpl, outpl)) {
...
LIVES_INLINE LiVESPixbuf *lives_pixbuf_cheat(boolean has_alpha, int width, int height,
  ...
  pbref = LIVES_CALLOC_SIZEOF(LiVESPixbuf *, 1);
  ...
  pixbuf = lives_pixbuf_new_from_data(buf, has_alpha, width, height, rowstride,
                                      (LiVESPixbufDestroyNotify)lives_free_buffer, (void *)pbref);

  *pbref = pixbuf;
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(pixbuf), NOFREE_KEY, NULL);
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(pixbuf), LAYER_PROXY_KEY, proxy_layer);
  lives_widget_object_set_data_auto(LIVES_WIDGET_OBJECT(pixbuf), PBREF_KEY, pbref);
...
lives_result_t get_resizable(lives_layer_t *layer, int *ppalette, int iclamping, int *opal, int oclamp_hint,
  ...
  boolean in_resizable = TRUE, out_resizable = FALSE;

  // palette is our start palette, opal_hint is our final palette
  // both values may be adjusted, pxpal is our "fake" in pal, pxopal fake out

  if (!weed_palette_is_resizable(palette, iclamping, IN_CHAN))
    in_resizable = FALSE;

  if (!weed_palette_is_resizable(opal_hint, oclamp_hint, OUT_CHAN))
    out_resizable = FALSE;
  ...
  if (opal_hint == WEED_PALETTE_ANY) out_resizable = TRUE;

  if (in_resizable) {
    if (!out_resizable) resolved = xopal_hint = opal_hint = palette;
    else resolved = palette;
    goto ok;
  }

  if (out_resizable) {
    if (layer) convert_layer_palette(layer, opal_hint, oclamp_hint);
    resolved = opal_hint;
    goto ok;
  }

  if (1) {
    // neither palette resizable
    if (weed_palette_is_rgb(palette)) {
      if (weed_palette_is_rgb(opal_hint)) {
        // the simple case - rgb(a) <--> rgb(a)
        if (weed_palette_has_alpha(palette) && weed_palette_has_alpha(opal_hint)) {
          if (layer) convert_layer_palette(layer, WEED_PALETTE_RGBA32, 0);
          resolved = WEED_PALETTE_RGBA32;
        } else {
          if (layer) convert_layer_palette(layer, WEED_PALETTE_RGB24, 0);
          resolved = WEED_PALETTE_RGB24;
        }
      } else {
        // rgb -> yuv
        // we have two options - convert to resizbale rgb, resize to opal
        // or convert to resizable yuv, resize to opal
        // if we have resizable yuv (444p or 4444p), we prefer option 2
        if (weed_palette_has_alpha(palette) && weed_palette_has_alpha(opal_hint)) {
          if (weed_palette_is_resizable(WEED_PALETTE_YUVA4444P, oclamp_hint, OUT_CHAN)) {
            if (layer) convert_layer_palette(layer, WEED_PALETTE_YUVA4444P, oclamp_hint);
            resolved = WEED_PALETTE_YUVA4444P;
          }
        } else {
          if (weed_palette_is_resizable(WEED_PALETTE_YUV444P, oclamp_hint, OUT_CHAN)) {
            if (layer) convert_layer_palette(layer, WEED_PALETTE_YUV444P, oclamp_hint);
            resolved = WEED_PALETTE_YUV444P;
          }
        }
      }
    } else {
      ...
      if (weed_palette_is_yuv(opal_hint)) {
        // yuv -> rgb
        if (weed_palette_is_planar(palette) || weed_palette_is_planar(opal_hint)) {
        ...
        if (resolved == WEED_PALETTE_NONE) {
          if (weed_palette_has_alpha(palette) && weed_palette_has_alpha(opal_hint)) {
            if (layer) convert_layer_palette(layer, WEED_PALETTE_YUVA8888, WEED_YUV_CLAMPING_UNCLAMPED);
            resolved = WEED_PALETTE_YUVA8888;
            xopal_hint = xpalette = WEED_PALETTE_RGBA32;
          } else {
            if (layer) convert_layer_palette(layer, WEED_PALETTE_YUV888, WEED_YUV_CLAMPING_UNCLAMPED);
            resolved = WEED_PALETTE_YUV888;
            xpalette = WEED_PALETTE_RGB24;
            if (weed_palette_has_alpha(opal_hint))
              xopal_hint = WEED_PALETTE_RGBA32;
            else xopal_hint = WEED_PALETTE_RGB24;
          }
        }
      } else {
        if (!weed_palette_is_planar(palette)) {
          if (weed_palette_has_alpha(palette) && weed_palette_has_alpha(opal_hint)) {
            if (layer) convert_layer_palette(layer, WEED_PALETTE_YUVA8888, WEED_YUV_CLAMPING_UNCLAMPED);
            resolved = WEED_PALETTE_YUVA8888;
            ...
            resolved = WEED_PALETTE_YUV888;
            xpalette = WEED_PALETTE_RGB24;
            if (opal_hint != WEED_PALETTE_ANY && weed_palette_has_alpha(opal_hint))
              xopal_hint = WEED_PALETTE_RGBA32;
            else
...
ok:
  if (resolved != *ppalette) {
    if (xopal_hint != opal_hint && resolved != opal_hint)
      opal_hint = resolved;
  }
...
boolean resize_layer(weed_layer_t *layer, int width, int height, LiVESInterpType interp, int opal_hint, int oclamp_hint) {
  ...
  //
  // - if in / out both rgb pre-convert to / from RGB24 / RGBA32 / BGR24 / BGRA32 / ARGB32 (if resizable)
  // - fallback of RGB24 / RGBA32 if not resizable - BGR24 can masquerade as RGB24. BGRA32 as RGBA32
  // etc. ARGB can be converted to RGBA32 or BGRA32 if necessary
  //
  // - if in / out both packed yuv, pre- convert to yuv888 / yuva8888, resize as rgb, convert to target
  //
  ...
  if (get_resizable(layer, &resolved, iclamping, &opal_hint, oclamp_hint,
                    &xpalette, &xopal_hint) == LIVES_RESULT_FAIL) {
#ifdef DEBUG_RESIZE
  ...
  if (resolved != WEED_PALETTE_ANY && palette != resolved) {
#ifdef DEBUG_RESIZE
  ...
  if (iwidth > 1 && iheight > 1) {
    ...
    /* if (new_gamma_type != WEED_GAMMA_UNKNOWN) { */
    /*   //weed_layer_set_gamma(layer, new_gamma_type); */
    /*   if (new_gamma_type == WEED_GAMMA_BT709 */
    /*       || (weed_palette_is_yuv(palette) */
    /*           && weed_layer_get_yuv_subspace(old_layer) == WEED_YUV_SUBSPACE_BT709)) */
    /*     subspace = WEED_YUV_SUBSPACE_BT709; */
    /* } */

    // set new values

    if (palette != xopal_hint && xopal_hint != WEED_PALETTE_NONE
        && opal_hint != WEED_PALETTE_ANY) {
      weed_layer_set_palette(layer, xopal_hint);
    }

    if (weed_palette_is_yuv(xopal_hint)) weed_layer_set_yuv_clamping(layer, oclamp_hint);
    weed_layer_set_size(layer, width / weed_palette_get_pixels_per_macropixel(xopal_hint), height);
    ...
    // from pass 1, so that we eliminatye any artifacts caused by splitting the image
