...
deinit2:
  ...
  if (mainw->record && !mainw->record_paused && LIVES_IS_PLAYING && (prefs->rec_opts & REC_EFFECTS) && !is_gen) {
    ticks_t actual_ticks = lives_get_current_playback_ticks(mainw->origsecs, mainw->orignsecs, NULL);
...
void rec_param_change(weed_plant_t *inst, int pnum) {
  ...
  actual_ticks = lives_get_current_playback_ticks(mainw->origsecs, mainw->orignsecs, NULL);
...
LIVES_GLOBAL_INLINE int rte_get_numfilters(void) {
...
void apply_key_defaults(weed_plant_t *inst, int key, int mode) {
  ...
  do {
    ...
    lives_free(params);
  } while (weed_get_plantptr_value(inst, WEED_LEAF_HOST_NEXT_INSTANCE, NULL) != NULL);
}
...
void free_key_defaults(int key, int mode) {
  ...
  for (int i = 0; i < nparams; i++) if (key_defs[i] != NULL) weed_plant_free(key_defs[i]);
