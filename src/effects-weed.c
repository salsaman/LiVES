  ...
  if (opwidth != rmaxw || opwidth != rminw || opheight != rmaxh || opheight != rminh) {
    ...
    if (!mainw->multitrack) {
      if (!svary || prefs->pb_quality == PB_QUALITY_LOW || !is_converter) {
        ...
        // has to be a multiple of 16 ! either for openGL or swscale
        opwidth = (opwidth >> 4) << 4;
  ...
  for (i = 0; i < num_in_tracks; i++) {
    ...
    if ((channel_flags & WEED_CHANNEL_REINIT_ON_PALETTE_CHANGE)
      ...
      if (!needs_reinit && weed_get_boolean_value(inst, WEED_LEAF_HOST_UNUSED, NULL) == WEED_FALSE)
        ...
        if (clip == 0 || IS_NORMAL_CLIP(clip)) {
          ...
          g_print("rzzzzessssdaseisdasdnnn 123aaa\n");
          needs_reinit = TRUE;
	  // *INDENT-OFF*
	}}}
    // *INDENT-ON*
    ...
    if (opalette != inpalette) {
      ...
      if (palette != inpalette) {
        if (channel_flags & WEED_CHANNEL_REINIT_ON_PALETTE_CHANGE) {
          g_print("ressssdaseisdasdnnn 123aaa\n");
          needs_reinit = TRUE;
        }
      }
      lives_freep((void **)&palettes);
    ...
    if ((!svary && (inwidth != width || inheight != height))
      ...
      if (!resized) {
        ...
        if (sfile && sfile->clip_type == CLIP_TYPE_GENERATOR && prefs->no_lb_gens) {
          /* get_player_size(&width, &height); */
          /* width = mainw->pwidth; */
          /* height = mainw->pheight; */
        } else if (letterbox && opwidth && opheight) calc_maxspect(opwidth, opheight, &width, &height);
	  ...
	  // *INDENT-ON*
          ...
          if (!is_pure_audio(filter, TRUE)) {
            if (mainw->pconx && !(mainw->preview || mainw->is_rendering)) {
              ...
              weed_instance_unref(instance);
              if (orig_inst == instance) orig_inst = xinstance;
              instance = xinstance;
...
matchvals:
  ...
  if (prefs->no_lb_gens) {
    ...
    if (IS_VALID_CLIP(mainw->blend_file) && mainw->blend_file != mainw->playing_file) {
      ...
      case PB_QUALITY_HIGH:
        ...
        calc_maxspect(lb_width, lb_height, &xwidth, &xheight);
      case PB_QUALITY_MED:
        calc_maxspect(oxwidth, oxheight, &xwidth, &xheight);
        break;
      default:
        break;
      }
  ...
  sfile->hsize = xwidth / weed_palette_get_pixels_per_macropixel(palette);
  sfile->vsize = xheight;
  ...
  if (can_change || opalette == WEED_PALETTE_END || !((channel_flags & WEED_CHANNEL_REINIT_ON_PALETTE_CHANGE))) {
    palette = check_filter_chain_palettes(is_bg, palette_list, npals);

    if (palette != WEED_PALETTE_END) {

      if (palette != opalette
          && (opalette == WEED_PALETTE_END || can_change
              || (!((channel_flags & WEED_CHANNEL_REINIT_ON_ROWSTRIDES_CHANGE)
      ...
      if (opalette == WEED_PALETTE_END) {
        palette = opalette = palette_list[0];
      }
    }
  }
  ...
  if (fg_gen_to_start != -1) {
    ...
    if (enabled_in_channels(weed_filters[key_to_fx[fg_gen_to_start][key_modes[fg_gen_to_start]]], FALSE) == 0) {
      ...
      if (inst) {
        char *tmp;
geninit1:
        error = weed_call_init_func(inst); // adds 1 ref
        weed_instance_unref(inst); // remove ref
...
deinit4:
          ...
          error = weed_call_deinit_func(inst);
          if (error != WEED_SUCCESS) weed_instance_unref(inst);
        ...
        if ((next_inst = get_next_compound_inst(inst))) {
...
boolean weed_delete_effectkey(int key, int mode) {
  ...
  for (; mode < (key < FX_KEYS_MAX_VIRTUAL ? prefs->rte_modes_per_key : 1); mode++) {
    mainw->osc_block = TRUE;
    if (key >= FX_KEYS_MAX_VIRTUAL || mode == prefs->rte_modes_per_key - 1 || key_to_fx[key][mode + 1] == -1) {
...
boolean rte_key_setmode(int key, int newmode) {
  ...
  if (newmode == -1) {
    // cycle forwards
    if (oldmode == prefs->rte_modes_per_key - 1 || key_to_fx[key][oldmode + 1] == -1) {
      newmode = 0;
    } else {
  ...
  if (newmode == -2) {
    ...
    newmode = key_modes[key] - 1;
    if (newmode < 0) {
      for (newmode = prefs->rte_modes_per_key - 1; newmode >= 0; newmode--) {
