...
boolean rtemode_callback(LiVESAccelGroup * group, LiVESWidgetObject * obj,
  ...
  if (mainw->rte_keys == -1) return TRUE;
  lives_proc_thread_add_hook_cb(mainw->player_proc, SYNC_ANNOUNCE_HOOK, 0,
				_rtemode_callback, "i", dirn);
  return TRUE;
}
...
boolean rtemode_callback_hook(LiVESToggleButton * button, livespointer user_data) {
  ...
  else lives_proc_thread_add_hook_cb(mainw->player_proc, SYNC_ANNOUNCE_HOOK, 0,
				     rte_key_setmode, "ii", key + 1, mode);
  return TRUE;
}
