...
lives_result_t lives_blankframe_srcfunc(weed_layer_t *layer) {
  weed_layer_t *xlayer = create_blank_layer(layer, NULL, 0, 0, WEED_GAMMA_UNKOWN);
  if (xlayer) return LIVEES_RESULT_SUCCESS;
  return LIVES_RESULT_ERROR;
}

// async
lives_result_t lives_imagedec_srcfunc(weed_layer_t *layer) {
  if (layer) return pull_frame_threaded(layer, 0, 0);
  return LIVES_RESULT_ERROR;
}
...
boolean weed_layer_create_from_file_progressive(weed_layer_t *layer, const char *fname, int width,
  ...
  if (!strcmp(img_explat, LIVES_FILE_EXT_PNG)) {
...
boolean pull_frame_at_size(weed_layer_t *layer, const char *image_ext, weed_timecode_t tc, int width, int height,
 ...
 success:
  ...
  // TODO - if this is a planned step, the we should now trugger reszie / palconv/ gamma
...
boolean check_layer_ready(weed_layer_t *layer) {
  ...
  if (lives_layer_get_status(layer) == LAYER_STATU_READY) return TRUE;
  else {
...
lives_result_t pull_frame_threaded(weed_layer_t *layer, int width, int height) {
  ...
  lives_proc_thread_t lpt;
#endif
  int clipno = live_layer_get_clip(layer);
  liveS_clip_t *sfile = RETURN_VALID_CLIP(clipno);

  if (sfile) {
    weed_timecode_t tc;
    lives_timecode_t *timer_data = weed_get_int64_array(layer, LIVES_LEAF_TIMER_DATA, NULL);
    const char *img_ext = get_image_ext_for_type(sfile->img_type);

    if (!timer_data) {
      lives_layer_reset_timing_data(layer);
      timer_data = weed_get_int64_array(layer, LIVES_LEAF_TIMER_DATA, NULL);
    }
    timer_data[LAYER_STATUS_QUEUD] = lives_get_session_ticks();

    tc = timer_data[LAYER_STATUS_TREF];
    if (!tc) tc = timer_data[LAYER_STATUS_TREF] = timer_data[LAYER_STATUS_QUEUED];
    weed_set_int64_array(layer, LIVES_LEAF_TIMER_DATA, N_LAYER_STATUSES, timer_data);
    lives_free(timer_data);
