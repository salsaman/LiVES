...
lives_result_t lives_imgloader_srcfunc(weed_layer_t *layer, boolean async) {
  if (layer) {
    if (async) return pull_frame_threaded(layer, 0, 0);
      ...
      if (sfile) {
        weed_timecode_t tc;
        ticks_t *timing_data = get_layer_timing(layer);
        const char *img_ext = get_image_ext_for_type(sfile->img_type);
        boolean res;

        timing_data[LAYER_STATUS_QUEUED] = lives_get_session_ticks();
        ...
        res = pull_frame(layer, img_ext, tc);
#else
...
check_stcache:
  ...
  if (!layer) {
	...
	if (mainw->pr_fcache) mainw->st_fcache = mainw->pr_fcache;
      }
      if (!mainw->st_fcache) {
	weed_layer_ref(mainw->en_fcache);
	if (mainw->en_fcache) mainw->st_fcache = mainw->en_fcache;
	tries = 0;
      }
...
check_encache:
  ...
  if (!layer) {
    ...
    if (tries--) {
      if (tries == 1) {
        weed_layer_ref(mainw->pr_fcache);
        if (mainw->pr_fcache) mainw->en_fcache = mainw->pr_fcache;
      }
      if (!mainw->en_fcache) {
        weed_layer_ref(mainw->st_fcache);
        if (mainw->st_fcache) mainw->en_fcache = mainw->st_fcache;
        tries = 0;
      }
    ...
    // *INDENT-ON*
      if (!layer) {
        ...
        if (tries--) {
          if (tries == 1) {
            weed_layer_ref(mainw->st_fcache);
            if (mainw->st_fcache) mainw->pr_fcache = mainw->st_fcache;
          }
          if (!mainw->pr_fcache) {
            weed_layer_ref(mainw->en_fcache);
            if (mainw->en_fcache) mainw->pr_fcache = mainw->en_fcache;
            tries = 0;
          }
...
fail:
  ...
  */
  ...
  LIVES_GLOBAL_INLINE boolean is_layer_ready(weed_layer_t *layer) {
    return lives_layer_get_status(layer) == LAYER_STATUS_READY;
  }
  ...
  /**
     ...
     finish_layer.
  */
  lives_result_t check_layer_ready(weed_layer_t *layer) {
    lives_clip_t *sfile;
#if USE_RESTHREAD
    lives_proc_thread_t resthread;
    ...
    while (1) {
      ...
      if (lives_layer_get_status(layer) == LAYER_STATUS_QUEUED)
        weed_layer_set_invalid(layer, TRUE);

      if (lives_layer_get_status(layer) == LAYER_STATUS_INVALID)
        return LIVES_RESULT_FAIL;

      if (lives_layer_get_status(layer) == LAYER_STATUS_LOADING) {
      ...
      if (lives_layer_get_status(layer) == LAYER_STATUS_LOADED) {
#if USE_RESTHREAD
...
#ifdef NO_FRAME_THREAD
      if (!pull_frame(layer, img_ext, tc)) return LIVES_RESULT_ERROR;
#else
