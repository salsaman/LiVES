...
static void make_accels(void) {
  // if connect, then we attch accels to widgets, else we attach them to accel group
  static LiVESWidgetClosure *stop_closure = NULL;
  static LiVESWidgetClosure *fullscreen_closure = NULL;
  static LiVESWidgetClosure *dblsize_closure = NULL;
  static LiVESWidgetClosure *sepwin_closure = NULL;
  static LiVESWidgetClosure *loop_closure = NULL;
  static LiVESWidgetClosure *loop_cont_closure = NULL;
  static LiVESWidgetClosure *fade_closure = NULL;
  static LiVESWidgetClosure *showfct_closure = NULL;
  static LiVESWidgetClosure *rec_closure = NULL;
  static LiVESWidgetClosure *mute_audio_closure = NULL;
...
void create_LiVES(void) {
  ...
  lives_signal_sync_connect_swapped(LIVES_GUI_OBJECT(mainw->show_defgroup), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  mainw->vj_mode_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->vj_mode), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  //lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->optimize), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  lives_signal_sync_connect_after(LIVES_GUI_OBJECT(mainw->show_devopts), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_Page_Up, LIVES_SHIFT_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(prevclip_callback), LIVES_INT_TO_POINTER(1), NULL));

  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_KP_Page_Up, LIVES_CONTROL_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(prevclip_callback), NULL, NULL));

  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_KP_Page_Up, LIVES_SHIFT_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(prevclip_callback), LIVES_INT_TO_POINTER(1), NULL));
  ...
  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_Page_Down, LIVES_SHIFT_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(nextclip_callback), LIVES_INT_TO_POINTER(1), NULL));

  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_KP_Page_Down, LIVES_CONTROL_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(nextclip_callback), NULL, NULL));

  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_KP_Page_Down, LIVES_SHIFT_MASK,
                            (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(nextclip_callback), LIVES_INT_TO_POINTER(1), NULL));
  ...
  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_m, (LiVESXModifierType)0, (LiVESAccelFlags)0,
                            lives_cclosure_new(LIVES_GUI_CALLBACK(rtemode_callback), NULL,
                                LIVES_INT_TO_POINTER(LIVES_ACTION_NEXT_MODE_CYCLE)));
  ...
  lives_accel_group_connect(LIVES_ACCEL_GROUP(mainw->accel_group), LIVES_KEY_m, (LiVESXModifierType)LIVES_SHIFT_MASK,
                            (LiVESAccelFlags)0, lives_cclosure_new(LIVES_GUI_CALLBACK(rtemode_callback),
                                LIVES_INT_TO_POINTER(LIVES_ACTION_PREV_MODE_CYCLE), NULL));
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->lock_selwidth), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_lock_selwidth_activate), NULL);
  mainw->record_perf_func
    = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->record_perf), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  mainw->fullscreen_cb_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->full_screen), LIVES_WIDGET_TOGGLED_SIGNAL,
                              LIVES_GUI_CALLBACK(on_full_screen_activate), NULL);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->sw_sound), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_boolean_toggled), &mainw->save_with_sound); // TODO - make pref
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->showsubs), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_showsubs_toggled), NULL);
  mainw->lb_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->letter), LIVES_WIDGET_TOGGLED_SIGNAL,
                   LIVES_GUI_CALLBACK(toggle_sets_pref), PREF_LETTERBOX);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->aload_subs), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_boolean_toggled), &prefs->autoload_subs);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->ccpd_sound), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  mainw->loop_cont_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->loop_continue), LIVES_WIDGET_TOGGLED_SIGNAL,
                          LIVES_GUI_CALLBACK(on_loop_cont_activate), NULL);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->loop_ping_pong), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_ping_pong_activate), NULL);
  mainw->mute_audio_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->mute_audio), LIVES_WIDGET_TOGGLED_SIGNAL,
                           LIVES_GUI_CALLBACK(on_mute_activate), NULL);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->sticky), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  mainw->desk_rec_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mainw->desk_rec), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  lives_signal_sync_connect_after(LIVES_GUI_OBJECT(mainw->autolives), LIVES_WIDGET_TOGGLED_SIGNAL,
...
void make_preview_box(void) {
  ...
  if (mainw->play_window) {
    if (!CURRENT_CLIP_HAS_VIDEO) {
      ...
      if (mainw->fs && mainw->sep_win &&  LIVES_IS_PLAYING) mainw->ignore_screen_size = TRUE;
  ...
  lives_standard_button_set_image(LIVES_BUTTON(mainw->p_rewindbutton), rewind_img, TRUE);
  ...
  lives_standard_button_set_image(LIVES_BUTTON(mainw->p_playbutton), play_img, TRUE);
  ...
  lives_standard_button_set_image(LIVES_BUTTON(mainw->p_playselbutton), playsel_img, TRUE);
  ...
  lives_standard_button_set_image(LIVES_BUTTON(mainw->p_loopbutton), loop_img, TRUE);
  ...
  lives_standard_button_set_image(LIVES_BUTTON(mainw->p_mutebutton), mainw->p_mute_img, TRUE);
