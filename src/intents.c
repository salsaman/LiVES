...
static void lives_object_instance_free(lives_obj_instance_t *obj) {
  ...
  lives_hooks_trigger(obj, hook_closures, FINAL_HOOK);
...
weed_error_t _set_plant_leaf_any_type_vargs(weed_plant_t *pl, const char *key, uint32_t st, weed_size_t ne, va_list args) {
  weed_error_t err;
  if (ne == 1) {
    switch (st) {
    case WEED_SEED_INT: {
