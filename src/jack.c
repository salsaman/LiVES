...
retry_connect:
  ...
  lives_hook_append(THREADVAR(hook_closures), THREAD_EXIT_HOOK, 0, defer_sigint_cb,
...
#endif
      lives_hook_append(THREADVAR(hook_closures), FINISHED_HOOK, 0, jack_interop_cleanup, jackd);
      need_clnup = TRUE;
    } else {
      jack_interop_cleanup(NULL, jackd);
    }
