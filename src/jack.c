...
boolean jack_warn(boolean is_trans, boolean is_con) {
  ...
  if (!ret) maybe_abort(TRUE, mainw->restart_params);

  if (com) lives_hook_cb_remove(fork_rcpt);
...
retry_connect:
  ...
  if (is_trans) {
    ...
    //lives_hook_cb_remove(defer_rcpt);
...
static void output_silence(size_t offset, jack_nframes_t nframes, jack_driver_t *jackd, float **out_buffer) {
  ...
  /* if (mainw->video_seek_beacon && lives_aplayer_get_seek_state(self) */
...
void lives_jack_set_client_attributes(jack_driver_t *jackd, int fileno, boolean activate, boolean running) {
  if (IS_VALID_CLIP(fileno)) {
    ...
    if (!running || (jackd && mainw->aud_rec_fd == -1)) {
      ...
      lives_aplayer_update_loop_mode(jackd->inst);
...
static int audio_process(jack_nframes_t nframes, void *arg) {
  ...
  if (!mainw->is_ready || !jackd || (!LIVES_IS_PLAYING && jackd->is_silent)) {
    in_ap = FALSE;
    reset_buffers = TRUE;
  ...
  if ((!jackd->in_use || !nframes) && !mainw->xrun_active) {
    if (!jackd->is_silent) {
      output_silence(0, nframes, jackd, out_buffer);
      jackd->is_silent = TRUE;
    }
...
static int start_ready_callback(jack_transport_state_t state, jack_position_t *pos, void *arg) {
  // mainw->video_seek_beacon is generally FALSE
  /*   // if we are not playing, the transport poll should start playing which will set set */
  /*   // mainw->video_seek_beacon to true, as soon as the video is at the right place */
  ...
  /*   return (mainw->video_seek_beacon & lives_aplayer_get_seek_state(aplayer)u); */
  /* } */
...
void jack_aud_pb_ready(jack_driver_t *jackd, boolean realign) {
    ...
    if (sfile->achans > 0 && (!mainw->preview || (mainw->preview && mainw->is_processing)) &&
        (sfile->laudio_time > 0. || sfile->opening ||
         (mainw->multitrack && mainw->multitrack->is_rendering &&
          lives_file_test((tmpfilename = lives_get_audio_file_name(clipno)), LIVES_FILE_TEST_EXISTS)))) {
      //lives_aplayer_seek_to(jackd->inst, clipno, seektime, LIVES_DIRECTION_NONE, 0., realign, FALSE);
    }
  }
}
