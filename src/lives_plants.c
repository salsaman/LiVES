...
void register_blueprint(uint64_t pltype, ...) {
  ...
  weed_plant_t **ld_array = NULL, *blup = NULL;
  bootstrap_template *blu = NULL;
  char *pltypestr;
  va_list va;
  int nleaves = 0;
  ...
  while (1) {
    ...
    if (blu) {
      LIVES_CALLOC_TYPE(bootstrap_leafdesc, ldesc, 1);
  ...
  //  TODO - need some way to store free func for autofree leaves !
  weed_leaf_set_autofree(blup, WEED_LEAF_VALUE, TRUE);

  lives_index_set_value(allblu, pltypestr, WEED_SEED_PLANTPTR, blup);
  weed_leaf_set_autofree(allblu, pltypestr, TRUE);

  if (pltype == LIVES_PLANT_LEAF_DEF) have_leaf_def_plant = TRUE;
...
static weed_plant_t *plant_from_template_va(bootstrap_template *templ, va_list va) {
  ...
  while (1) {
    ...
    LiVESList *list;
    for (list = templ->leaves; list; list = list->next) {
      ldef = (bootstrap_leafdesc *)list->data;
...
weed_plant_t *plant_from_blueprint(int pltype, ...) {
  ...
  if (lives_plant_get_subtype(blu) == LIVES_PLANT_TMP) {
    opl = plant_from_template_va((bootstrap_template *)weed_get_voidptr_value(blu, WEED_LEAF_VALUE, NULL), va);
    va_end(va);
    return opl;
  }
...
const char *lives_index_get_prefix(lives_index_t *idx) {return idx ? weed_get_const_string_value(idx, LIVES_LEAF_PREFIX, NULL) : NULL;}

weed_seed_t lives_index_get_itemtype(lives_index_t *idx) {return idx ? weed_get_int_value(idx, LIVES_LEAF_ITEM_TYPE, NULL) : WEED_SEED_INVALID;}
