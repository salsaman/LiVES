...
static void *_speedy_alloc(size_t nmemb, size_t xsize) {
  ...
  if (list) {
    ...
    smblock_pool->alloc_list = malloc(sizeof(void *));
    smblock_pool->alloc_list = list;
    nchunks_req = nchunks_req - nchunks;
    if (nchunks_req > 0) {
      if (((void **)list)[1]) {
        int nxtchunks = GET_BLOCK_SIZE(list->next);
        if (nxtchunks > 0) {
          SET_BLOCK_SIZE(list->next, nxtchunks + nchunks_req);
          SET_BLOCK_OFFS(list->next, offs + nchunks_req);
        }
      } else {
        alloc_block_t *block = make_alloc_block(nchunks_req, offs + nchunks_req);
...
static void *_speedy_alloc(size_t nmemb, size_t xsize) {
  ...
  off_t offs = 0;
  int nchunks_req = (xsize + smblock_pool->chunk_size - 1) / smblock_pool->chunk_size;
  LiVESList *list = smblock_pool->chunk_list;
  ssize_t avail_size = 0;
  int nchunks;
  ...
  if (list) {
    ...
    void *ptr = (char *)smblock_pool->buffer + offs * smblock_pool->chunk_size;
    pthread_mutex_unlock(&(smblock_pool->mutex));
    return ptr;
  }
...
static void *find_smblock(void *ptr, int *size_ret, off_t * offs_ret) {
  off_t toffs = ((char *)ptr - (char *)(smblock_pool->buffer)) / smblock_pool->chunk_size, coffs = 0;
  if (toffs < 0 || toffs >= smblock_pool->num_chunks) return NULL;

  for (LiVESList *list = smblock_pool->alloc_list; list; list = list->next) {
    LiVESList *listx = (LiVESList *)list->data;
    if (!listx || !listx->data) continue;
    off_t offs = GET_BLOCK_OFFS(listx);
    if (offs == toffs) {
      if (list->prev) list->prev->next = list->next;
      else smblock_pool->alloc_list = list->next;
...
static void *speedy_realloc(void *op, size_t xsize) {
  LiVESList *list;
  int nchunks_req, nchunks = 0, nxtchunks = 0;
  int freed_space;
  off_t offs = 0, toffs;
  char *nptr = NULL;
  ...
  toffs = ((char *)op - (char *)smblock_pool->buffer) / smblock_pool->chunk_size;
  if (toffs < 0 || toffs >= smblock_pool->num_chunks) {
  ...
  if (nchunks_req <= nchunks) {
    ...
    if (nchunks > nchunks_req) {
      if (nxtchunks) {
        ...
        alloc_block_t *block = make_alloc_block(nchunks - nchunks_req, offs + nchunks_req);
        list = lives_list_append(list, (void *)block);
      }
    }
    pthread_mutex_unlock(&(smblock_pool->mutex));
    ...
    if (nchunks + nxtchunks >= nchunks_req) {
      ...
      nptr = lives_malloc(xsize);
      if (!nptr) {
        pthread_mutex_unlock(&(smblock_pool->mutex));
        return NULL; // handle the error appropriately
      }
      memcpy(nptr, op, nchunks * smblock_pool->chunk_size);
      speedy_free(op);
      pthread_mutex_unlock(&(smblock_pool->mutex));
      return nptr;
    }
...
void speedy_free(void *ptr) {
  ...
  off_t toffs = ((char *)ptr - (char *)smblock_pool->buffer) / smblock_pool->chunk_size;
  if (toffs < 0 || toffs >= smblock_pool->num_chunks) {
    free(ptr);  // Use standard free if pointer is out of bounds
    return;
  }
  ...
  int nchunks = 0;
  off_t offs = 0;
  LiVESList *list = find_smblock(ptr, &nchunks, &offs);
  if (!list) {
    pthread_mutex_unlock(&(smblock_pool->mutex));
    free(ptr);  // Use standard free if block not found
    return;
  }
  ...
  if (list->next) {
    ...
    if (nxtchunks > 0) {
      nchunks += nxtchunks;
      smblock_pool->chunk_list = list->next;
    }
  }
  ...
  if (list->prev) {
    ...
    if (prevchunks > 0) {
      nchunks += prevchunks;
      SET_BLOCK_SIZE(list->prev, nchunks);
      smblock_pool->chunk_list = list->prev;
    } else {
      SET_BLOCK_SIZE(list, nchunks);
    }
  } else {
...
#define calloc_bigblock_relaxed(size)					\
