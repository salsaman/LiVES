...
#if GTK_CHECK_VERSION(3,0,0)
static boolean expose_track_event (GtkWidget *eventbox, lives_painter_t *cr, gpointer user_data) {
#else
static gint expose_track_event (GtkWidget *eventbox, GdkEventExpose *event, gpointer user_data) {
  ...
  if (mt->redraw_block) {
    gdk_window_process_updates(lives_widget_get_xwindow(eventbox),FALSE);
    return TRUE;
  }
  ...
  if (GPOINTER_TO_INT(g_object_get_data (G_OBJECT(eventbox), "drawn"))) {
    ...
    if (bgimage!=NULL&&lives_pixbuf_get_width(bgimage)>0) {
      //      lives_painter_t *cr = lives_painter_create_from_widget (eventbox);
	...
	draw_aparams(mt,eventbox,mt->aparam_view_list,mt->avol_init_event,startx,width);
      if (mt->block_selected!=NULL) draw_block(mt,mt->block_selected,-1,-1); // oops - causes expose to be called in a loop - seems we cannot stop this
      if (idlefunc>0) {
	mt->idlefunc=mt_idle_add(mt);
      }
      return FALSE;
...
static void notebook_error(GtkNotebook *nb, guint tab, lives_mt_nb_error_t err, lives_mt *mt) {
  ...
  lives_widget_set_fg_color(mt->nb_label, GTK_STATE_NORMAL, &palette->normal_fore);
...
on_drag_filter_end           (GtkWidget       *widget,
  ...
  if (cfile->achans>0&&enabled_in_channels(get_weed_filter(mt->selected_filter),TRUE)==1) {
	...
	if (lives_widget_get_xwindow(labelbox)!=window) {
	  gdk_window_get_pointer(lives_widget_get_xwindow(mt->timeline), &mt->sel_x, &mt->sel_y, NULL);
...
static void mouse_select_start(GtkWidget *widget, lives_mt *mt) {
  ...
  gdk_window_get_pointer(lives_widget_get_xwindow(mt->tl_eventbox), &mt->sel_x, &mt->sel_y, NULL);
...
static void mouse_select_move(GtkWidget *widget, lives_mt *mt) {
  ...
  gdk_window_get_pointer(lives_widget_get_xwindow(mt->tl_eventbox), &x, &y, NULL);
  gdk_window_get_position(lives_widget_get_xwindow(mt->timeline_eb), &min_x, NULL);
...
boolean on_timeline_update (GtkWidget *widget, GdkEventMotion *event, gpointer user_data) {
  ...
  gdk_window_get_pointer(lives_widget_get_xwindow(widget), &x, NULL, NULL);
