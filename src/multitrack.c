...
void set_mt_colours(lives_mt *mt) {
  ...
  if (palette->style & STYLE_1) {
    ...
    lives_widget_set_bg_color(mt->tl_hbox, LIVES_WIDGET_STATE_NORMAL, &palette->normal_back);
    lives_widget_set_fg_color(mt->tl_hbox, LIVES_WIDGET_STATE_NORMAL, &palette->normal_fore);
...
boolean on_multitrack_activate(LiVESMenuItem *menuitem, weed_plant_t *event_list) {
  ...
  if (prefs->show_gui) {
    lives_object_ref(mainw->top_vbox);
...
void do_block_context(lives_mt *mt, LiVESXEventButton *event, track_rect *block) {
  ...
  if (block->ordered) { // TODO
    split_here = lives_standard_menu_item_new_with_mnemonic(_("_Split Block At Cursor"));
