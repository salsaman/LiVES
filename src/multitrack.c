...
int mt_file_from_clip(lives_mt *mt, int clip) {
...
LIVES_GLOBAL_INLINE int get_track_for_block(lives_mt *mt, track_rect *block) {
  return LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(block->eventbox), LAYER_NUMBER_KEY));
}
...
boolean save_event_list_inner(lives_mt *mt, int fd, weed_plant_t *event_list, unsigned char **mem) {
  ...
  if (mt) {
    weed_set_int_value(event_list, WEED_LEAF_WIDTH, mainw->files[mt->render_file]->hsize);
    weed_set_int_value(event_list, WEED_LEAF_HEIGHT, mainw->files[mt->render_file]->vsize);
    weed_set_int_value(event_list, WEED_LEAF_AUDIO_CHANNELS, mainw->files[mt->render_file]->achans);
    weed_set_int_value(event_list, WEED_LEAF_AUDIO_RATE, mainw->files[mt->render_file]->arate);
    weed_set_int_value(event_list, WEED_LEAF_AUDIO_SAMPLE_SIZE, mainw->files[mt->render_file]->asampsize);

    if (mainw->files[mt->render_file]->signed_endian & AFORM_UNSIGNED)
      weed_set_boolean_value(event_list, WEED_LEAF_AUDIO_SIGNED, WEED_FALSE);
    else weed_set_boolean_value(event_list, WEED_LEAF_AUDIO_SIGNED, WEED_TRUE);

    if (mainw->files[mt->render_file]->signed_endian & AFORM_BIG_ENDIAN)
      weed_set_int_value(event_list, WEED_LEAF_AUDIO_ENDIAN, WEED_AUDIO_BIG_ENDIAN);
    else weed_set_int_value(event_list, WEED_LEAF_AUDIO_ENDIAN, WEED_AUDIO_LITTLE_ENDIAN);

    if (prefs->letterbox_mt) weed_set_boolean_value(event_list, WEED_LEAF_KEEP_ASPECT, WEED_TRUE);
    else weed_set_boolean_value(event_list, WEED_LEAF_KEEP_ASPECT, WEED_FALSE);
  }
...
boolean write_backup_layout_numbering(lives_mt *mt) {
  ...
  if (fd != -1) {
    for (clist = mainw->cliplist; !THREADVAR(write_failed) && clist; clist = clist->next) {
...
void recover_layout_cancelled(boolean is_startup) {
  char *fname = lives_strdup_printf("%s.%d.%d.%d.%s", LAYOUT_FILENAME,
                                    lives_getuid(), lives_getgid(),
                                    capable->mainpid, LIVES_FILE_EXT_LAYOUT);

  char *eload_file = lives_build_filename(prefs->workdir, fname, NULL);
  lives_free(fname);
...
boolean mt_load_recovery_layout(lives_mt *mt) {
  ...
  if (mt) {
    ...
    lives_free(fname);
    fname = lives_strdup_printf("%s.%d.%d.%d.%s", LAYOUT_FILENAME, lives_getuid(), lives_getgid(),
                                capable->mainpid, LIVES_FILE_EXT_LAYOUT);
    eload_file = lives_build_filename(prefs->workdir, fname, NULL);
    lives_free(fname);
    ...
    if (mt->event_list) {
      weed_plant_t *avol_init_event = mt->avol_init_event;
      lives_rm(eload_file);
      lives_rm(aload_file);
      mt->avol_init_event = NULL;
      mt_init_tracks(mt, TRUE);
      mt->avol_init_event = avol_init_event;
      remove_markers(mt->event_list);
      save_mt_autoback(mt);
    }
  } else {
...
boolean recover_layout(void) {
  ...
  if (prefs->startup_interface == STARTUP_CE) {
    ...
    reset_mt_play_sizes(mainw->multitrack);
  }
...
LIVES_GLOBAL_INLINE boolean is_audio_eventbox(LiVESWidget * ebox) {
...
boolean get_track_index(lives_mt * mt, weed_timecode_t tc) {
  ...
  if (!event || !mt->play_width || !mt->play_height) return retval;

  opwidth = mainw->files[mt->render_file]->hsize;
  opheight = mainw->files[mt->render_file]->vsize;
  ...
  if (mt->current_track < numtracks && clips[mt->current_track] < 1 &&
      (!mt->current_rfx || !mt->init_event || !mt->current_rfx->source || chindx == -1 ||
       !is_audio_channel_in((weed_plant_t *)mt->current_rfx->source, chindx))) {
    if (track_index != -1 && mt->fx_box) {
      add_mt_param_box(mt);
      retval = TRUE;
    }
    lives_free(clips);
  ...
  lives_free(clips);
  if (track_index != -1 && mt->track_index == -1 && mt->fx_box) {
    add_mt_param_box(mt);
    retval = TRUE;
  }
  return retval;
...
weed_timecode_t mt_set_play_position(lives_mt * mt) {
  // get start event
  ...
  if (mainw->jack_can_stop && (prefs->jack_opts & JACK_OPTS_TIMEBASE_START) && (prefs->jack_opts & JACK_OPTS_TRANSPORT_SLAVE)) {
    ...
    tc = q_gint64(mt->ptr_time * TICKS_PER_SECOND_DBL, mainw->files[mt->render_file]->fps);
...
boolean mt_trdown(LiVESAccelGroup * group, LiVESWidgetObject * obj, uint32_t keyval, LiVESXModifierType mod,
  ...
  if (!mt->aud_track_selected || mt->current_track == -1) {
    if (mt->current_track > -1) mt->current_track++;
      ...
      while (llist) {
        if (LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(llist->data), HIDDEN_KEY)) == 0) {
          mt->current_track = i;
          break;
        }
        llist = llist->next;
...
LIVES_GLOBAL_INLINE lives_mt_poly_state_t get_poly_state_from_page(lives_mt * mt) {
...
static boolean notebook_page(LiVESWidget * nb, LiVESWidget * nbp, uint32_t tab, livespointer user_data) {
  ...
  case POLY_CLIPS:
    ...
    if (mt->poly_state != POLY_CLIPS && nb) polymorph(mt, POLY_CLIPS);
    else lives_widget_reparent(mt->poly_box, lives_notebook_get_nth_page(LIVES_NOTEBOOK(mt->nb), page));
    break;
  case POLY_IN_OUT:
    if (!mt->block_selected && mt->poly_state != POLY_IN_OUT) {
  ...
  case POLY_FX_STACK:
    ...
    break;
  case POLY_EFFECTS:
    if (!mt->block_selected && mt->poly_state != POLY_EFFECTS) {
      notebook_error(LIVES_NOTEBOOK(mt->nb), tab, NB_ERROR_SEL, mt);
      return FALSE;
    }
  ...
  case POLY_PARAMS:
    ...
    lives_widget_reparent(mt->poly_box, lives_notebook_get_nth_page(LIVES_NOTEBOOK(mt->nb), page));
    if (mt->selected_init_event && mt->poly_state != POLY_PARAMS) {
      fubar(mt);
...
static void _select_block(lives_mt * mt) {
  ...
  if (block) {
    ...
    if (!mainw->files[mt->render_file]->achans || !mt->audio_draws || (mt->opts.back_audio_tracks == 0 ||
    ...
    if (mainw->files[mt->render_file]->achans == 0 || !mt->audio_draws || (mt->opts.back_audio_tracks == 0 ||
        eventbox != mt->audio_draws->data))
      add_context_label(mt, (tmp2 = lives_markup_printf_escaped(_("Current track: %s (layer %d)\n"),
    ...
    lives_free(tmp2);
    add_context_label(mt, (tmp2 = lives_markup_printf_escaped(_("Source: %s"),
                                  (tmp = get_menu_name(mainw->files[filenum], FALSE)))));
    lives_free(tmp);
    lives_free(tmp2);
    add_context_label(mt, (_("Right click for context menu.\n")));
    ...
    if (mainw->files[mt->render_file]->achans > 0) lives_widget_set_sensitive(mt->fx_blocka, TRUE);

    mt_redraw_eventbox(mt, eventbox);
...
static boolean on_drag_filter_end(LiVESWidget * widget, LiVESXEventButton * event, livespointer user_data) {
  ...
  if (mainw->files[mt->render_file]->achans > 0 && enabled_in_channels(get_weed_filter(mt->selected_filter), TRUE) == 1) {
    for (list = mt->audio_draws; list; list = list->next) {
      eventbox = (LiVESWidget *)list->data;
      if (LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), HIDDEN_KEY)) != 0) continue;
      ...
      if (lives_widget_get_xwindow(eventbox) == window || lives_widget_get_xwindow(labelbox) == window) {
        ...
        tchan = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), LAYER_NUMBER_KEY));
  ...
  if (!ok) {
    for (list = mt->video_draws; list; list = list->next) {
      eventbox = (LiVESWidget *)list->data;
      if (LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), HIDDEN_KEY)) != 0) continue;
      labelbox = (LiVESWidget *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "labelbox");
  ...
  if (nins == 1) {
    ...
    mt->current_track = get_track_for_block(mt, mt->putative_block);
...
static void populate_filter_box(int ninchans, lives_mt * mt, int pkgnum) {
  ...
  lives_widget_set_bg_color(lives_bin_get_child(LIVES_BIN(mt->fx_list_scroll)),
  ...
  if (!mt->block_selected && ninchans == 1) return;
  ...
  for (i = 0; i < nfilts; i++) {
    ...
    if (filter && !weed_plant_has_leaf(filter, WEED_LEAF_HOST_MENU_HIDE)) {
      int mandouts = 0, nouts = 0;
      weed_plant_t **ctmpls;

      if ((is_pure_audio(filter, FALSE) && (!eventbox || !is_audio_eventbox(eventbox))) ||
          (!is_pure_audio(filter, FALSE) && eventbox && is_audio_eventbox(eventbox))) continue;

      if (weed_filter_hints_unstable(filter) && !prefs->unstable_fx) continue;

      nins = enabled_in_channels(filter, TRUE);

      ctmpls = weed_filter_get_out_chantmpls(filter, &nouts);
      for (j = 0; j < nouts; j++) {
        if (!weed_chantmpl_is_optional(ctmpls[j])) {
          if (!has_non_alpha_palette(ctmpls[j], filter)) break;
          mandouts++;
        }
      }
      ...
      if ((nins == ninchans || (ninchans == 1000000 && nins >= ninchans)) && mandouts == 1) {
        pkgnum = 0;
        fname = weed_filter_idx_get_name(sorted, TRUE, TRUE);

        if ((pkgstring = weed_filter_idx_get_package_name(sorted))) {
          // filter is in package
          if (pkg_name && strcmp(pkgstring, pkg_name)) {
            // but wrong one
            lives_free(fname);
        ...
        if (!pkgnum) {
          ...
          if (pkg_name && *pkg_name && !pkgstring) {
...
track_rect *mt_selblock(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (!menuitem) return get_block_from_time(eventbox, timesecs, mt);

  mt->putative_block = get_block_from_time(eventbox, timesecs, mt);

  if (mt->putative_block && mt->putative_block->state == BLOCK_UNSELECTED) desel = FALSE;
...
void mt_spin_start_value_changed(LiVESSpinButton * spinbutton, livespointer user_data) {
  ...
  if ((((mt->region_start != mt->region_end && !has_region) || (mt->region_start == mt->region_end && has_region))) &&
      mt->event_list && get_first_event(mt->event_list)) {
  ...
  if ((((mt->region_start != mt->region_end && !has_region) || (mt->region_start == mt->region_end && has_region))) &&
    ...
    if (mt->selected_tracks) {
      ...
      if (mt->region_start != mt->region_end) {
        ...
        switch (lives_list_length(mt->selected_tracks)) {
        case 1:
          if (mainw->files[mt->render_file]->achans == 0) lives_widget_set_sensitive(mt->fx_region_a, FALSE);
          lives_widget_set_sensitive(mt->fx_region_2a, FALSE);
          lives_widget_set_sensitive(mt->fx_region_2v, FALSE);
          lives_widget_set_sensitive(mt->fx_region_2av, FALSE);
          break;
...
static boolean in_out_ebox_pressed(LiVESWidget * eventbox, LiVESXEventButton * event, livespointer user_data) {
  ...
  if (mt->block_selected) return FALSE;
...
static boolean on_drag_clip_end(LiVESWidget * widget, LiVESXEventButton * event, livespointer user_data) {
  ...
  lives_xwindow_set_cursor(lives_widget_get_xwindow(LIVES_MAIN_WINDOW_WIDGET), NULL);
  lives_set_cursor_style(LIVES_CURSOR_BUSY, NULL);
  //lives_widget_context_update();
  ...
  if (mainw->files[mt->render_file]->achans > 0 && mt->opts.back_audio_tracks > 0 &&
      LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(mt->audio_draws->data), HIDDEN_KEY)) == 0) {
  ...
  for (LiVESList *list = mt->video_draws; list; list = list->next) {
    eventbox = (LiVESWidget *)list->data;
    ...
    if (lives_widget_get_xwindow(eventbox) == window || lives_widget_get_xwindow(labelbox) == window ||
      ...
      mt->current_track = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), LAYER_NUMBER_KEY));
      mt->aud_track_selected = FALSE;
...
void mt_init_start_end_spins(lives_mt * mt) {
  ...
  int wois = widget_opts.icon_size;
  ...
  widget_opts.icon_size = LIVES_ICON_SIZE_SMALL_TOOLBAR;
...
void mouse_mode_context(lives_mt * mt) {
  ...
  if (prefs->atrans_fx != -1) fname = weed_filter_idx_get_name(prefs->atrans_fx, FALSE, FALSE);
  else fname = lives_strdup(mainw->string_constants[LIVES_STRING_CONSTANT_NONE]);
  add_context_label(mt, (_("\nAutotransition effect is:")));
  text = lives_strdup_printf("<b>%s</b>", (tmp = lives_markup_escape_text(fname, -1)));
...
void update_insert_mode(lives_mt * mt) {
  ...
  } else if (mt->opts.insert_mode == INSERT_MODE_AMATCH) {
    mtext = lives_menu_item_get_text(mt->ins_audmatch);
  }
  ...
  lives_signal_handler_block(mt->ins_audmatch, mt->ins_audmatch_func);
  lives_check_menu_item_set_active(LIVES_CHECK_MENU_ITEM(mt->ins_audmatch), mt->opts.insert_mode == INSERT_MODE_AMATCH);
  lives_signal_handler_unblock(mt->ins_audmatch, mt->ins_audmatch_func);
}
...
static void on_insert_mode_changed(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  } else if (menuitem == (LiVESMenuItem *)mt->ins_audmatch) {
...
void add_aparam_menuitems(lives_mt * mt) {
  ...
  if (mt->avol_fx == -1 || !mt->audio_draws) {
    ...
    if (mt->opts.aparam_view_list && !mainw->multi_opts.aparam_view_list) {
  ...
  for (i = 0; i < rfx->num_params; i++) {
    ...
    if ((rfx->params[i].hidden | HIDDEN_MULTI) == HIDDEN_MULTI && rfx->params[i].type == LIVES_PARAM_NUM) {
      menuitem = lives_standard_check_menu_item_new_with_label(rfx->params[i].name,
                 mt->opts.aparam_view_list && lives_list_find(mt->opts.aparam_view_list, LIVES_INT_TO_POINTER(i)));
      ...
      lives_signal_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                           LIVES_GUI_CALLBACK(mt_aparam_view_toggled), (livespointer)mt);
    }
  }
  widget_opts.mnemonic_label = TRUE;
  rfx_free(rfx);
...
static void apply_avol_filter(lives_mt * mt) {
  ...
  if (!init_event) {
    ...
    // will be re-added unless init_event is non-NULL
  ...
  old_tc = get_event_timecode(deinit_event);
  new_tc = get_event_timecode(new_end_event);

  if (old_tc == new_tc) {
    weed_event_t *frame_event = get_prev_frame_event(deinit_event);
    if (frame_event) new_tc = get_event_timecode(frame_event);
  }
...
static void set_audio_filter_channel_values(lives_mt * mt) {
  ...
  if (num_in > 0) {
    for (i = 0; i < num_in; i++) {
      weed_set_int_value(in_channels[i], WEED_LEAF_AUDIO_CHANNELS, mainw->files[mt->render_file]->achans);
      weed_set_int_value(in_channels[i], WEED_LEAF_AUDIO_RATE, mainw->files[mt->render_file]->arate);
    }
  ...
  if (num_out > 0) {
    for (i = 0; i < num_out; i++) {
      weed_set_int_value(out_channels[i], WEED_LEAF_AUDIO_CHANNELS, mainw->files[mt->render_file]->achans);
      weed_set_int_value(out_channels[i], WEED_LEAF_AUDIO_RATE, mainw->files[mt->render_file]->arate);
    }
...
static char *mt_set_vals_string(void) {
  ...
  return lives_strdup_printf(
           _("Multitrack values set to %.3f fps, frame size %d x %d, audio channels %d, "
...
static weed_plant_t *load_event_list_inner(lives_mt * mt, int fd, boolean show_errors, int *num_events,
  ...
  if (mt && show_errors && !(prefs->warning_mask & WARN_MASK_LAYOUT_LB)
      && weed_plant_has_leaf(event_list, WEED_LEAF_KEEP_ASPECT)) {
    boolean letterbox = weed_get_boolean_value(event_list, WEED_LEAF_KEEP_ASPECT, NULL);
  ...
  } else if (mt && !show_errors && !mem) return NULL; // no change needed

  if (mt) {
    if (event_list == mainw->stored_event_list || (mt && !mt->ignore_load_vals)) {
      if (fps > -1) {
        mainw->files[mt->render_file]->fps = mainw->files[mt->render_file]->pb_fps = fps;
        if (mt) mt->fps = mainw->files[mt->render_file]->fps;
        mainw->files[mt->render_file]->ratio_fps = check_for_ratio_fps(mainw->files[mt->render_file]->fps);
      }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_WIDTH)) {
        ...
        if (width > 0) {
          mainw->files[mt->render_file]->hsize = width;
          if (mt) mt->layout_set_properties = TRUE;
        }
      }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_HEIGHT)) {
        ...
        if (height > 0) {
          mainw->files[mt->render_file]->vsize = height;
          if (mt) mt->layout_set_properties = TRUE;
        }
      }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_AUDIO_CHANNELS)) {
        ...
        if (achans >= 0 && mt) {
          ...
          mainw->files[mt->render_file]->achans = achans;
          if (mt) mt->layout_set_properties = TRUE;
        }
      }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_AUDIO_RATE)) {
        ...
        if (arate > 0) {
          mainw->files[mt->render_file]->arate = mainw->files[mt->render_file]->arps = arate;
          if (mt) mt->layout_set_properties = TRUE;
        }
      }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_AUDIO_SAMPLE_SIZE)) {
        ...
        if (asamps == 8 || asamps == 16) {
          mainw->files[mt->render_file]->asampsize = asamps;
          if (mt) mt->layout_set_properties = TRUE;
        } else if (mainw->files[mt->render_file]->achans > 0) {
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_AUDIO_SIGNED)) {
        ...
        if (asigned == WEED_TRUE) {
          if (mainw->files[mt->render_file]->signed_endian & AFORM_UNSIGNED)
            mainw->files[mt->render_file]->signed_endian ^= AFORM_UNSIGNED;
        } else {
          if (!(mainw->files[mt->render_file]->signed_endian & AFORM_UNSIGNED))
            mainw->files[mt->render_file]->signed_endian |= AFORM_UNSIGNED;
        }
      ...
      if (weed_plant_has_leaf(event_list, WEED_LEAF_AUDIO_ENDIAN)) {
        ...
        if (aendian == WEED_AUDIO_LITTLE_ENDIAN) {
          if (mainw->files[mt->render_file]->signed_endian & AFORM_BIG_ENDIAN)
            mainw->files[mt->render_file]->signed_endian ^= AFORM_BIG_ENDIAN;
        } else {
          if (!(mainw->files[mt->render_file]->signed_endian & AFORM_BIG_ENDIAN))
            mainw->files[mt->render_file]->signed_endian |= AFORM_BIG_ENDIAN;
        }
      ...
      if (mt) {
        ...
        mainw->files[mt->render_file]->fps = mainw->files[mt->render_file]->pb_fps;
        if (mt) mt->fps = mainw->files[mt->render_file]->fps;
        mainw->files[mt->render_file]->ratio_fps = check_for_ratio_fps(mainw->files[mt->render_file]->fps);
      }
    }
  ...
  do {
    ...
    if (event) {
      ...
      if (eventprev) weed_set_voidptr_value(eventprev, WEED_LEAF_NEXT, event);
...
char *set_values_from_defs(lives_mt * mt, boolean from_prefs) {
  ...
  int hsize = mainw->files[mt->render_file]->hsize;
  int vsize = mainw->files[mt->render_file]->vsize;
  int arate = mainw->files[mt->render_file]->arate;
  int achans = mainw->files[mt->render_file]->achans;
  int asamps = mainw->files[mt->render_file]->asampsize;
  int ase = mainw->files[mt->render_file]->signed_endian;

  if (mainw->stored_event_list) {
    load_event_list_inner(mt, -1, TRUE, NULL, NULL, NULL);
    mt->user_width = mainw->files[mt->render_file]->hsize;
    mt->user_height = mainw->files[mt->render_file]->vsize;
    mainw->files[mt->render_file]->pb_fps = mt->fps = mt->user_fps = mainw->files[mt->render_file]->fps;
    mainw->files[mt->render_file]->arps = mt->user_arate = mainw->files[mt->render_file]->arate;
    mt->user_achans = mainw->files[mt->render_file]->achans;
    mt->user_asamps = mainw->files[mt->render_file]->asampsize;
    mt->user_signed_endian = mainw->files[mt->render_file]->signed_endian;
  } else {
    if (!from_prefs) {
      mainw->files[mt->render_file]->hsize = mt->user_width;
      mainw->files[mt->render_file]->vsize = mt->user_height;
      mainw->files[mt->render_file]->pb_fps = mainw->files[mt->render_file]->fps = mt->fps = mt->user_fps;
      mainw->files[mt->render_file]->arps = mainw->files[mt->render_file]->arate = mt->user_arate;
      mainw->files[mt->render_file]->achans = mt->user_achans;
      mainw->files[mt->render_file]->asampsize = mt->user_asamps;
      mainw->files[mt->render_file]->signed_endian = mt->user_signed_endian;
    } else {
      mt->user_width = mainw->files[mt->render_file]->hsize = prefs->mt_def_width;
      mt->user_height = mainw->files[mt->render_file]->vsize = prefs->mt_def_height;
      mt->user_fps = mainw->files[mt->render_file]->pb_fps = mainw->files[mt->render_file]->fps = mt->fps = prefs->mt_def_fps;
      mt->user_arate = mainw->files[mt->render_file]->arate = mainw->files[mt->render_file]->arps = prefs->mt_def_arate;
      mt->user_achans = mainw->files[mt->render_file]->achans = prefs->mt_def_achans;
      mt->user_asamps = mainw->files[mt->render_file]->asampsize = prefs->mt_def_asamps;
      mt->user_signed_endian = mainw->files[mt->render_file]->signed_endian = prefs->mt_def_signed_endian;
    }
  }
  mainw->files[mt->render_file]->ratio_fps = check_for_ratio_fps(mainw->files[mt->render_file]->fps);

  if (mainw->files[mt->render_file]->hsize != hsize || mainw->files[mt->render_file]->vsize != vsize ||
      mainw->files[mt->render_file]->arate != arate || mainw->files[mt->render_file]->achans != achans ||
      mainw->files[mt->render_file]->asampsize != asamps || mainw->files[mt->render_file]->signed_endian != ase) {
...
void event_list_free_undos(lives_mt * mt) {
  ...
  if (mt->undos) lives_list_free(mt->undos);
  mt->undos = NULL;
  mt->undo_buffer_used = 0;
  mt->undo_offset = 0;
...
void stored_event_list_free_undos(void) {
  if (mainw->stored_layout_undos) lives_list_free(mainw->stored_layout_undos);
...
void stored_event_list_free_all(boolean wiped) {
  ...
  if (mainw->stored_event_list) event_list_free(mainw->stored_event_list);
...
static void set_mt_title(lives_mt * mt) {
  char *wtxt = lives_strdup_printf(_("Multitrack %dx%d : %d bpp %.3f fps"), mainw->files[mt->render_file]->hsize,
                                   mainw->files[mt->render_file]->vsize, mainw->files[mt->render_file]->bpp,
...
static void set_child_grad(LiVESWidget * widget, livespointer data) {
  char *tmp = (char *)data;
  if (widget == mainw->multitrack->timecode) return;
  set_css_value_direct(widget, LIVES_WIDGET_STATE_NORMAL, "",
                       "background-image", tmp);
  if (LIVES_IS_CONTAINER(widget))
    lives_container_foreach(LIVES_CONTAINER(widget), set_child_grad, tmp);
}
...
void set_mt_colours(lives_mt * mt) {
  ...
  lives_widget_set_bg_color(LIVES_WIDGET(mt->timeline), LIVES_WIDGET_STATE_NORMAL, &palette->normal_back);
  lives_widget_set_fg_color(LIVES_WIDGET(mt->timeline), LIVES_WIDGET_STATE_NORMAL, &palette->normal_fore);
  ...
  lives_tool_button_set_border_color(mt->grav_menuitem, LIVES_WIDGET_STATE_NORMAL, &palette->menu_and_bars);
  ...
  lives_tool_button_set_border_color(mt->mm_menuitem, LIVES_WIDGET_STATE_NORMAL, &palette->menu_and_bars);
  ...
  lives_tool_button_set_border_color(mt->ins_menuitem, LIVES_WIDGET_STATE_NORMAL, &palette->menu_and_bars);
  ...
  if (mt->fx_params_label) lives_widget_apply_theme2(mt->fx_params_label, LIVES_WIDGET_STATE_NORMAL, TRUE);
  ...
  if ((mt->poly_state == POLY_FX_STACK || mt->poly_state == POLY_EFFECTS || mt->poly_state == POLY_TRANS ||
      ...
      && LIVES_IS_BIN(mt->fx_list_scroll) && lives_bin_get_child(LIVES_BIN(mt->fx_list_scroll)))
  ...
  if (mainw->pretty_colours) {
    ...
    set_css_value_direct(lives_widget_get_parent(mt->btoolbar2), LIVES_WIDGET_STATE_NORMAL, "",
                         "background-image", tmp);
    set_child_grad(lives_widget_get_parent(mt->btoolbar2), tmp);
    lives_free(colref); lives_free(colref2);
    lives_free(tmp);
  }
#endif

  lives_widget_set_size_request(mainw->volume_scale, -1, -1);
...
lives_mt *multitrack(weed_plant_t *event_list, int orig_file, double fps) {
  ...
  if (mainw->multi_opts.set) {
    ...
    mt->avol_init_event = mt->opts.avol_init_event;
    mt->avol_fx = mt->opts.avol_fx;
  } else {
    ...
    mt->opts.avol_init_event = mt->avol_init_event = NULL;
    mt->opts.avol_fx = mt->avol_fx;
  }
...
# endif
  ...
  lives_widget_add_accelerator(mt->save_event_list, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_s, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_widget_add_accelerator(mt->quit, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_q, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->undo), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(multitrack_undo), (livespointer)mt);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->redo), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(multitrack_redo), (livespointer)mt);
  ...
  lives_widget_add_accelerator(mt->insert, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_i, LIVES_CONTROL_MASK, (LiVESAccelFlags)0);
  ...
  */
  ...
  lives_widget_add_accelerator(mt->jumpback, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_j, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_widget_add_accelerator(mt->jumpnext, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_l, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  for (i = 0; i < num_filters; i++) {
    ...
    if (filter && !weed_plant_has_leaf(filter, WEED_LEAF_HOST_MENU_HIDE)) {
      ...
      if (enabled_in_channels(filter, TRUE) >= 1000000 && enabled_out_channels(filter, FALSE) == 1) {
        fxname = weed_filter_idx_get_name(sorted, TRUE, TRUE);
        ...
        if (!is_pure_audio(filter, FALSE)) {
          ...
          lives_widget_object_set_data(LIVES_WIDGET_OBJECT(menuitem), "idx", LIVES_INT_TO_POINTER(sorted));
          lives_signal_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                               LIVES_GUI_CALLBACK(mt_add_block_effect), (livespointer)mt);
        ...
        if (get_transition_param(filter, FALSE) == -1) lives_container_add(LIVES_CONTAINER(submenu_menu2v), menuitem);
          ...
          if (has_video_chans_in(filter, FALSE)) {
            ...
            lives_signal_sync_connect(LIVES_GUI_OBJECT(menuitem2), LIVES_WIDGET_ACTIVATE_SIGNAL,
                                      LIVES_GUI_CALLBACK(mt_set_atrans_effect), (livespointer)mt);
      ...
      if (pkg) lives_free(pkg);
      if (fxname) lives_free(fxname);
    }
  }
  ...
  // FIXME **
  if (1 || mt->opts.back_audio_tracks > 0 || !mainw->files[mt->render_file]->achans)
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_split_curr_activate), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->split_sel), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_split_sel_activate), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->ins_gap_sel), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_insgap_sel_activate), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->ins_gap_cur), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_insgap_cur_activate), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->remove_gaps), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(remove_gaps), (livespointer)mt);

  lives_widget_add_accelerator(mt->remove_gaps, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_g, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->remove_first_gaps), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(remove_first_gaps), (livespointer)mt);

  lives_widget_add_accelerator(mt->remove_first_gaps, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_f, LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_selection_lock), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(select_from_zero_time), (livespointer)mt);

  menuitem = lives_standard_menu_item_new_with_label(_("Select to _End Time"));
  lives_container_add(LIVES_CONTAINER(mt->selection_menu), menuitem);

  lives_signal_connect(LIVES_GUI_OBJECT(menuitem), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(select_to_end_time), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->tc_to_rs), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(tc_to_rs), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->tc_to_re), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(tc_to_re), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->rs_to_tc), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(rs_to_tc), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->re_to_tc), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(re_to_tc), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->change_vals), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(mt_change_vals_activate), (livespointer)mt);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->backup), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_mt_backup_activate), (livespointer)mt);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->show_info), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(toggle_sets_pref), (livespointer)PREF_MT_SHOW_CTX);
  ...
  lives_widget_add_accelerator(mt->view_in_out, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_n, (LiVESXModifierType)0, LIVES_ACCEL_VISIBLE);
  ...
  lives_widget_add_accelerator(mt->view_effects, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_e, (LiVESXModifierType)0, LIVES_ACCEL_VISIBLE);
  ...
  lives_widget_add_accelerator(ccursor, LIVES_WIDGET_ACTIVATE_SIGNAL, mt->accel_group,
                               LIVES_KEY_c, (LiVESXModifierType)LIVES_CONTROL_MASK, LIVES_ACCEL_VISIBLE);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->quit), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_quit_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->load_vals), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_load_vals_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->ac_audio_check), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_ac_audio_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->aload_subs), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_boolean_toggled), &prefs->autoload_subs);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->clipedit), LIVES_WIDGET_ACTIVATE_SIGNAL,
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(view_mt_details), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(multitrack_view_details), (livespointer)mt);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(full_screen), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_full_screen_activate), NULL);
  mt->loop_cont_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->loop_continue), LIVES_WIDGET_TOGGLED_SIGNAL,
                       LIVES_GUI_CALLBACK(on_loop_cont_activate), NULL);
  mt->mute_audio_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->mute_audio), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->add_vid_front), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(add_video_track_front), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->render), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_render_activate), (livespointer)mt);
  lives_signal_connect(LIVES_GUI_OBJECT(mt->prerender_aud), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_prerender_aud_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->jumpback), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_jumpback_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->jumpnext), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_jumpnext_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->mark_jumpback), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_jumpback_mark_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->mark_jumpnext), LIVES_WIDGET_ACTIVATE_SIGNAL,
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->load_event_list), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_load_event_list_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->clear_event_list), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_clear_event_list_activate), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->view_audio), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_view_audio_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->change_max_disp), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_change_max_disp_tracks), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->render_vid), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_render_vid_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->render_aud), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_render_aud_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->normalise_aud), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_norm_aud_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(ign_ins_sel), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_ign_ins_sel_toggled), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(show_frame_events), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(show_frame_events_activate), NULL);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(ccursor), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_center_on_cursor), (livespointer)mt);
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->follow_play), LIVES_WIDGET_TOGGLED_SIGNAL,
                            LIVES_GUI_CALLBACK(mt_fplay_toggled), (livespointer)mt);
  lives_signal_connect(LIVES_GUI_OBJECT(zoom_in), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(mt_zoom_in), (livespointer)mt);
  lives_signal_connect(LIVES_GUI_OBJECT(zoom_out), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(mt_zoom_out), (livespointer)mt);
  mt->seltrack_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->select_track), LIVES_WIDGET_ACTIVATE_SIGNAL,
                      LIVES_GUI_CALLBACK(on_seltrack_activate), (livespointer)mt);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->expl_missing), LIVES_WIDGET_ACTIVATE_SIGNAL,
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(show_mt_keys), LIVES_WIDGET_ACTIVATE_SIGNAL,
                            LIVES_GUI_CALLBACK(on_mt_showkeys_activate), NULL);
  ...
  lives_signal_connect(LIVES_GUI_OBJECT(mt->view_effects), LIVES_WIDGET_ACTIVATE_SIGNAL,
                       LIVES_GUI_CALLBACK(on_mt_list_fx_activate), (livespointer)mt);
  ...
  lives_widget_set_sensitive(mainw->m_rewindbutton, (mt->event_list && get_first_event(mt->event_list)));
  ...
  lives_widget_set_sensitive(mainw->m_playbutton, (mt->event_list && get_first_event(mt->event_list)));
  ...
  widget_opts.text_size = LIVES_TEXT_SIZE_MEDIUM;
...
#if GTK_CHECK_VERSION(3, 16, 0)
  if (mainw->pretty_colours) {
    set_css_value_direct(LIVES_WIDGET(mt->timecode), LIVES_WIDGET_STATE_NORMAL, "", "border-top-left-radius", "20px");
    set_css_value_direct(LIVES_WIDGET(mt->timecode), LIVES_WIDGET_STATE_NORMAL, "", "border-top-right-radius", "20px");
    set_css_value_direct(LIVES_WIDGET(mt->timecode), LIVES_WIDGET_STATE_NORMAL, "", "border-bottom-right-radius", "20px");
    set_css_value_direct(LIVES_WIDGET(mt->timecode), LIVES_WIDGET_STATE_NORMAL, "", "border-bottom-left-radius", "20px");
  }
#endif

  mt_update_timecodes(mt, 0.);
  ...
  */
  ...
  mt->grav_normal_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->grav_normal), LIVES_WIDGET_TOGGLED_SIGNAL,
                         LIVES_GUI_CALLBACK(on_grav_mode_changed), (livespointer)mt);
  ...
  mt->grav_left_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->grav_left), LIVES_WIDGET_TOGGLED_SIGNAL,
                       LIVES_GUI_CALLBACK(on_grav_mode_changed), (livespointer)mt);
  ...
  mt->grav_right_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->grav_right), LIVES_WIDGET_TOGGLED_SIGNAL,
                        LIVES_GUI_CALLBACK(on_grav_mode_changed), (livespointer)mt);
  ...
  mt->mm_move_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->mm_move), LIVES_WIDGET_TOGGLED_SIGNAL,
  ...
  mt->ins_normal_func = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->ins_normal), LIVES_WIDGET_TOGGLED_SIGNAL,
                        LIVES_GUI_CALLBACK(on_insert_mode_changed), (livespointer)mt);

  lives_container_add(LIVES_CONTAINER(mt->ins_submenu), mt->ins_audmatch);
  ...
  if (!lives_scale_button_set_orientation(LIVES_SCALE_BUTTON(mainw->volume_scale),
                                          LIVES_ORIENTATION_HORIZONTAL)) {
    if (mainw->vol_label) {
  ...
  lives_box_pack_start(LIVES_BOX(mt->xtravbox), mt->hbox, FALSE, TRUE, 0);
  lives_widget_set_vexpand(mt->hbox, TRUE);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->preview_eventbox), LIVES_WIDGET_LEAVE_NOTIFY_EVENT,
                            LIVES_GUI_CALLBACK(on_framedraw_leave), NULL);
  ...
  mt->apply_fx_button = lives_standard_button_new_full(_("_Apply"), DEF_BUTTON_WIDTH >> 1,
                        DEF_BUTTON_HEIGHT, LIVES_BOX(hbox), TRUE, NULL);

  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->apply_fx_button), LIVES_WIDGET_CLICKED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_set_pvals_clicked), (livespointer)mt);
  ...
  lives_signal_sync_connect_after(LIVES_GUI_OBJECT(mt->solo_check), LIVES_WIDGET_TOGGLED_SIGNAL,
                                  LIVES_GUI_CALLBACK(on_solo_check_toggled), (livespointer)mt);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->resetp_button), LIVES_WIDGET_CLICKED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_resetp_clicked), (livespointer)mt);

  widget_opts.expand = LIVES_EXPAND_DEFAULT_HEIGHT;
  widget_opts.pack_end = TRUE;
  mt->del_node_button = lives_standard_button_new_full(_("_Del. node"), DEF_BUTTON_WIDTH >> 1,
                        -1, LIVES_BOX(hbox), TRUE, NULL);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->del_node_button), LIVES_WIDGET_CLICKED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_del_node_clicked), (livespointer)mt);

  widget_opts.pack_end = TRUE;
  mt->next_node_button = lives_standard_button_new_full(_("_Next node"), DEF_BUTTON_WIDTH >> 1,
                         -1, LIVES_BOX(hbox), TRUE, NULL);
  widget_opts.pack_end = FALSE;
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->next_node_button), LIVES_WIDGET_CLICKED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_next_node_clicked), (livespointer)mt);

  widget_opts.pack_end = TRUE;
  mt->prev_node_button = lives_standard_button_new_full(_("_Prev node"), DEF_BUTTON_WIDTH >> 1,
                         -1, LIVES_BOX(hbox), TRUE, NULL);
  widget_opts.pack_end = FALSE;
  widget_opts.expand = LIVES_EXPAND_DEFAULT;
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->prev_node_button), LIVES_WIDGET_CLICKED_SIGNAL,
                            LIVES_GUI_CALLBACK(on_prev_node_clicked), (livespointer)mt);
  ...
  mt->in_image = lives_standard_drawing_area_new(LIVES_GUI_CALLBACK(all_expose), &mt->insurface);
  ...
  lives_box_pack_start(LIVES_BOX(vbox), mt->in_frame, TRUE, TRUE, 0);

  lives_signal_connect(LIVES_GUI_OBJECT(mt->in_frame), LIVES_WIDGET_BUTTON_PRESS_EVENT,
                       LIVES_GUI_CALLBACK(in_out_ebox_pressed), (livespointer)mt);
  lives_signal_connect(LIVES_GUI_OBJECT(mt->in_frame), LIVES_WIDGET_BUTTON_RELEASE_EVENT,
                       LIVES_GUI_CALLBACK(on_drag_clip_end), (livespointer)mt);
  ...
  mt->checkbutton_start_anchored = lives_standard_check_button_new((tmp = (_("Anchor _start"))), FALSE,
                                   LIVES_BOX(mt->in_hbox), (tmp2 = (_("Anchor the start point to the timeline"))));
  lives_free(tmp);
  lives_free(tmp2);
  ...
  mt->spin_in_func = lives_signal_connect_after(LIVES_GUI_OBJECT(mt->spinbutton_in), LIVES_WIDGET_VALUE_CHANGED_SIGNAL,
                     LIVES_GUI_CALLBACK(in_out_start_changed), mt);

  mt->check_start_func = lives_signal_connect_after(LIVES_GUI_OBJECT(mt->checkbutton_start_anchored),
                         LIVES_WIDGET_TOGGLED_SIGNAL, LIVES_GUI_CALLBACK(in_anchor_toggled), mt);

  vbox = lives_vbox_new(FALSE, widget_opts.packing_height);

  lives_box_pack_end(LIVES_BOX(mt->in_out_box), vbox, TRUE, TRUE, widget_opts.packing_width);

  mt->out_image = lives_standard_drawing_area_new(LIVES_GUI_CALLBACK(all_expose), &mt->outsurface);
  ...
  lives_box_pack_start(LIVES_BOX(vbox), mt->out_frame, TRUE, TRUE, 0);
  ...
  mt->spin_out_func = lives_signal_sync_connect_after(LIVES_GUI_OBJECT(mt->spinbutton_out), LIVES_WIDGET_VALUE_CHANGED_SIGNAL,
                      LIVES_GUI_CALLBACK(in_out_end_changed), mt);

  mt->check_end_func = lives_signal_sync_connect_after(LIVES_GUI_OBJECT(mt->checkbutton_end_anchored),
                       LIVES_WIDGET_TOGGLED_SIGNAL, LIVES_GUI_CALLBACK(out_anchor_toggled), mt);
  ...
  mt->tlx_vbox = lives_vbox_new(FALSE, 0);
  ...
  lives_table_set_row_homogeneous(LIVES_TABLE(mt->timeline_table_header), FALSE);
  ...
  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->tlx_eventbox), LIVES_WIDGET_BUTTON_PRESS_EVENT,
                            LIVES_GUI_CALLBACK(on_track_header_click), (livespointer)mt);

  lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->tlx_eventbox), LIVES_WIDGET_BUTTON_RELEASE_EVENT,
                            LIVES_GUI_CALLBACK(on_track_header_release), (livespointer)mt);
  ...
  mt->mouse_mot1 = lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->tlx_eventbox), LIVES_WIDGET_MOTION_NOTIFY_EVENT,
                   LIVES_GUI_CALLBACK(on_track_header_move), (livespointer)mt);
  ...
  lives_box_pack_start(LIVES_BOX(mt->tlx_vbox), mt->tl_hbox, TRUE, TRUE, widget_opts.packing_height >> 1);
  ...
  if (prefs->show_msg_area) {
    ...
    mt->msg_area = mainw->msg_area =
                     lives_standard_drawing_area_new(LIVES_GUI_CALLBACK(reshow_msg_area),
                         &mainw->msg_surface);
  ...
  if (prefs->show_msg_area) lives_box_pack_start(LIVES_BOX(mt->vpaned), mt->message_box, TRUE, TRUE, 0);
  ...
  menu_sets_visible(LIVES_CHECK_MENU_ITEM(mt->show_info), mt->context_frame, FALSE);
...
void delete_audio_track(lives_mt * mt, LiVESWidget * eventbox, boolean full) {
  ...
  if (xeventbox) {
    ...
    lives_widget_destroy(label);
    if (LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(xeventbox), HIDDEN_KEY)) == 0) {
...
boolean multitrack_delete(lives_mt * mt, boolean save_layout) {
  ...
  mt->opts.avol_init_event = mt->avol_init_event;
  mt->opts.avol_fx = mt->avol_fx;
  ...
  if (mt->amixer) on_amixer_close_clicked(NULL, mt);
  ...
  mainw->multi_opts.ptr_time = mt->ptr_time;

  mainw->multi_opts.set = TRUE;

  if (mt->insurface) lives_painter_surface_destroy(mt->insurface);
  if (mt->outsurface) lives_painter_surface_destroy(mt->outsurface);
  ...
  if (mt->selected_tracks) lives_list_free(mt->selected_tracks);

  if (mainw->event_list == mt->event_list) mainw->event_list = NULL;
  if (mt->event_list) event_list_free(mt->event_list);
  mt->event_list = NULL;

  if (mt->clip_selected >= 0 && mainw->files[mt_file_from_clip(mt, mt->clip_selected)])
  ...
  // also, clears out any events for widgets we are going to destroy in switch_fo_file
  lives_widget_context_update(); // IMPORTANT !!!
  /////////////////////////////////
...
static track_rect *add_block_start_point(LiVESWidget * eventbox, weed_timecode_t tc, int filenum,
  ...
  if (!block) lives_widget_object_set_data(LIVES_WIDGET_OBJECT(eventbox), "blocks", (livespointer)new_block);
...
static track_rect *add_block_end_point(LiVESWidget * eventbox, weed_plant_t *event) {
  ...
  if (block) block->end_event = event;
  return block;
}
...
void mt_init_tracks(lives_mt * mt, boolean set_min_max) {
  LiVESList *tlist = mt->audio_draws;

  while (mt->audio_draws) {
    if (mt->audio_draws->data) lives_widget_destroy((LiVESWidget *)mt->audio_draws->data);
  ...
  while (mt->video_draws) {
    if (mt->video_draws->data) lives_widget_destroy((LiVESWidget *)mt->video_draws->data);
    mt->video_draws->data = NULL;
  ...
  if (!mt->timeline_table) {
    ...
    lives_table_attach(LIVES_TABLE(mt->timeline_table_header), mt->tl_label, 0, 7, 0, 2,
  ...
  if (mainw->files[mt->render_file]->achans > 0 && mt->opts.back_audio_tracks > 0) {
    for (int at = 1; at <= mt->opts.back_audio_tracks; at++) {
      add_audio_track(mt, -at, FALSE);
    }
  }
  ...
  if (!mt->timeline_eb) {
    ...
    mt->timeline_reg = lives_standard_drawing_area_new(LIVES_GUI_CALLBACK(all_expose),
                       &mt->tl_reg_surf);
    ...
    lives_signal_connect(LIVES_GUI_OBJECT(mt->timeline_eb), LIVES_WIDGET_BUTTON_RELEASE_EVENT,
                         LIVES_GUI_CALLBACK(on_timeline_release), (livespointer)mt);
  ...
  if (mt->event_list) {
    ...
    if (navs > 0) {
      ...
      lives_free(navals);
      num_tracks = maxval + mt->opts.back_audio_tracks;;
        ...
        for (j = 0; j < num_tracks; j++) {
          ...
          if (clip_index[j] > 0 && frame_index[j] > -1 && clip_index[j] <= MAX_FILES &&
            ...
            if ((tracks[j] != renumbered_clips[clip_index[j]]) || forced_end) {
              ...
              if (tracks[j] > 0) {
                // end of previous rectangle
                add_block_end_point(LIVES_WIDGET(lives_list_nth_data(mt->video_draws, j)), last_event);
              }
              if (clip_index[j] > 0) {
	...
	// *INDENT-ON*
        ...
        if (!next_frame_event) {
          ...
          j = 0;
          for (slist = mt->video_draws; slist; slist = slist->next) {
            if (tracks[j++] > 0) {
...
LiVESWidget *add_audio_track(lives_mt * mt, int track, boolean behind) {
  ...
  LiVESWidget *label, *dummy, *arrow;
  LiVESWidget *eventbox, *vbox;
  ...
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(audio_draw), HIDDEN_KEY, LIVES_INT_TO_POINTER(0));
  ...
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(arrow), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(track));
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(audio_draw), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(track));
  ...
  for (i = 0; i < mainw->files[mt->render_file]->achans; i++) {
    ...
    tname = get_achannel_name(mainw->files[mt->render_file]->achans, i);
    ...
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(eventbox), HIDDEN_KEY, LIVES_INT_TO_POINTER(0));
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(eventbox), "bgimg", NULL);
  ...
  if (!mt->was_undo_redo && !mainw->stored_event_list) {
    if (mt->avol_init_event) {
  ...
  if (!mt->was_undo_redo && mt->amixer && track >= 0) {
  ...
  if (!mt->was_undo_redo && !mainw->stored_event_list) {
    if (mt->avol_init_event) apply_avol_filter(mt);
  }
  return audio_draw;
}
...
static int add_video_track(lives_mt * mt, boolean behind) {
  ...
  if (mt->audio_draws && mt->audio_draws->data && mt->opts.back_audio_tracks > 0 &&
      LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(mt->audio_draws->data), HIDDEN_KEY)) == 0) {
    max_disp_vtracks--;
    if (LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(mt->audio_draws->data),
                             "expanded"))) max_disp_vtracks -= mainw->files[mt->render_file]->achans;
  }
  ...
  if (!behind) {
    ...
    // shift all rows down
    // increment LAYER_NUMBER_KEYs, change labels
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(eventbox), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(0));
    for (i = 0; i < mt->num_video_tracks - 1; i++) {
      ...
      lives_widget_object_set_data(LIVES_WIDGET_OBJECT(xeventbox), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(i + 1));
      lives_widget_object_set_data(LIVES_WIDGET_OBJECT(xcheckbutton), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(i + 1));
      lives_widget_object_set_data(LIVES_WIDGET_OBJECT(xarrow), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(i + 1));
      ...
      if (mt->opts.pertrack_audio) {
        ...
        lives_widget_object_set_data(LIVES_WIDGET_OBJECT(aeventbox), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(i + 1));
    ...
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(eventbox), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(mt->num_video_tracks - 1));
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(checkbutton), LAYER_NUMBER_KEY,
                                 LIVES_INT_TO_POINTER(mt->num_video_tracks - 1));
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(arrow), LAYER_NUMBER_KEY, LIVES_INT_TO_POINTER(mt->num_video_tracks - 1));
...
int add_video_track_behind(LiVESMenuItem * menuitem, livespointer user_data) {
  int tnum;
  lives_mt *mt = (lives_mt *)user_data;
  if (menuitem) mt_desensitise(mt);
  tnum = add_video_track(mt, TRUE);
  if (menuitem) mt_sensitise(mt);
  return tnum;
}
  ...
  if (!LIVES_IS_PLAYING) {
    ...
    if (mt->fx_order == FX_ORD_NONE && WEED_EVENT_IS_FILTER_MAP(mt->fm_edit_event)) {
      if (init_event_is_process_last(mt->selected_init_event)) {
        ...
        if (mt->fx_ibefore_button) lives_widget_set_sensitive(mt->fx_ibefore_button, FALSE);
        if (mt->fx_iafter_button) lives_widget_set_sensitive(mt->fx_iafter_button, FALSE);
      } else {
        do_fx_move_context(mt);
        if (mt->fx_ibefore_button) lives_widget_set_sensitive(mt->fx_ibefore_button, TRUE);
        if (mt->fx_iafter_button) lives_widget_set_sensitive(mt->fx_iafter_button, TRUE);
      }
    }
...
void mt_clear_timeline(lives_mt * mt) {
  ...
  if (mainw->files[mt->render_file]->achans == 0) mt->opts.pertrack_audio = FALSE;
  ...
  // reset avol_fx
  if (mainw->files[mt->render_file]->achans > 0 && mainw->fx_candidates[FX_CANDIDATE_AUDIO_VOL].delegate != -1) {
    // user (or system) has delegated an audio volume filter from the candidates
    mt->avol_fx = LIVES_POINTER_TO_INT(lives_list_nth_data(mainw->fx_candidates[FX_CANDIDATE_AUDIO_VOL].list,
                                       mainw->fx_candidates[FX_CANDIDATE_AUDIO_VOL].delegate));
  } else mt->avol_fx = -1;
...
void mt_delete_clips(lives_mt * mt, int file) {
  ...
  for (; list; i++) {
    ...
    if (clips_to_files[i] == file) {
      ...
      if (clist->prev) clist->prev->next = clist_nnext;
  ...
  if (olist) lives_list_free(olist);

  if (mt->event_list && removed && used_in_current_layout(mt, file)) {
    int current_file = mainw->current_file;

    if (!event_list_rectify(mt, mt->event_list) || !get_first_event(mt->event_list)) {
  ...
  while (add || cliplist) {
    ...
    if (i != mainw->scrap_file && i != mainw->ascrap_file) {
      if (i == orig_file || (mt->clip_selected == -1 && i == mainw->pre_src_file)) {
        ...
        if (mainw->files[i]->tcache_dubious_from > 0)
          free_thumb_cache(i, mainw->files[i]->tcache_dubious_from);
      }
      ...
      thumbnail = mt_make_thumb(mt, i, width, height, mainw->files[i]->start, LIVES_INTERP_BEST, TRUE);
      ...
      lives_signal_sync_connect(LIVES_GUI_OBJECT(eventbox), LIVES_WIDGET_BUTTON_PRESS_EVENT,
                                LIVES_GUI_CALLBACK(clip_ebox_pressed), (livespointer)mt);
      lives_signal_sync_connect(LIVES_GUI_OBJECT(eventbox), LIVES_WIDGET_BUTTON_RELEASE_EVENT,
                                LIVES_GUI_CALLBACK(on_drag_clip_end), (livespointer)mt);
      if (add) {
        lives_widget_set_no_show_all(mt->poly_box, FALSE);
        lives_widget_show_all(mt->poly_box);
    ...
    if (cliplist) cliplist = cliplist->next;
  }
}
...
static void set_audio_mixer_vols(lives_mt * mt, weed_plant_t *elist) {
  int *atracks;
  double *avols;
  int catracks, xtrack, xavol, natracks, navols;

  atracks = weed_get_int_array_counted(elist, WEED_LEAF_AUDIO_VOLUME_TRACKS, &natracks);
  if (!atracks) return;
  avols = weed_get_double_array_counted(elist, WEED_LEAF_AUDIO_VOLUME_VALUES, &navols);
  if (!avols) return;

  catracks = lives_list_length(mt->audio_vols);
  for (int i = 0; i < natracks; i++) {
    xtrack = atracks[i];
...
boolean on_multitrack_activate(LiVESMenuItem * menuitem, weed_plant_t *event_list) {
  ...
  if (prefs->mt_enter_prompt && !mainw->stored_event_list && prefs->show_gui && !(mainw->recoverable_layout &&
    ...
    lives_set_cursor_style(LIVES_CURSOR_BUSY, LIVES_MAIN_WINDOW_WIDGET);
  ...
  } else prefs->letterbox_mt = future_prefs->letterbox_mt;
...
track_rect *move_block(lives_mt * mt, track_rect * block, double timesecs, int old_track, int new_track) {
  ...
  if (mt->opts.insert_mode == INSERT_MODE_NORMAL) {
    ...
    while (tc <= tclen) {
      ...
      if (!event) break; // must be end of timeline
      if (new_track >= 0) {
  ...
  while (mt->specific_event && get_event_timecode(mt->specific_event) == start_tc) {
  ...
  if (block && (mt->opts.grav_mode == GRAV_MODE_LEFT ||
                (mt->opts.grav_mode == GRAV_MODE_RIGHT && block->next)) &&
      !did_backup) {
    ...
    if (mt->opts.grav_mode == GRAV_MODE_LEFT) {
      ...
      if (rtc >= 0.) {
        lblock = block->prev;
        if (lblock) rstart = get_event_timecode(lblock->end_event) / TICKS_PER_SECOND_DBL;
      }
    ...
    if (old_track > -1) {
      ...
      if (tracks_sel) lives_list_free(tracks_sel);
  ...
  if (!did_backup) {
    if (mt->avol_fx != -1 && (!block || !block->next) && mt->audio_draws &&
        mt->audio_draws->data && get_first_event(mt->event_list)) {
      apply_avol_filter(mt);
    }
  }
  ...
  if (!did_backup && mt->framedraw && mt->current_rfx && mt->init_event &&
...
void unselect_all(lives_mt * mt) {
  ...
  if (mt->block_selected) lives_widget_queue_draw(mt->block_selected->eventbox);
...
static void _clear_context(lives_mt * mt) {
  ...
  if (palette->style & STYLE_1) {
    lives_widget_set_fg_color(lives_bin_get_child(LIVES_BIN(mt->context_scroll)), LIVES_WIDGET_STATE_NORMAL,
                              &palette->normal_fore);
    lives_widget_set_bg_color(lives_bin_get_child(LIVES_BIN(mt->context_scroll)), LIVES_WIDGET_STATE_NORMAL,
                              &palette->normal_back);
  }
...
#define NEW_BLOCK_ZOOM_EXTRA 0.2
...
boolean resize_timeline(lives_mt * mt) {
  ...
  if (mt->new_block) {
    double tlstart = get_event_timecode(mt->new_block->start_event) / TICKS_PER_SECOND_DBL;
    double tlend = get_event_timecode(mt->new_block->end_event) / TICKS_PER_SECOND_DBL;
    double tl_span = (tlend - tlstart), new_span, old_span;
    double tl_cur = mt->ptr_time;
    double scale;

    mt->ptr_time = tlstart + tl_span / 2.;

    tlstart -= tl_span * NEW_BLOCK_ZOOM_EXTRA;
    if (tlstart < 0.) tlstart = 0.;

    tlend += tl_span * NEW_BLOCK_ZOOM_EXTRA;

    new_span = (tlend - tlstart);
    old_span = (mt->tl_max - mt->tl_min);

    scale = new_span / old_span;

    if (mt->tl_max > end_secs) end_secs = mt->tl_max;
    set_timeline_end_secs(mt, end_secs);

    if (scale > 1.) mt_zoom(mt, -scale);
...
static void update_in_image(lives_mt * mt) {
  ...
  if (!mt->insurface) return;

  if (block) {
    track = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(block->eventbox), LAYER_NUMBER_KEY));
  ...
  calc_maxspect((lives_widget_get_allocation_width(mt->in_frame) >> 1) << 1,
                (lives_widget_get_allocation_height(mt->in_frame) >> 1) << 1, &width,
                &height);

  thumb = mt_make_thumb(mt, filenum,
...
static void update_out_image(lives_mt * mt, weed_timecode_t end_tc) {
  ...
  if (block) {
    ...
    if (frame_end >= mainw->files[filenum]->frames) frame_end = mainw->files[filenum]->frames - 1;
  } else {
    filenum = mt->file_selected;
    frame_end = mainw->files[filenum]->end;
  }

  calc_maxspect((lives_widget_get_allocation_width(mt->out_frame) >> 1) << 1,
                (lives_widget_get_allocation_height(mt->out_frame) >> 1) << 1, &width,
                &height);
...
boolean show_in_out_images(livespointer user_data) {
  ...
  // wait for config events which will create insurface and outsurface
  if (!mt->insurface || !mt->outsurface) return TRUE;
...
void in_out_start_changed(LiVESWidget * widget, livespointer user_data) {
  ...
  if (!start_anchored) {
    if (new_start_tc > block->offset_start) {
      ...
      while (event && get_event_timecode(event) < tl_start) event = get_next_event(event);

      while (event && get_event_timecode(event) < new_tl_tc) {
      ...
      while (event && get_event_timecode(event) < tl_start) {
        start_event = event;
        event = get_next_event(event);
      }
      while (event && get_event_timecode(event) == tl_start) {
  ...
  if (mt->poly_state == POLY_IN_OUT) {
    mt_set_in_out_spin_ranges(mt, new_start_tc, offset_end);
    lives_signal_handler_block(mt->spinbutton_out, mt->spin_out_func);
    lives_signal_handler_block(mt->spinbutton_in, mt->spin_in_func);
  ...
  if (!resize_timeline(mt)) {
    mt_redraw_eventbox(mt, block->eventbox);
    mt_paint_lines(mt, mt->ptr_time, TRUE, NULL);
  }
...
void in_out_end_changed(LiVESWidget * widget, livespointer user_data) {
	...
	// *INDENT-ON*
      ...
      if (track >= 0) {
        ...
        insert_frames(filenum, offset_end, new_end_tc, tl_end + (weed_timecode_t)(TICKS_PER_SECOND_DBL / mt->fps),
                      LIVES_DIRECTION_FORWARD, block->eventbox, mt, block);
      ...
      if (ablock) {
        ...
        if (!new_end_event) {
          ...
          mt->event_list = insert_blank_frame_event_at(mt->event_list,
                           q_gint64(new_tl_tc + TICKS_PER_SECOND_DBL / mt->fps, mt->fps), &shortcut);
          ablock->end_event = shortcut;
        } else ablock->end_event = new_end_event;
  ...
  // *INDENT-ON*
    ...
    if (track >= 0) insert_frames(filenum, offset_end, offset_start, tl_end +
                                    (weed_timecode_t)((double)(track >= 0
                                        && !mt->aud_track_selected)*TICKS_PER_SECOND_DBL / mt->fps),
  ...
  if (mt->avol_fx != -1 && mt->avol_init_event && mt->audio_draws &&
      mt->audio_draws->data && !block->next) {
    apply_avol_filter(mt);
  }

  if (mt->poly_state == POLY_IN_OUT) {
    mt_set_in_out_spin_ranges(mt, block->offset_start, new_end_tc);
    lives_signal_handler_block(mt->spinbutton_out, mt->spin_out_func);
    lives_signal_handler_block(mt->spinbutton_in, mt->spin_in_func);
  ...
  if (!resize_timeline(mt)) {
    mt_redraw_eventbox(mt, block->eventbox);
    if (ablock && ablock != block) mt_redraw_eventbox(mt, block->eventbox);
    mt_paint_lines(mt, mt->ptr_time, TRUE, NULL);
    // TODO - redraw chans ??
  }
  if (prefs->mt_auto_back >= 0) {
...
void avel_reverse_toggled(LiVESToggleButton * togglebutton, livespointer user_data) {
  ...
  if (avel < 0.) mt_set_in_out_spin_ranges(mt, old_in_val * TICKS_PER_SECOND_DBL, old_out_val * TICKS_PER_SECOND_DBL);
  else mt_set_in_out_spin_ranges(mt, old_out_val * TICKS_PER_SECOND_DBL, old_in_val * TICKS_PER_SECOND_DBL);
...
void avel_spin_changed(LiVESSpinButton * spinbutton, livespointer user_data) {
  ...
  if (!block->end_anchored) {
    ...
    if (block->next && new_tl_tc >= get_event_timecode(block->next->start_event)) {
    ...
    if (new_tl_tc != old_tl_tc) {
      ...
      if (!block->next || block->next->start_event != block->end_event) remove_audio_for_track(block->end_event, -1);
  ...
  min_tc = 0;
  if (block->prev) min_tc = get_event_timecode(block->prev->end_event);
  ...
  if (was_adjusted || new_tl_tc != old_tl_tc) {
    ...
    if (new_tl_tc > old_tl_tc) shortcut = block->start_event;
      ...
      if (block->prev) shortcut = block->prev->end_event;
      else shortcut = NULL;
    }
    ...
    mt_set_in_out_spin_ranges(mt, start_tc, new_end_tc);
    lives_signal_handler_block(mt->spinbutton_out, mt->spin_out_func);
    lives_signal_handler_block(mt->spinbutton_in, mt->spin_in_func);
...
void in_anchor_toggled(LiVESToggleButton * togglebutton, livespointer user_data) {
  ...
  mt_set_in_out_spin_ranges(mt, block->offset_start, offset_end);
  ...
  if (mt->current_track >= 0 && mt->opts.pertrack_audio) {
    ...
    if (xeventbox) {
      ...
      if (xblock) xblock->start_anchored = block->start_anchored;
    }
  }
}
...
void out_anchor_toggled(LiVESToggleButton * togglebutton, livespointer user_data) {
  ...
  if (mt->current_track >= 0 && mt->opts.pertrack_audio) {
    ...
    if (xeventbox) {
      xblock = get_block_from_time(xeventbox, get_event_timecode(block->start_event) / TICKS_PER_SECOND_DBL, mt);
      if (xblock) xblock->end_anchored = block->end_anchored;
    }
  }
}
...
void polymorph(lives_mt * mt, lives_mt_poly_state_t poly) {
  ...
  case POLY_FX_STACK:
      ...
      if (num_fx > 0) {
        for (i = 0; i < num_fx; i++) {
          ...
          if (init_event) {
            ...
            lives_free(fhash);
            fname = weed_filter_idx_get_name(fidx, FALSE, FALSE);
            ...
            if (!is_input) {
              ...
              if (def_out_track > -1) {
                ...
                olayer = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(yeventbox), LAYER_NUMBER_KEY));
    ...
    if (has_effect) add_hsep_to_box(LIVES_BOX(mt->fx_list_box));
    ...
    widget_opts.expand = LIVES_EXPAND_DEFAULT_WIDTH;
    mt->prev_fm_button
      = lives_standard_button_new_with_label(_("_Prev filter map"),
    ...
    if (fxcount > 1) {
      ...
      lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->fx_ibefore_button), LIVES_WIDGET_CLICKED_SIGNAL,
                                LIVES_GUI_CALLBACK(on_fx_insb_clicked), (livespointer)mt);
      ...
      lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->fx_iafter_button), LIVES_WIDGET_CLICKED_SIGNAL,
                                LIVES_GUI_CALLBACK(on_fx_insa_clicked), (livespointer)mt);
    ...
    lives_signal_sync_connect(LIVES_GUI_OBJECT(mt->next_fm_button), LIVES_WIDGET_CLICKED_SIGNAL,
                              LIVES_GUI_CALLBACK(on_next_fm_clicked), (livespointer)mt);
...
static void mouse_select_move(LiVESWidget * widget, LiVESXEventMotion * event, lives_mt * mt) {
  ...
  if (mt->block_selected) unselect_all(mt);
...
void do_block_context(lives_mt * mt, LiVESXEventButton * event, track_rect * block) {
  ...
  if (block->ordered) { // TODO
    ...
    block_start_time = get_event_timecode(block->start_event) / TICKS_PER_SECOND_DBL + 1. / mainw->files[mt->render_file]->fps;
    block_end_time = get_event_timecode(block->end_event) / TICKS_PER_SECOND_DBL + (double)(!is_audio_eventbox(
                       block->eventbox)) / mainw->files[mt->render_file]->fps;
  ...
  if (is_audio_eventbox(block->eventbox) && mt->avol_init_event) {
    ...
    lives_free(avol_fxname); lives_free(text);
...
void do_track_context(lives_mt * mt, LiVESXEventButton * event, double timesecs, int track) {
  ...
  if (mt->file_selected > 0 && ((track < 0 && mainw->files[mt->file_selected]->achans > 0 &&
    ...
    if (track >= 0) {
      ...
      lives_signal_connect(LIVES_GUI_OBJECT(insert_here), LIVES_WIDGET_ACTIVATE_SIGNAL,
                           LIVES_GUI_CALLBACK(insert_at_ctx_cb), (livespointer)mt);
    } else {
      insert_here = lives_standard_menu_item_new_with_label(_("_Insert Audio Here"));
      lives_signal_connect(LIVES_GUI_OBJECT(insert_here), LIVES_WIDGET_ACTIVATE_SIGNAL,
                           LIVES_GUI_CALLBACK(insert_audio_at_ctx_cb), (livespointer)mt);
    }
  ...
  if (has_something) {
    ...
    lives_signal_connect(LIVES_GUI_OBJECT(menu), LIVES_WIDGET_UNMAP_SIGNAL,
                         LIVES_GUI_CALLBACK(rdrw_cb), (livespointer)mt);
...
boolean on_track_release(LiVESWidget * eventbox, LiVESXEventButton * event, livespointer user_data) {
  ...
  if (track != -1) {
    for (list = mt->video_draws; list; list = list->next) {
      ...
      if (lives_widget_get_xwindow(xeventbox) == window || lives_widget_get_xwindow(xlabelbox) == window ||
          lives_widget_get_xwindow(xahbox) == window) {
        track = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(xeventbox), LAYER_NUMBER_KEY));
        mt->aud_track_selected = FALSE;
        got_track = TRUE;
        break;
      }
  ...
  if (mt->opts.mouse_mode == MOUSE_MODE_SELECT) {
    ...
    if (got_track && !mt->is_rendering && mt->putative_block && !LIVES_IS_PLAYING &&
        event->button == 1) {
      ...
      tcpp = TICKS_PER_SECOND_DBL * ((mt->tl_max - mt->tl_min) /
                                     (double)lives_widget_get_allocation_width
                                     (LIVES_WIDGET(lives_list_nth_data(mt->video_draws, 0))));
...
boolean on_track_click(LiVESWidget * eventbox, LiVESXEventButton * event, livespointer user_data) {
  ...
  if (mt->opts.mouse_mode == MOUSE_MODE_SELECT && event->type == LIVES_BUTTON_PRESS) {
    ...
    if (lives_event_get_time((LiVESXEvent *)event) - last_press_time < capable->dclick_time
        && (x - last_x) * (x - last_x) + (y - last_y) * (y - last_y) < capable->dclick_dist * capable->dclick_dist) {
      doubleclick = TRUE;
      return TRUE;
    } else {
      ...
      if (event->time != dclick_time) {
        ...
        if (block) {
          ...
          if (!mt->is_rendering) {
            ...
            if (event->button == 1) {
              if (block && (mt->tl_min * TICKS_PER_SECOND_DBL > get_event_timecode(block->start_event))) {
                mt->putative_block = NULL;
                return TRUE;
              }
              if (!is_audio_eventbox(eventbox))
...
static void _animate_multitrack(lives_mt * mt) {
  ...
  mt_paint_lines(mt, currtime, TRUE, NULL);
}
...
static boolean multitrack_end(LiVESMenuItem * menuitem, livespointer user_data) {
  lives_mt *mt = (lives_mt *)user_data;
  return multitrack_delete(mt, !(prefs->warning_mask & WARN_MASK_EXIT_MT) || !menuitem);
}
...
static void mt_view_audio_toggled(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (!mt->opts.show_audio) lives_widget_object_set_data(LIVES_WIDGET_OBJECT(mt->audio_draws->data), HIDDEN_KEY,
        LIVES_INT_TO_POINTER(TRACK_I_HIDDEN_USER));
  else lives_widget_object_set_data(LIVES_WIDGET_OBJECT(mt->audio_draws->data), HIDDEN_KEY, LIVES_INT_TO_POINTER(0));
...
static void remove_gaps_inner(LiVESMenuItem * menuitem, livespointer user_data, boolean only_first) {
  ...
  while (vsel || (mt->current_track == -1 && !audio_done)) {
    offset = 0;
...
static void split_block(lives_mt * mt, track_rect * block, weed_timecode_t tc, int track, boolean no_recurse) {
  ...
  if (!is_audio_eventbox(eventbox)) {
    if (!no_recurse) {
      ...
      if (aeventbox) {
        ...
        if (ablock) split_block(mt, ablock, tc + TICKS_PER_SECOND_DBL / mt->fps, track, TRUE);
      }
    }
    ...
    if (!no_recurse) {
      ...
      if (oeventbox) split_block(mt, get_block_from_time(oeventbox, tc / TICKS_PER_SECOND_DBL - 1. / mt->fps, mt),
  ...
  if (block->ordered ||
      (is_audio_eventbox(eventbox))) offset_start = block->offset_start - get_event_timecode(start_event)
...
static void insgap_inner(lives_mt * mt, int tnum, boolean is_sel, int passnm) {
  ...
  // FILTER_INITs and FILTER_DEINITS - we move the filter init/deinit if "move effects with blocks" is selected
  // and all in_tracks are in the tracks to be moved
  ...
  case 1:
    ...
    if (tnum >= 0 && mt->opts.pertrack_audio) {
      ...
      if (aeventbox) {
        ...
        if (ablock) {
          while (ablock->next) ablock = ablock->next;
    ...
    while (event) {
      if (WEED_EVENT_IS_FRAME(event)) {
        ...
        if (new_event != event) {
          ...
          if (ablock && ablock->start_event == new_event) {
            ablock = ablock->prev;
            if (ablock && event == ablock->end_event) ablock->end_event = new_event;
          }
          mt_fixup_events(mt, event, new_event);
        }
      }
  ...
  case 2:
    ...
    while (event && (tc = get_event_timecode(event)) >= start_tc) {
      if (WEED_EVENT_IS_FILTER_DEINIT(event)) {
        init_event = (weed_plant_t *)weed_get_voidptr_value(event, WEED_LEAF_INIT_EVENT, NULL);
        ...
        if (nintracks == 2) {
          move_event_left(mt->event_list, event, TRUE, mt->fps);
          if (init_tc >= start_tc && init_event != mt->avol_init_event)
            move_event_right(mt->event_list, init_event, TRUE, mt->fps);
        }
      }
      event = get_prev_event(event);
    }
...
void on_insgap_sel_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  tstart = mt_time_to_string(QUANT_TIME(mt->region_start));
  tend = mt_time_to_string(QUANT_TIME(mt->region_end));
...
void multitrack_undo(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (last_undo->action != MT_UNDO_NONE) {
    ...
    if (!get_first_event(mt->event_list)) {
      event_list_free(mt->event_list);
      mt->event_list = NULL;
    }
    ...
    if (aparam_view_list) lives_list_free(aparam_view_list);
    ...
    if (mt->event_list) remove_markers(mt->event_list);
    ...
    if (seltracks) lives_list_free(seltracks);
  ...
  lives_widget_queue_draw(mt->tlx_vbox);
  ...
  if (mt->poly_state == POLY_PARAMS) {
    if (mt->last_fx_type == MT_LAST_FX_BLOCK && mt->block_selected) polymorph(mt, POLY_FX_STACK);
...
void mt_add_region_effect(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (end_tc > last_frame_tc) mt->event_list = add_blank_frames_up_to(mt->event_list, last_frame_event,
        end_tc - (double)(tracks[0] < 0) * TICKS_PER_SECOND_DBL / mt->fps, mt->fps);
  ...
  // create user message
  filter_name = weed_filter_idx_get_name(mt->current_fx, FALSE, FALSE);
  ...
  case 2:
    ...
    track_desc = lives_strdup_printf(_("tracks %s and %s"), (tmp1 = get_track_name(mt, tracks[0], FALSE)),
                                     (tmp = get_track_name(mt, tracks[1], FALSE)));
    lives_free(tmp);
    lives_free(tmp1);
    break;
  default:
  ...
  tstart = mt_time_to_string(QUANT_TICKS(start_tc));
  tend = mt_time_to_string(QUANT_TICKS(end_tc + TICKS_PER_SECOND_DBL / mt->fps));

  d_print(_("Added %s %s to %s from time %s to time %s\n"), tname, filter_name, track_desc, tstart, tend);

  lives_free(tstart); lives_free(tend); lives_free(filter_name);
  lives_free(tname); lives_free(track_desc);
...
boolean on_render_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  int arate = mainw->files[mt->render_file]->arate;
  int arps = mainw->files[mt->render_file]->arps;
  int asampsize = mainw->files[mt->render_file]->asampsize;
  int achans = mainw->files[mt->render_file]->achans;
  int signed_endian = mainw->files[mt->render_file]->signed_endian;
  ...
  if (render_to_clip(FALSE)) {
    ...
    if (mainw->files[mt->render_file]->frames == 0) {
      mainw->files[mt->render_file]->hsize = mainw->files[mt->render_file]->vsize = 0;
    }
    set_undoable(NULL, FALSE);
    mainw->files[mt->render_file]->changed = TRUE;
    add_to_clipmenu();
    ...
    if (prefs->crash_recovery) add_to_recovery_file(mainw->files[mt->render_file]->handle);
    reset_clipmenu();
...
void update_filter_events(lives_mt * mt, weed_plant_t *first_event, weed_timecode_t start_tc, weed_timecode_t end_tc,
  ...
  if (moved_events) lives_list_free(moved_events);
}
...
void on_split_sel_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  while (selt) {
    ...
    eventbox = (LiVESWidget *)lives_list_nth_data(mt->video_draws, track);
    block = get_block_from_time(eventbox, timesecs, mt);
    if (block) split_block(mt, block, timesecs * TICKS_PER_SECOND_DBL, track, FALSE);
    selt = selt->next;
  }
...
static void on_delblock_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if ((aeventbox = LIVES_WIDGET(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "atrack"))) != NULL) {
    ...
    if (mt->block_selected) on_delblock_activate(NULL, user_data);
    mt->block_selected = block;
    mt->current_track = current_track;
  }
  ...
  while (first_event && get_event_timecode(first_event) == start_tc) {
  ...
  if (cfile->achans > 0 && mt->audio_draws && mt->opts.back_audio_tracks > 0 && eventbox == mt->audio_draws->data &&
    ...
    if (xeventbox) lives_widget_queue_draw(xeventbox);
    xeventbox = (LiVESWidget *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "achan1");
    if (xeventbox) lives_widget_queue_draw(xeventbox);
  }
  ...
  if ((!mt->moving_block || !get_first_frame_event(mt->event_list)) && mt->avol_fx != -1 && !blocknext &&
      mt->audio_draws && get_first_event(mt->event_list)) {
    /// owwww...
    apply_avol_filter(mt);
  }
...
void on_seltrack_toggled(LiVESWidget * checkbutton, livespointer user_data) {
  int track = LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(checkbutton), LAYER_NUMBER_KEY));
  lives_mt *mt = (lives_mt *)user_data;
...
void mt_prepare_for_playback(lives_mt * mt) {
  ...
  if (mt->mt_frame_preview) {
    // put blank back in preview window
    if (palette->style & STYLE_1) {
      if (mt->framedraw) lives_widget_set_bg_color(mt->fd_frame, LIVES_WIDGET_STATE_NORMAL, &palette->normal_back);
    }
  }
...
void mt_post_playback(lives_mt * mt) {
  ...
  if (mainw->cancelled != CANCEL_USER_PAUSED && !((mainw->cancelled == CANCEL_NONE ||
    ...
    mt_paint_lines(mt, curtime, TRUE, NULL);
    mt->no_frame_update = FALSE;
    mt_show_current_frame(mt, FALSE);
  }
...
#define AMATCH_DELTA 30. // seconds to search around the estimate (backwards time; fwds time is 1 / 3 this)
  ...
  if (block && (mt->opts.grav_mode == GRAV_MODE_LEFT || (block->next
                && mt->opts.grav_mode == GRAV_MODE_RIGHT)) &&
    ...
    if (mt->opts.grav_mode == GRAV_MODE_LEFT) {
      if (block->prev) mt->region_start = get_event_timecode(block->prev->end_event) / TICKS_PER_SECOND_DBL;
    ...
    mt->region_end = oldr_end;
    mt_sensitise(mt);
    if (selblock) mt->block_selected = selblock;
  }
  ...
  mt->new_block = block;
  if (block && !resize_timeline(mt) && !did_backup) {
    lives_painter_surface_t *bgimage = (lives_painter_surface_t *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox),
                                       "bgimg");
    if (bgimage) {
      mt_draw_block(mt, NULL, bgimage, block, 0, lives_widget_get_allocation_width(eventbox));
      lives_widget_queue_draw(eventbox);
    }
  }
  mt->new_block = NULL;
...
boolean multitrack_audio_insert(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (mt->opts.insert_mode == INSERT_MODE_NORMAL) {
    ...
    while (tc <= tclen) {
      ...
      if (!event) break; // must be end of timeline
      // is audio track, see if we are in an audio block
      if ((tc == 0 && get_audio_block_start(mt->event_list, mt->current_track, tcnow, TRUE)) ||
          (get_audio_block_start(mt->event_list, mt->current_track, tcnow, FALSE))) {
        if (!did_backup) {
          if (needs_idlefunc || (!did_backup && mt->auto_changed)) mt->idlefunc = mt_idle_add(mt);
        }
        return FALSE;
  ...
  istart = mt_time_to_string(QUANT_TIME(secs));
  iend = mt_time_to_string(QUANT_TIME(secs + (ins_end - ins_start) / TICKS_PER_SECOND_DBL));
...
void insert_frames(int filenum, weed_timecode_t offset_start, weed_timecode_t offset_end, weed_timecode_t tc,
  ...
  if (!in_block) {
    ...
    istart = mt_time_to_string(QUANT_TICKS((orig_st + start_tc)));
    iend = mt_time_to_string(QUANT_TICKS((orig_end + start_tc)));
...
static EXPOSE_FN_DECL(expose_timeline_reg_event, timeline, user_data) {
  ...
  if (event) cairo = lives_painter_create_from_surface(mt->tl_surf);
...
boolean on_timeline_update(LiVESWidget * widget, LiVESXEventMotion * event, livespointer user_data) {
  ...
  if (mt->opts.mouse_mode == MOUSE_MODE_SELECT && mt->tl_selecting && event) mouse_select_move(widget, event, mt);
...
void get_region_overlap(lives_mt * mt) {
  ...
  while (event && !all_present(event, mt->selected_tracks)) {
    event = get_next_frame_event(event);
  }

  if (!event) mt->region_start = 0.;
...
void do_sel_context(lives_mt * mt) {
  ...
  if (!mt->selected_tracks) {
    ...
    int llen = lives_list_length(mt->selected_tracks);
    msg = lives_strdup_printf(P_("%d video track selected.\n", "%d video tracks selected.\n", llen),
                              llen);
  }
  add_context_label(mt, msg);
...
boolean on_timeline_release(LiVESWidget * eventbox, LiVESXEventButton * event, livespointer user_data) {
  ...
  if (event) mt->region_updating = FALSE;
  ...
  if ((mt->region_end != mt->region_start) && eventbox == mt->timeline_reg) {
    ...
    lives_widget_set_sensitive(mt->remove_gaps, FALSE);
    lives_widget_set_sensitive(mt->remove_first_gaps, FALSE);
    if (mt->init_event && mt->poly_state == POLY_PARAMS)
      lives_spin_button_set_value(LIVES_SPIN_BUTTON(mt->node_spinbutton),
  ...
  if (mt->opts.mouse_mode == MOUSE_MODE_SELECT && event) mouse_select_end(eventbox, event, mt);
...
void on_prev_fm_clicked(LiVESWidget * button, livespointer user_data) {
  ...
  if (event) tc = get_event_timecode(event);
...
static weed_timecode_t get_prev_node_tc(lives_mt * mt, weed_timecode_t tc) {
  ...
  for (int i = 0; i < num_params; i++) {
    ...
    while (event && (ev_tc = get_event_timecode(event)) < tc) {
      ...
      event = (weed_plant_t *)weed_get_voidptr_value(event, WEED_LEAF_NEXT_CHANGE, NULL);
    }
  }
  return prev_tc;
}
...
static boolean is_node_tc(lives_mt * mt, weed_timecode_t tc) {
  ...
  for (int i = 0; i < num_params; i++) {
    ...
    ev_tc = -1;
    while (event &&
           (ev_tc = get_event_timecode(event)) < tc)
      event = (weed_plant_t *)weed_get_voidptr_value(event, WEED_LEAF_NEXT_CHANGE, NULL);
...
void on_node_spin_value_changed(LiVESSpinButton * spinbutton, livespointer user_data) {
  ...
  if (mt->current_track >= 0) {
    if (mt->opts.fx_auto_preview || mainw->play_window) mt_show_current_frame(mt, FALSE);
  }
...
void on_del_node_clicked(LiVESWidget * button, livespointer user_data) {
  ...
  for (i = 0; i < num_params; i++) {
    ...
    if (ev_tc == tc) {
      ...
      if (event != pchain[i]) {
        ...
        if (prev_pchange) weed_set_voidptr_value(prev_pchange, WEED_LEAF_NEXT_CHANGE, next_pchange);
        if (next_pchange) weed_set_voidptr_value(next_pchange, WEED_LEAF_PREV_CHANGE, prev_pchange);
      } else {
  ...
  if (mt->current_fx == mt->avol_fx && mt->avol_init_event && mt->opts.aparam_view_list) {
...
static void add_to_pchain(weed_plant_t *event_list, weed_plant_t *init_event, weed_plant_t *pchange, int index,
  ...
  if (event && get_event_timecode(event) == tc) {
    ...
    if (event) weed_set_voidptr_value(event, WEED_LEAF_PREV_CHANGE, pchange);
  }

  if (last_event) weed_set_voidptr_value(last_event, WEED_LEAF_NEXT_CHANGE, pchange);
  else {
...
void on_set_pvals_clicked(LiVESWidget * button, livespointer user_data) {
  ...
  if (mt->current_rfx && mainw->textwidget_focus) {
  ...
  filter_name = weed_filter_idx_get_name(mt->current_fx, FALSE, FALSE);
  tracks = weed_get_int_array(mt->init_event, WEED_LEAF_IN_TRACKS, NULL);
  numtracks = enabled_in_channels(get_weed_filter(mt->current_fx), TRUE); // count repeated channels
  ...
  case 2:
    tname = lives_fx_cat_to_text(LIVES_FX_CAT_TRANSITION, FALSE); // transition
    track_desc = lives_strdup_printf(_("tracks %s and %s"), (tmp = get_track_name(mt, tracks[0], FALSE)),
                                     (tmp2 = get_track_name(mt, tracks[1], FALSE)));
    lives_free(tmp);
    lives_free(tmp2);
    break;
  default:
    ...
    if (fd < 0) {
      ...
      for (int i = 1; i <= MAX_FILES; i++) {
        ...
        if (mainw->files[i]) {
          ...
          if (sfile->layout_map) {
            ...
            while (map) {
              ...
              if (map->data) {
                ...
                if ((file && !strcmp(array[0], file)) || (!file && !dir &&
          ...
          if (file && ((lmap && lmap[i] != 0) || (lmap_audio && lmap_audio[i] != 0.))) {
            if (lmap) max_frame = lmap[i];
            else max_frame = 0;
...
void add_markers(lives_mt * mt, weed_plant_t *event_list, boolean add_block_ids) {
  ...
  while (event) {
    if (WEED_EVENT_IS_FRAME(event)) {
      ...
      while (blist) {
        ...
        if (block) {
          if (block->prev && (get_event_timecode(block->prev->end_event) ==
                              q_gint64(tc - TICKS_PER_SECOND_DBL / mt->fps, mt->fps))
              && (tc == get_event_timecode(block->start_event)) &&
            ...
            if (mt->audio_draws && lives_list_length(mt->audio_draws) >= track + mt->opts.back_audio_tracks) {
              // insert in audio too
  ...
  if (track_blocks) lives_list_free(track_blocks);
}
...
boolean on_save_event_list_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (!(*layout_name)) esave_file = choose_file(esave_dir, NULL, filt, LIVES_FILE_CHOOSER_ACTION_SAVE, NULL, hbox);
  ...
  if (!esave_file || !check_storage_space(-1, FALSE)) {
    char *cdir;
...
void *find_init_event_in_ttable(ttable * trans_table, uint64_t in, boolean normal) {
  for (int i = 0; i < FX_KEYS_MAX - FX_KEYS_MAX_VIRTUAL; i++) {
    ...
    if (!trans_table[i].out) return NULL;
  }
  return NULL;
}
...
static void **remove_nulls_from_filter_map(void **init_events, int *num_events) {
  ...
  for (i = 0; i < *num_events; i++) if (!init_events[i]) num_nulls++;
  ...
  *num_events -= num_nulls;
  ...
  for (i = 0; i < *num_events + num_nulls; i++) if (init_events[i]) new_init_events[j++] = init_events[i];
...
void move_init_in_filter_map(lives_mt * mt, weed_plant_t *event_list, weed_plant_t *event, weed_plant_t *ifrom,
  ...
  int i, j;
  weed_plant_t *deinit_event = weed_get_plantptr_value(ifrom, WEED_LEAF_DEINIT_EVENT, NULL);
  ...
  while (event != deinit_event) {
    ...
    for (i = 0; i < num_inits; i++) {
      ...
      if (got_after && init_event_in_list(events_before, num_before, (weed_plant_t *)init_events[i])) {
        lives_free(init_events);
        if (events_before) lives_free(events_before);
        if (events_after) lives_free(events_after);
...
boolean compare_filter_maps(weed_plant_t *fm1, weed_plant_t *fm2, int ctrack) {
  ...
  int i1, i2, num_events1, num_events2;
  void **inits1, **inits2;
  ...
  if (ctrack == LIVES_TRACK_ANY && ((!weed_plant_has_leaf(fm1, WEED_LEAF_INIT_EVENTS) &&
                                     weed_get_voidptr_value(fm2, WEED_LEAF_INIT_EVENTS, NULL)) ||
                                    (!weed_plant_has_leaf(fm2, WEED_LEAF_INIT_EVENTS) &&
                                     weed_get_voidptr_value(fm1, WEED_LEAF_INIT_EVENTS, NULL)))) return FALSE;

  if (ctrack == LIVES_TRACK_ANY && (weed_plant_has_leaf(fm1, WEED_LEAF_INIT_EVENTS)) &&
      weed_plant_has_leaf(fm2, WEED_LEAF_INIT_EVENTS) &&
      ((!weed_get_voidptr_value(fm1, WEED_LEAF_INIT_EVENTS, NULL) &&
        weed_get_voidptr_value(fm2, WEED_LEAF_INIT_EVENTS, NULL)) ||
       (weed_get_voidptr_value(fm1, WEED_LEAF_INIT_EVENTS, NULL) &&
        !weed_get_voidptr_value(fm2, WEED_LEAF_INIT_EVENTS, NULL)))) return FALSE;
  ...
  inits1 = weed_get_voidptr_array(fm1, WEED_LEAF_INIT_EVENTS, NULL);
  inits2 = weed_get_voidptr_array(fm2, WEED_LEAF_INIT_EVENTS, NULL);

  if (!inits1 && !inits2) return TRUE;
  ...
  for (i1 = 0; i1 < num_events1; i1++) {
    ...
    if (i2 < num_events2) {
      if (ctrack != LIVES_TRACK_ANY) {
        if (!inits2[i2] || !init_event_is_relevant((weed_plant_t *)inits2[i2], ctrack)) {
          i2++;
          i1--;
  ...
  if (inits1) lives_free(inits1);
  if (inits2) lives_free(inits2);
  return TRUE;
}
...
static char *filter_map_check(ttable * trans_table, weed_plant_t *filter_map, weed_timecode_t deinit_tc,
  ...
  if (weed_leaf_seed_type(filter_map, WEED_LEAF_INIT_EVENTS) == WEED_SEED_INT64) {
    if (num_init_events == 1 && weed_get_int64_value(filter_map, WEED_LEAF_INIT_EVENTS, NULL) == 0) return ebuf;
    init_events = (uint64_t *)(weed_get_int64_array(filter_map, WEED_LEAF_INIT_EVENTS, NULL));
  } else {
    if (num_init_events == 1 && !weed_get_voidptr_value(filter_map, WEED_LEAF_INIT_EVENTS, NULL)) return ebuf;
    pinit_events = weed_get_voidptr_array(filter_map, WEED_LEAF_INIT_EVENTS, NULL);
  ...
  for (i = 0; i < num_init_events; i++) {
    if (find_init_event_in_ttable(trans_table, init_events[i], FALSE)) copy_events[i] = (void *)init_events[i]; // !!
    else {
      copy_events[i] = NULL;
  ...
  if (copy_events) lives_free(copy_events);
  lives_free(init_events);
  return ebuf;
}
...
static char *add_filter_deinits(weed_plant_t *event_list, ttable * trans_table, void ***pchains,
  ...
  for (i = 0; i < FX_KEYS_MAX - FX_KEYS_MAX_VIRTUAL; i++) {
    ...
    if (trans_table[i].in != 0) {
      ...
      filter_hash = weed_get_string_value(init_event, WEED_LEAF_FILTER, NULL);
...
static char *add_null_filter_map(weed_plant_t *event_list, weed_plant_t *last_fm, weed_timecode_t tc, char *ebuf) {
  ...
  if (num_events == 1 && !weed_get_voidptr_value(last_fm, WEED_LEAF_INIT_EVENTS, NULL)) return ebuf;
...
#define S_MISSING		(1 << 0)
#define S_RECOVERABLE		(1 << 1)
...
boolean event_list_rectify(lives_mt * mt, weed_plant_t *event_list) {
  ...
  lives_memset(status, 0, MAX_FILES * sizint);
  lives_memset(mhandles, 0, MAX_FILES * sizeof(char *));

  if (weed_plant_has_leaf(event_list, WEED_LEAF_FPS)) fps = weed_get_double_value(event_list, WEED_LEAF_FPS, NULL);
  ...
  while (event) {
            ...
            if (weed_plant_has_leaf(filter, WEED_LEAF_IN_CHANNEL_TEMPLATES)) {
              if (!weed_plant_has_leaf(event, WEED_LEAF_IN_COUNT)) {
                ...
                if (num_ctmpls != num_inct) {
                  ...
                  inct = weed_get_int_array(event, WEED_LEAF_IN_COUNT, NULL);
                  ...
                  for (i = 0; i < num_ctmpls; i++) {
                    ...
                    if (thisct == 0 && !weed_chantmpl_is_optional(ctmpls[i])) {
                      ...
                      if (thisct > 1 && (!weed_plant_has_leaf(ctmpls[i], WEED_LEAF_MAX_REPEATS) ||
                                         (weed_get_int_value(ctmpls[i], WEED_LEAF_MAX_REPEATS, NULL) > 0 &&
                                          weed_get_int_value(ctmpls[i], WEED_LEAF_MAX_REPEATS, NULL) < thisct))) {
                  ...
                  if (!was_deleted) {
                    ...
                    if (num_ctmpls != num_outct) {
                      ...
                      outct = weed_get_int_array(event, WEED_LEAF_OUT_COUNT, NULL);
                            ...
                            if (weed_plant_has_leaf(event, WEED_LEAF_IN_TRACKS)) {
                              ...
                              for (i = 0; i < ntracks; i++) {
                                if (!has_video_chans_in(filter, FALSE)) {
                                  ...
                                  if (trax[i] < 0) {
                                    // TODO ** inform user
                                    if (mt && mt->opts.back_audio_tracks < -trax[i]) {
                                      mt->opts.back_audio_tracks = -trax[i];
                                      ebuf = rec_error_add(ebuf, "Adding backing audio track(s)", -1, tc);
                                    } else {
                                      if (-trax[i] > force_backing_tracks)
                                        force_backing_tracks = -trax[i];
				      // *INDENT-OFF*
				    }}}}
			      // *INDENT-ON*
    ...
    case WEED_EVENT_TYPE_FILTER_DEINIT:
      ...
      if (!weed_plant_has_leaf(event, WEED_LEAF_INIT_EVENT)) {
        ...
        if (!init_event) {
          ...
          host_tag_s = weed_get_string_value((weed_plant_t *)init_event, WEED_LEAF_HOST_TAG, NULL);
          ...
          filter_hash = weed_get_string_value((weed_plant_t *)init_event, WEED_LEAF_FILTER, NULL);
          if ((filter_idx = weed_get_idx_for_hashname(filter_hash, TRUE)) != -1) {
    ...
    case WEED_EVENT_TYPE_FILTER_MAP:
      ...
      if (weed_plant_has_leaf(event, WEED_LEAF_INIT_EVENTS)) {
        ...
        if (weed_leaf_seed_type(event, WEED_LEAF_INIT_EVENTS) == WEED_SEED_INT64)
          init_events = (uint64_t *)weed_get_int64_array(event, WEED_LEAF_INIT_EVENTS, NULL);
        else  {
          void **pinit_events = weed_get_voidptr_array(event, WEED_LEAF_INIT_EVENTS, NULL);
          init_events = (uint64_t *)lives_malloc(num_init_events * sizeof(uint64_t));
          for (i = 0; i < num_init_events; i++) init_events[i] = (uint64_t)pinit_events[i];
          lives_free(pinit_events);
        }
      ...
      if (last_filter_map) {
        if (compare_filter_maps(last_filter_map, event, LIVES_TRACK_ANY)) {
          ...
          was_deleted = TRUE;
                 ...
                 !weed_get_voidptr_value(event, WEED_LEAF_INIT_EVENTS, NULL)) {
        delete_event(event_list, event);
        was_deleted = TRUE;
      }
        ...
        if (!weed_plant_has_leaf(event, WEED_LEAF_VALUE)) {
          ...
          if (!weed_plant_has_leaf(event, WEED_LEAF_INIT_EVENT)) {
            ...
            if ((init_event = find_init_event_in_ttable(trans_table, event_id, TRUE)) == NULL) {
              ...
              if ((filter_idx = weed_get_idx_for_hashname(filter_hash, TRUE)) != -1) {
                filter = get_weed_filter(filter_idx);
                pnum = weed_get_int_value(event, WEED_LEAF_INDEX, NULL);
                    ...
                    if (weed_leaf_seed_type(event, WEED_LEAF_VALUE) != weed_leaf_seed_type(ptmpls[pnum], WEED_LEAF_DEFAULT)) {
                      ...
                      if (pchains[idx][pnum] == init_event) {
                        if (weed_leaf_seed_type((weed_plant_t *)init_event, WEED_LEAF_IN_PARAMETERS) == WEED_SEED_INT64) {
                          ...
                          for (i = 0; i < num_params; i++) {
                            if (!orig_pchanges[i] && i == pnum) pin_pchanges[i] = (void *)event;
      ...
      if (tc == last_frame_tc) {
        ...
        for (i = 0; i < num_tracks; i++) {
          if (clip_index[i] > 0 && (clip_index[i] > MAX_FILES || renumbered_clips[clip_index[i]] < 1 ||
                                    !mainw->files[renumbered_clips[clip_index[i]]])) {
            // clip has probably been closed, so we remove its frames

            new_clip_index[i] = -1;
            new_frame_index[i] = 0;

            if (!(status[clip_index[i]] & S_MISSING)) {
              status[clip_index[i]] |= S_MISSING;
              ...
              missing_clips = TRUE;
#if 0 // TODO
              // check if it may possibly be recoverable
              if (mainw->bad_lmaps) {
                for (LiVESList *list = mainw->bad_lmaps; list; list = list->next) {
                  layout_map *lmap_entry = (layout_map *)list->data;
                  LiVESList *lmap_entry_list = lmap_entry->list;
                  // check handle linked to missing clips: if the directory exists
                  // we can suggest recovery
                  char **array = lives_strsplit((char *)lmap_entry_list->data, "|", -1);
                  if (!lives_strcmp(array[0], lmap_name)) {
                    char *hdrfile;
        ...
        if (WEED_EVENT_IS_AUDIO_FRAME(event)) {
          ...
          if ((num_atracks & 1) != 0) {
            ...
            if (!weed_plant_has_leaf(event, WEED_LEAF_AUDIO_SEEKS) || weed_leaf_num_elements(event,
              ...
              aclip_index = weed_get_int_array(event, WEED_LEAF_AUDIO_CLIPS, NULL);
              aseek_index = weed_get_double_array(event, WEED_LEAF_AUDIO_SEEKS, NULL);
              ...
              for (i = 0; i < num_atracks; i += 2) {
                if (aclip_index[i + 1] > 0) {
                  if ((aclip_index[i + 1] > MAX_FILES || renumbered_clips[aclip_index[i + 1]] < 1 ||
                       !mainw->files[renumbered_clips[aclip_index[i + 1]]]) && aseek_index[i + 1] != 0.) {
                    // clip has probably been closed, so we remove its frames
                    ebuf = rec_error_add(ebuf, "Invalid audio clip number", aclip_index[i + 1], tc);
#ifdef DEBUG_MISSING_CLIPS
                    g_print("found invalid audio clip number %d on track %d, renumbered_clips=%d\n", aclip_index[i + 1], i,
                            renumbered_clips[aclip_index[i + 1]]);
#endif
                    missing_clips = TRUE;
                  } else {
                ...
                if (aclip_index[i] < 0) {
                  // TODO ** inform user
                  if (mt && mt->opts.back_audio_tracks < -aclip_index[i]) {
                    mt->opts.back_audio_tracks = -aclip_index[i];
                    ebuf = rec_error_add(ebuf, "Adding backing audio track(s)", -1, tc);
                  } else {
                    if (-aclip_index[i] > force_backing_tracks)
                      force_backing_tracks = -aclip_index[i];
                  }
                }
              }
              if (j == 0) {
    ...
    if (!was_deleted && check_filter_map && last_filter_map
        && (!event_next || get_event_timecode(event_next) > last_deinit_tc)) {
  ...
  if (last_filter_map) ebuf = add_null_filter_map(event_list, last_filter_map, last_tc, ebuf);
  ...
  while (event) {
    ...
    case WEED_EVENT_TYPE_FILTER_INIT:
      ...
      if (weed_plant_has_leaf(event, WEED_LEAF_IN_PARAMETERS)) {
        ...
        if (weed_leaf_seed_type(event, WEED_LEAF_IN_PARAMETERS) == WEED_SEED_INT64) {
          pin_params = (uint64_t *)weed_get_int64_array(event, WEED_LEAF_IN_PARAMETERS, NULL);
      ...
      if (mt && event != mt->avol_init_event) {
    ...
    case WEED_EVENT_TYPE_PARAM_CHANGE:
      ...
      break;
    case WEED_EVENT_TYPE_FILTER_DEINIT:
      if (mt && weed_get_voidptr_value(event, WEED_LEAF_INIT_EVENT, NULL) != mt->avol_init_event) {
    ...
    if (was_moved) {
      if (last_frame_event) event = last_frame_event;
...
#if 0
    for (int clno = 0; clno < MAX_FILES;; clno++) {
      if (status[clno] & S_RECOVERABLE) g_print("File ref %d may be at %s\n", clno, "???");
    }
#endif
...
char *get_eload_filename(lives_mt * mt, boolean allow_auto_reload) {
  ...
  if (allow_auto_reload) {
    ar_checkbutton = make_autoreload_check(LIVES_HBOX(hbox), prefs->ar_layout);
    lives_signal_sync_connect(LIVES_GUI_OBJECT(ar_checkbutton), LIVES_WIDGET_TOGGLED_SIGNAL,
                              LIVES_GUI_CALLBACK(toggle_sets_pref), (livespointer)PREF_AR_LAYOUT);
  }
...
weed_plant_t *load_event_list(lives_mt * mt, char *eload_file) {
  ...
  do {
    ...
    if ((event_list = load_event_list_inner(mt, fd, mt != NULL, &num_events, NULL, NULL)) == NULL) {
      ...
      if (THREADVAR(read_failed) == fd + 1) {
        ...
        if (mt) retval = do_read_failed_error_s_with_retry(eload_name, NULL);
      }

      if (mt && retval != LIVES_RESPONSE_RETRY) {
  ...
  if (mt->avol_fx != old_avol_fx && mt->opts.aparam_view_list) {
...
void on_clear_event_list_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (resp == 2 && !rev_resp) {
    ...
    char *layout_file, *lmap_file, *tmp;
    if (!do_yesno_dialogf_with_countdown(2, TRUE, "\nLayout %s will be deleted permanently from the disk.\n"
                                         "Are you sure ?\n", mt->layout_name)) {
      mt->idlefunc = mt_idle_add(mt);
      return;
    }
...
boolean on_load_event_list_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  // prevent audio tracks being added twice to avol_init_event
  avol_init_event = mt->avol_init_event;
  mt->avol_init_event = NULL;
  mt_init_tracks(mt, TRUE);
  mt->avol_init_event = avol_init_event;
...
void mt_change_vals_activate(LiVESMenuItem * menuitem, livespointer user_data) {
  ...
  if (resaudw) {
    ...
    xse = mainw->files[mt->render_file]->signed_endian;
  }
  ...
  if (xachans == 0 && mt->audio_draws) {
    LiVESList *slist = mt->audio_draws;
    while (slist) {
      if (lives_widget_object_get_data(LIVES_WIDGET_OBJECT(slist->data), "blocks")) {
  ...
  mt->auto_changed = TRUE;
  if (prefs->mt_auto_back >= 0) save_mt_autoback(mt);
  mt->changed = TRUE;
}
...
static track_rect *get_nth_block_for_track(lives_mt * mt, int itrack, int iblock) {
  ...
  if (!eventbox) return NULL; //<invalid track
  block = (track_rect *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "blocks");
  while (block) {
...
boolean mt_track_is_video(lives_mt * mt, int ntrack) {
  if (ntrack >= 0 && mt->video_draws && ntrack < lives_list_length(mt->video_draws)) return TRUE;
  return FALSE;
}


LIVES_GLOBAL_INLINE boolean mt_track_is_audio(lives_mt * mt, int ntrack) {
  if (ntrack <= 0 && mt->audio_draws && ntrack >= -(lives_list_length(mt->audio_draws))) return TRUE;
  return FALSE;
}
...
int mt_get_block_count(lives_mt * mt, int ntrack) {
  ...
  if (!eventbox) return -1; //<invalid track
  lastblock = (track_rect *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "block_last");
  if (!lastblock) return -1; ///< no blocks in track
  block = (track_rect *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(eventbox), "blocks");
  while (block) {
...
double mt_get_block_sttime(lives_mt * mt, int ntrack, int iblock) {
  ...
  if (!block) return -1; ///< invalid track or block number
...
void mt_do_autotransition(lives_mt * mt, track_rect * block) {
  ...
  track = get_track_for_block(mt, block);
  ...
  // crossfade audio
  if (mt->init_event && mt->opts.autocross_audio)
