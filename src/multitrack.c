...
static void notebook_error(LiVESNotebook *nb, uint32_t tab, lives_mt_nb_error_t err, lives_mt *mt) {
  ...
  lives_container_set_border_width(LIVES_CONTAINER(lives_notebook_get_nth_page(LIVES_NOTEBOOK(nb), page)), widget_opts.border_width);
...
lives_mt *multitrack(weed_plant_t *event_list, int orig_file, double fps) {
  ...
  if (mainw->multi_opts.set) {
    ...
    mt->opts.overlay_timecode = FALSE;
  }
...
# endif
  ...
  lives_container_add(LIVES_CONTAINER(mt->files_menu), separator);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->edit_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  */
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->play_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->effects_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  mt->ac_audio_check = lives_standard_check_menu_item_new_with_label(_("Crossfade Audio with Autotransition"), mt->opts.autocross_audio);
  ...
  mt->fx_region_1v = lives_standard_menu_item_new_with_label(cname);
  ...
  mt->fx_region_1a = lives_standard_menu_item_new_with_label(cname);
  ...
  submenu_menuc = lives_menu_new();
  lives_menu_item_set_submenu(LIVES_MENU_ITEM(mt->fx_region_3), submenu_menuc);
  ...
  for (i = 0; i < num_filters; i++) {
    ...
    if (filter != NULL && !weed_plant_has_leaf(filter, WEED_LEAF_HOST_MENU_HIDE)) {
	  ...
	  if (pkg != NULL) {
	    if (xpkg == NULL || strcmp(pkg, xpkg)) {
	      // create new submenu for pkg
	      tname = lives_fx_cat_to_text(LIVES_FX_CAT_VIDEO_EFFECT, TRUE); // video effects
	      cname = lives_strdup_printf("_%s from package %s...", tname, pkg);
	      lives_free(tname);
	    ...
	    submenu2 = submenu_menu4vp;
	    xpkg = pkg;
	  } else {
	    submnenu = submenu_menuv;
	    submnenu2 = submenu4v;
	  }
  ...
  /// None autotransition
  menuitem2 = lives_standard_check_menu_item_new_with_label(mainw->string_constants[LIVES_STRING_CONSTANT_NONE], prefs->atrans_fx == -1);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->tracks_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->selection_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->tools_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->view_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  separator = lives_standard_menu_item_new();
  lives_container_add(LIVES_CONTAINER(mt->help_menu), separator);
  lives_widget_set_sensitive(separator, FALSE);
  ...
  */
  ...
  mt->mm_select = lives_standard_check_menu_item_new_with_label(_("Mouse Mode: _Select"), mt->opts.mouse_mode == MOUSE_MODE_SELECT);
  ...
  mt->ins_normal = lives_standard_check_menu_item_new_with_label(_("Insert Mode: _Normal"), mt->opts.insert_mode == INSERT_MODE_NORMAL);
...
LiVESWidget *add_audio_track(lives_mt *mt, int track, boolean behind) {
  ...
  lives_widget_object_set_data_auto(LIVES_WIDGET_OBJECT(audio_draw), "track_name", lives_strdup_printf(_("Layer %d audio"), track));
  ...
  if (!behind) {
    ...
    for (i = 0; i < mt->num_video_tracks - 1; i++) {
      ...
      if (mt->opts.pertrack_audio) {
        ...
        lives_widget_object_set_data_auto(LIVES_WIDGET_OBJECT(aeventbox), "track_name", lives_strdup_printf(_("Layer %d audio"), i + 1));
...
static void update_in_image(lives_mt *mt) {
  ...
  calc_maxspect(lives_widget_get_allocation_width(mt->in_frame), lives_widget_get_allocation_height(mt->in_frame), &width, &height);
...
static void update_out_image(lives_mt *mt, weed_timecode_t end_tc) {
  ...
  calc_maxspect(lives_widget_get_allocation_width(mt->out_frame), lives_widget_get_allocation_height(mt->out_frame), &width, &height);
...
static void mt_jumpto_mark(lives_mt *mt, lives_direction_t dir) {
  ...
  if (dir == DIRECTION_POSITIVE) marktime = time;
  if (marktime > 0.)
...
void migrate_layouts(const char *old_set_name, const char *new_set_name) {
 ...
 LiVESList *layout_audio_is_affected(int clipno, double stime, double etime) {
  ...
  while (lmap != NULL) {
    ...
    if (max_time > 0. && time <= max_time && (etime == 0. || etime <= mainw->files[clipno]->stored_layout_audio)) {
      mainw->xlays = lives_list_append_unique(mainw->xlays, array[0]);
    }
    lives_strfreev(array);
    lmap = lmap->next;
