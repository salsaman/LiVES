...
#define NIRVA_AUTO_CMD_8 "REMOVE_FROM_ARRAY" // strand, vakue
...
// value is set when the bundle is created, then may not be changed
// e.g the "UID" strand, the bundle_type strands
#define BLUEPRINT_FLAG_CONST 			(1ull << 4)

#define BLUEPRINT_FLAG_COMMENT 			(1ull << 5)

#define BLUEPRINT_FLAG_DIRECTIVE 		(1ull << 6)

#define BLUEPRINT_FLAG_HOOK_PENDING 		(1ull << 7)

// item naming convntions
// for STANDARD STRANDS, the default name is STRAND_DOMAIN_ITEM, e.g STRAND_GNERIC_FLAGS

// for NANED STRANDS, the ITEM name be be overriden, so we could change this to e.g
// STRAND_GENERIC_BITMAP

// for included bundles (sub-bundles), the defautt is still STRAND_DOMAIN_ITEM
...
// finally when ADDing, ie pointing to an a remote bundle ot arrary, one always defines a custom
// domain / item. In this case it would be somthing like EXTERN_TARGET_FLAGS, for DOMAIN TARGET,
// ITEM FLAGS
...
#define _DEF_BUNDLE						\
    ADD_NAMED_STRAND(VALUE, UINT64, BUNDLE_TYPE),		\
...
// bootstrap: - manually create blueprint for strand_desc
// - the string array based version created here will do
//
// create strand_descs from this and create blueprint for blueprint bundle
// - This becomes the template for creating all other blueprints
// the blueprint_ptr strand can be pointed at self

// use the bprint template to create a "soft" version of strand_desc blueprint
// brpint and strand_desc can now be used to construct blueprints for all remaining
// bundle types
//
#define _BLUEPRINT_BUNDLE EXTEND_BUNDLE(DEF), INCLUDE_BUNDLES(BLUEPRINT, STRAND_DESC)

// DEFAULT is for data strands, TEXT is for comments and directives
// the data_type of DEFAULT must match TYPE. If default is not present it
// is assumed to be 0, NULL, 0., etc.
// BUNDLE_TYPE is a uint64_t used to restrict the type of bundle for the far_pointer
// and bundlepointer types. If not present then pointers may be to any bundle type.

//
// For bundleptr types it will be necessary to add somehting like the following
/// in text.
// ADD_HOOK_AUTOMATION(!SELF, UPDATING_VALUE, COND_RETURN, NIRVA_EQUALS,
//_COND_STRAND_VALUE NEW_VALUE.BUNDLE_TYPE, _COND_STRAND_VAL BUNDLE_TYPE)
//
// for CONST_POINTERS:
// ADD_HOOK_AUTOMATION(!TOP, DESTRUCTION, COND_ONCE, SET_STRAND_VALUE
// !SELF, NIRVA_NULL)

/* A bundle that is used to hold simple values.
   Initially (unless directed otherwise), VALUE.DATA is not created, and VALUE_TYPE is
   set to 0. Once data i set, then VALUE.TYPE is locked to that type and cannot be changed.
*/
#define _VALUE_BUNDLE ADD_STRAND(VALUE, TYPE), ADD_OPT_STRAND(VALUE, DATA)

// a bundle describing a STRAND in a bundle. This is used for the blueprint bundle
// strands themselves can either be created as STRAND_BUNDLES or else directly
// as opaque name/type/value tuples in the implementation layer
#define _STRAND_DESC_BUNDLE EXTEND_BUNDLE(DEF), MAKE_MANDATORY(GENERIC, FLAGS), \
    MAKE_MANDATORY(GENERIC, NAME), ADD_STRAND(VALUE, TYPE), ADD_OPT_STRAND(VALUE, DEFAULT), \
    ADD_OPT_ARRAY(BLUEPRINT, TEXT)

// an optional bundle to use if the implementation does not support name/type/value tuples
#define _STRAND_BUNDLE ADD_STRAND(GENERIC, NAME), EXTEND_BUNDLE(VALUE)

// bundle included in attributes. The type must confer with the ATTRIBUTE_VALUE_TYPE
// (using strand / attr value mapping)
// volatile attrubtes can also map onto native varables
#define _ATTR_VALUE_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(VALUE),	\
    ADD_OPT_STRAND(INTROSPECTION, NATIVE_TYPE),				\
    ADD_OPT_STRAND(INTROSPECTION, NATIVE_SIZE),				\
    ADD_OPT_NAMED_STRAND(INTROSPECTION, NATIVE_PTR, VALUE)
...
#define _PMAP_DESC_BUNDLE EXTEND_BUNDLE(ATTR_DESC), ADD_STRAND(FUNCTION, PARAM_NUM)
...
#define _PMAP_BUNDLE EXTEND_BUNDLE(DEF), ADD_CONST_BUNDLEPTR(PMAP, TEMPLATE, PMAP_DESC), \
...
#define _CONDVAL_NODE_BUNDLE EXTEND_BUNDLE(DEF), INCLUDE_BUNDLE(AUTOMATION, CONDITION), \
    ADD_CONST_BUNDLEPTR(CASCADE, CONSTVAL, CONSTVAL_MAP), ADD_CONST_BUNDLEPTR(NEXT, SUCCESS, CONDVAL_NODE), \
...
// placeholder is a special strand type int with name " "
// this ensures we have at least 1 strand in case the implementation does not support empty
// bundles. If empty bundles can be created then it is unnecessary.
// The directives signify that optional strands with any name may be included
// in this case bundleptrs to bundles of type connection
#define _KEY_LOOKUP_BUNDLE ADD_CONST_BUNDLEPTR(LOOKUP, ARRAY, ANY), ADD_STRAND(SPECIAL, OPT_ANY), \
      ADD_HOOK_AUTOMATION(!CONTAINER, DELETING_STRAND,			\
			  _COND_ALWAYS, REMOVE_FROM_ARRAY, *ARRAY, *OLD_VALUE), \
      ADD_HOOK_AUTOMATION(!CONTAINER, STRAND_ADDED, _COND_ALWAYS, ADD_HOOK_AUTOMATION, \
			  NEW_VALUE, UPDATING_VALUE, _COND_ALWAYS, PROXY_TO, *!SELF)
...
#define _INDEX_BUNDLE INCLUDE_BUNDLE(STANDARD, KEY_LOOKUP),		\
...
#define KEYED_NAME_BUNDLE EXTEND_BUNDLE(INDEX), ADD_KEYED_ARRAY(ANY_BUNDLE, NAME, ENTRIES)

#define KEYED_UID_BUNDLE EXTEND_BUNDLE(INDEX), ADD_KEYED_ARRAY(ANY_BUNDLE, UID, ENTRIES)
...
// ADD_CONST_BUNDLEPTR(PARENT, DESC, ATTR_DESC) ??
#define _ATTR_DESC_BUNDLE EXTEND_BUNDLE(DEF), MAKE_MANDATORY(GENERIC, FLAGS), \
    ADD_STRAND(ATTRIBUTE, VALUE_TYPE), ADD_OPT_STRAND(VALUE, MAX_VALUES), \
...
// volatile, const, optional, mandatory. They usually com in bundles - attr_containers,
// and can be constructed from attr_desc, or attr_deesc container.
// objects have their own attr bundles, and they are also carreid around as functional
// inputs and outputs. Scripts and conditions may also have a referenced attr bundle
#define _ATTRIBUTE_BUNDLE EXTEND_BUNDLE(ATTR_DESC), INCLUDE_BUNDLE(STANDARD, ATTR_VALUE), \
    ADD_CONST_BUNDLEPTR(PARENT, TEMPLATE, ATTR_DESC),			\
    INCLUDE_NAMED_BUNDLE(ATTRIBUTE, CONNECTION, CONNECTION_OUT),	\
    INCLUDE_BUNDLE(INTROSPECTION, REFCOUNT), INCLUDE_BUNDLES(ATTRIBUTE, HOOK_STACK)
...
#define _FN_INPUT_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(EMISSION),	\
    ...
    ADD_NAMED_OPT_STRAND(VALUE, VOIDPTR, USER_DATA), ADD_CONST_BUNDLEPTR(INPUT, ATTR_BUNDLE, ATTR_CON)
...
#define _FN_OUTPUT_BUNDLE EXTEND_BUNDLE(DEF),				\
      ...
      ADD_ARRAY(FUNCTION, RESPONSE), ADD_CONST_BUNDLEPTR(OUTPUT, ATTR_BUNDLE, ATTR_CON), \
...
#define _FUNC_DESC_BUNDLE EXTEND_BUNDLE(DEF),			\
    ADD_NAMED_OPT_STRAND(VALUE, UINT64, EXT_ID),		\
    ADD_STRAND(FUNCTION, CATEGORY),				\
    INCLUDE_NAMED_BUNDLE(STANDARD, ATTR_DESC_CON, INPUT),	\
...
#define _FUNC_MAP_BUNDLE					\
  EXTEND_BUNDLE(DEF),						\
    ...
    INCLUDE_NAMED_BUNDLE(STANDARD, ATTR_DESC_CON, OUTPUTS),	\
    INCLUDE_NAMED_BUNDLES(STANDARD, PMAP_DESC, PMAPPING),	\
...
#define _FUNC_DESC_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX), \
    ADD_KEYED_ARRAY(FUNC_DESC, NAME, ENTRIES)
//
...
#define _ATTR_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX),	\
      ADD_KEYED_ARRAY(ATTR_DESC, NAME, ATTRIBUTES), INCLUDE_BUNDLES(ATTRIBUTE, HOOK_STACK)

#define _ATTR_DESC_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX), \
    ADD_KEYED_ARRAY(ATTR_DESC, NAME, ATTRS)

#define _OBJECT_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX),	\
    ADD_KEYED_ARRAY(OBJECT, UID, OBJECTS)

#define _OFFSPRING_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX), \
    ADD_KEYED_ARRAY(OBJECT, UID, OFFSPRING)

#define _SCRIPTLET_CON_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(INDEX), \
...
#define _OBJECT_TEMPLATE_BUNDLE EXTEND_BUNDLE(DEF),			\
      ...
      INCLUDE_NAMED_BUNDLE(STANDARD, ATTR_CON, ATTRIBUTES),		\
...
#define _COND_GEN_001 COND_CHECK, NIRVA_VAL_LT,_COND_STRAND_VAL REFCOUNT, _COND_INT_VAL 0, COND_END

#define _REFCOUNT_BUNDLE EXTEND_BUNDLE(DEF), ADD_STRAND(INTROSPECTION, REFCOUNT), \
    ADD_NAMED_STRAND(THREADS, MUTEX, COUNTER_MUTEX), ADD_NAMED_STRAND(VALUE, BOOLEAN, SHOULD_FREE), \
    ADD_HOOK_AUTOMATION(REFCOUNT, UPDATING_VALUE, _COND_CHECK, _COND_GEN_001, SET_STRAND_VALUE, \
			SHOULD_FREE, _COND_INT_VAL 1),			\
    ADD_NAMED_STRAND(THREADS, MUTEX, DESTRUCT_MUTEX),			\
    ADD_NAMED_ARRAY(UINT64, ADDERS), INCLUDE_NAMED_BUNDLES(HOOK, STACK, REQUESTS)
...
#define _OBJECT_INSTANCE_BUNDLE ADD_CONST_BUNDLEPTR(OBJECT, TEMPLATE, OBJECT_TEMPLATE), \
    ...
    ADD_HOOK_AUTOMATION(SUBTYPE, VALUE_UPDATED, _COND_ALWAYS, CASCADE_HOOKS), \
...
// hook stacks include STATUS, IDLE,+  REQUEST
#define _THREAD_INSTANCE_BUNDLE EXTEND_BUNDLE(OBJECT_INSTANCE),	    \
    ADD_STRAND(THREADS, NATIVE_THREAD),	ADD_STRAND(THREADS, NATIVE_STATE)
...
#define _OWNED_ATTRS_BUNDLE EXTEND_BUNDLE(DEF), ADD_CONST_BUNDLEPTR(OWNER, OBJECT, OBJECT), \
    ADD_CONST_BUNDLEPTR(LIST, DATA, ATTR_CON)
...
// for semi / full TRANSFOMR automation, the requiremnts are parsed by the automation
//  the funcdesc is used for static functions to enable presentation as native funcs
// this is an array so we can present the transform in various guises
#define _CONTRACT_BUNDLE EXTEND_BUNDLE(OBJECT_INSTANCE), INCLUDE_BUNDLE(TRANSFORM, ICAP), \
    ADD_CONST_BUNDLEPTR(ATTRIBUTE, POOL, ATTR_POOL), INCLUDE_NAMED_BUNDLES(AUTOMATION, SCRIPT, REQUIREMENTS), \
    INCLUDE_BUNDLES(CONTRACT, FUNC_WRAPPER)

//** bundle for an array which checks conditions before allowing an item to be added
#define _COND_ARRAY_BUNDLE ADD_NAMED_ARRAY(VALUE, ANY_BUNDLE, ARRAY),	\
  INCLUDE_BUNDLES(AUTOMATION, CONDITION)
...
#define _TRANSFORM_BUNDLE EXTEND_BUNDLE(OBJECT_INSTANCE),		\
      ADD_CONST_BUNDLEPTR(TRANSFORM, CONTRACT, CONTRACT),			\
      ADD_STRAND(TRANSFORM, STATUS), ADD_STRAND(TRANSFORM, RESULT),	\
      ADD_HOOK_AUTOMATION(STATUS, VALUE_UPDATED, _COND_ALWAYS, CASCADE_HOOKS), \
...
#define NIRVA_ASSERT_NULL(val, func, ...) NIRVA_UNLESS(NIRVA_EQUAL((val),NIRVA_NULL),func(__VA_ARGS__))
...
#define NIRVA_IF_NOT(a, ...) NIRVA_IF(NIRVA_LOGIC_NOT(a)) NIRVA_INLINE(__VA_ARGS__)
...
#define NIRVA_IF_NOT_EQUAL(a,b,...) NIRVA_IF(NIRVA_OP_NOT(NIRVA_EQUAL,a,b)) NIRVA_INLINE(__VA_ARGS__)
...
#define NIRVA_CALL_IF_NOT_EQUAL(a,b,macro, ...) NIRVA_UNLESS_OP(NIRVA_EQUAL,a,b, \
...
// of its blueprint (a strand_desc_bundle defining it)
...
#define _COND_REC_FUNC_001 "NIRVA_LACKS_ATTR,IMPL_FUNC_nirva_strand_get_type,NIRVA_OP_OR," \
  "NIRVA_LACKS_ATTR,IMPL_FUNC_nirva_list_opt_strands,NIRVA_LOGIC_LAST"
...
#define NIRVA_COND_TEST(n, ...) (NIRVA_COND_##n##_TEST(__VA_ARGS__))
...
#define NIRVA_OP_LAST 				0
#define NIRVA_LOGIC_LAST       			0
// the default
#define NIRVA_OP_AND				1
// current val OR next op
#define NIRVA_OP_OR				2
// current val XOR next op
#define NIRVA_OP_XOR				3
// current val == next op
#define NIRVA_OP_EQUALS				4
// the next are like LOGIC(a,b) - the result, TRUE or FALSE is then ANDed with the nxt
// triplet, unless we had a NIRVA_OP first
#define NIRVA_LOGIC_FIRST				16
// note leading _
#define _NIRVA_LOGIC_NOT				16
#define NIRVA_COND_16_TEST(a)			    	_NIRVA_LOGIC_NOT
#define NIRVA_VAL_EQUALS				17
#define NIRVA_COND_17_TEST(a,b)			    	NIRVA_EQUAL(a,b)
#define NIRVA_VAL_GT					18
#define NIRVA_COND_18_TEST(a,b) 	   		((a)>(b))
#define NIRVA_VAL_LT					19
#define NIRVA_COND_19_TEST(a,b)   	 		((a)<(b))
#define NIRVA_VAL_GTE					20
#define NIRVA_COND_20_TEST(a,b)		    		((a)>=(b))
#define NIRVA_VAL_LTE					21
...
#define NIRVA_COND_24_TEST(a,b)			    	(((a)||(b))&&!((a)&&(b)))
...
static  int64_t check_cond(int64_t op, char **strands, int i, NIRVA_BUNDLEPTR attrs) {
  ...
  if (tcode0 == 'A') {
    ...
    if (t0 == ATTR_TYPE_INT) {
      ...
      if (op == _NIRVA_LOGIC_NOT) {
        NIRVA_RETURN(NIRVA_COND_RESPONSE(NIRVA_LOGIC_NOT(p0)));
      }
      tcode1 = *str1;
      if (tcode1 == 'A') {
        uint32_t t1 = 0;
  ...
  if (condres != NIRVA_COND_SUCCESS) {
    ...
    if (gcondres != NIRVA_COND_ERROR) {
      ...
      if (gcondres != NIRVA_COND_WAIT_RETRY) {
        if (op_macro && op_macro != NIRVA_OP_AND && condres == NIRVA_COND_FAIL) {
          if ((op_macro == NIRVA_OP_EQUALS && gcondres == condres)
              || (op_macro == NIRVA_OP_XOR && gcondres == condres))
    ...
    if (op_macro == NIRVA_OP_OR && gcondres == NIRVA_COND_FAIL)
      gcondres = condres;
    else if (op_macro == NIRVA_OP_XOR && gcondres == NIRVA_COND_SUCCESS)
...
NIRVA_IMPL_ASSIGN(attrs, nirva_get_value_bundleptr, b_in, BUNDLE_STANDARD_ATTR_CON);
...
for (int i = 0; strands[i]; i++) {
  int op_macro = NIRVA_OP_AND, logic_macro;
  NIRVA_IMPL_ASSIGN(logic_macro,  atoi, strands[i]);
  if (logic_macro == NIRVA_LOGIC_LAST) break;
...
NIRVA_STATIC int64_t _def_nirva_condcheck(NIRVA_BUNDLEPTR conds, NIRVA_BUNDLEPTR  b_in, NIRVA_BUNDLEPTR b_out) {
  ...
  int64_t condres, gcondres = NIRVA_COND_SUCCESS;
  NIRVA_DEF_VARS(NIRVA_INT,  op_macro = NIRVA_OP_AND)
  do {
...
_INTERNAL_FUNC_make_n_bundles(1, bun, ATTR_CON_BUNDLE_TYPE, va);	\
NIRVA_VA_END(va)							\
NIRVA_FUNC_END
...
#define _NIRVA_DEPLOY_def_nirva_action					\
    ...
    NIRVA_CALL(make_func_bundles, pb_in, pb_out, STRAND_GENERIC_NAME,	\
      ...
      NIRVA_CALL(set_strand_val,*pb_in, BUNDLE_STANDARD_ATTR_CON, STRAND_TYPE_BUNDLEPTR,attrs) \
...
#define INIT_CORE_DATA_BUNDLES BUNLISTx(BDEF_IGN,BDEF_INIT,,) all_strands = REG_ALL;
#define INIT_CORE_ATTR_BUNDLES ABUNLISTx(BDEF_IGN,ATTR_BDEF_INIT,,) all_attr_bundles = REG_ALL_ATTRS;
...
#define INIT_IMPL_FUNCS _INIT_IMFN(OPT,001) _INIT_IMFN(OPT,002) _INIT_IMFN(OPT,003) \
    _INIT_IMFN(OPT,004)
#define _INIT_CORE_BUNDLES INIT_IMPL_FUNCS INIT_CORE_DATA_BUNDLES //INIT_CORE_ATTR_BUNDLES
...
#define _DEFINE_CORE_BUNDLES						\
  ...
  const char **all_strands, BUNLIST(BDEF_DEF_CONCAT,**,_BUNDLEDEF = NULL) \
    **LAST_BUNDLEDEF = NULL;						\
  const char **all_attr_bundles, ABUNLIST(ATTR_BDEF_DEF_CONCAT,**,_ATTR_BUNDLE = NULL) **LAST_ATTR_BUNDLE = NULL; \
  const char ***builtin_bundledefs[] = {BUNLIST(BDEF_DEF_CONCAT, &,_BUNDLEDEF) NULL}; \
  const char ***builtin_attr_bundles[] = {ABUNLIST(ATTR_BDEF_DEF_CONCAT, &,_ATTR_BUNDLE) NULL}; \
  NIRVA_CONST_STRING_ARRAY maker_get_bundledef(bundle_type btype)	\
...
#define ATTR_BDEF_INIT(ATTR_BNAME, PRE, EXTRA) ATTR_BNAME##_ATTR_BUNDLE = MABX(ATTR_BNAME);
