...
#define NIRVA_RESTRICTION_27 RESTRICT_BUNDLE_TYPE(REQUEST)
...
#define _REQUEST_BUNDLE EXTEND_BUNDLE(DEF), EXTEND_BUNDLE(EMISSION),	\
    ADD_OPT_STRAND(HOOK, NUMBER),					\
    ADD_OPT_STRAND(HOOK, HANDLE), ADD_STRAND(GENERIC, FLAGS),		\
    ADD_NAMED_OPT_STRAND(DATETIME, DELTA, REPEAT),			\
    ADD_NAMED_OPT_STRAND(DATETIME, TIMESTAMP, EXEC_TARGET),		\
    ADD_OPT_STRAND(REQUEST, DETAILS)
...
#define _FUNC_DATA_BUNDLE EXTEND_BUNDLE(DEF), ADD_CONST_BUNDLEPTR(LINKED, TRANSFORM, TRANSFORM), \
    ...
    ADD_OPT_CONST_BUNDLEPTR(HOOK, DETAILS, HOOK_DETAILS),		\
    INCLUDE_OPT_BUNDLE(STANDARD, VALUE_CHANGE),				\
...
#define _HOOK_CB_FUNC_BUNDLE EXTEND_BUNDLE(DEF), ADD_STRAND(HOOK, HANDLE), \
      ...
      ADD_STRAND(HOOK, CB_DATA), INCLUDE_OPT_BUNDLE(STANDARD, REQUEST),	\
      INCLUDE_OPT_BUNDLE(STANDARD, SEGMENT)
...
#define _NIRVA_STRAND_GET_TYPE NIRVA_INTERNAL_strand_get_type
  ...
  NIRVA_END_RETURN(ns)
    ...
    NIRVA_DEF_FUNC_INTERNAL(NIRVA_FUNC_RETURN, add_idx_value, NIRVA_BUNDLEPTR idx, NIRVA_CONST_STRING keyval, NIRVA_CONST_STRING prefix,
...
const char **nirva_restrictions[36];						\
