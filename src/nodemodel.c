...
static lives_filter_error_t res_substep(plan_step_t *step) {
  ...
  if (layer) {
    ...
    resized = resize_layer_full(layer, xwidth, xheight, interp, opalette, oclamping, osampling, osubspace, tgt_gamma);
...
static lives_filter_error_t run_apply_inst_step(plan_step_t *step, weed_instance_t *inst) {
  ...
  filter_error = act_on_instance(inst, key, plan->layers,
  ...
  if (filter_error == FILTER_ERROR_NEEDS_REINIT) {
    ...
    if (rte_key_soft_deinited(key))
      filter_error = FILTER_INFO_BYPASSED;
    else {
      filter_mutex_lock(key);
      filter_error = act_on_instance(inst, key, plan->layers,
                                     plan->model->opwidth, plan->model->opheight);
  ...
  if (step->fin_gamma == WEED_GAMMA_LINEAR) {
    // return colour params back to sRGB values
  ...
  do {
    ...
    for (LiVESList *steps = plan->steps; steps; steps = steps->next) {
      ...
      if (state == STEP_STATE_ERROR || state == STEP_STATE_FINISHED
          ...
          || state == STEP_STATE_CANCELLED || (paused && step->state == STEP_STATE_PAUSED))
        continue;

      if (state == STEP_STATE_RESUMING) {
      ...
      if (step->state != STEP_STATE_RUNNING) {
        if (cancelled || paused || error) {
          if (cancelled || error) step->state = STEP_STATE_CANCELLED;
          if (paused) step->state = STEP_STATE_PAUSED;
          continue;
        }

        step->state = STEP_STATE_WAITING;
        //g_print("check step %d state, %d wayr\n", step_count, state);
        ...
        if (step->st_type == STEP_TYPE_LOAD) {
          ...
          for (i = 0; i < step->ndeps; i++) {
            ...
            if (state == STEP_STATE_ERROR || state == STEP_STATE_CANCELLED
              ...
              if (state != STEP_STATE_PAUSED) step->state = STEP_STATE_CANCELLED;
              break;
            }
        ...
        if ((error || cancelled) && step->state != STEP_STATE_RUNNING) {
          step->state = STEP_STATE_CANCELLED;
          continue;
        }
        ...
        case STEP_TYPE_CONVERT: {
          ...
          //double est_dur;
          int op_order[N_OP_TYPES];
          int out_width = step->ini_width, out_height = step->ini_height;
          int in_width = step->fin_width, in_height = step->fin_height;
          int in_iwidth = step->fin_iwidth, in_iheight = step->fin_iheight;
          int outpl = step->ini_pal, inpl = step->fin_pal;
          int out_gamma_type = step->ini_gamma, in_gamma_type = step->fin_gamma;
        ...
        case STEP_TYPE_APPLY_INST: {
          ...
          if (!step->target) {
            ...
            for (int i = 0; i < plan->model->ntracks; i++) {
              if (plan->layers[i]) {
                if (weed_layer_check_valid(plan->layers[i]))
                  lives_layer_set_status(plan->layers[i], LAYER_STATUS_READY);
              }
            }
            step->state = STEP_STATE_FINISHED;
            break;
      ...
      if (state == STEP_STATE_RUNNING) {
        ...
        if (paused) {
          ...
          if (!lpt) {
            layer = plan->layers[step->track];
            if (layer) lpt = lives_layer_get_proc_thread(layer);
          }
          if (lpt) {
            if (!lives_proc_thread_get_pause_requested(lpt))
              lives_proc_thread_request_pause(lpt);
  ...
  do {
    ...
    case STEP_TYPE_LOAD: {
      ...
      if (step->fin_pal)
        step->end_res[RES_TYPE_MEM] =
          lives_frame_calc_bytesize(step->fin_width, step->fin_height, step->fin_pal, FALSE, NULL, NULL);
      ...
      if (!inplace || !step->start_res[RES_TYPE_MEM]) {
        if (!step->start_res[RES_TYPE_MEM])
          step->start_res[RES_TYPE_MEM] =
            lives_frame_calc_bytesize(out->width, out->height, opal, FALSE, NULL, NULL);
    ...
    case STEP_TYPE_COPY_IN_LAYER: {
      if (st_type == STEP_TYPE_COPY_IN_LAYER) {
        ...
        step->dchan = idx;
        step->start_res[RES_TYPE_MEM] +=
          lives_frame_calc_bytesize(orig->width, orig->height, pal, FALSE, NULL, NULL);
    ...
    case STEP_TYPE_APPLY_INST: {
      ...
      step->tdata->est_start = n->ready_ticks;
      step->target_idx = idx;
      step->ini_gamma = step->fin_gamma = n->gamma_type;
      ...
      if (n->model_type == NODE_MODELS_FILTER ||
        ...
        for (int i = 0; i < n->n_outputs; i++) {
          ...
          memused += lives_frame_calc_bytesize(out->width, out->height, pal, FALSE, NULL, NULL);
        }
        ...
        pal = n->optimal_pal;
        if (pal)
          memused = lives_frame_calc_bytesize(in->width, in->height, pal, FALSE, NULL, NULL);
...
void display_plan(exec_plan_t *plan) {
  ...
  for (LiVESList *list = plan->steps; list; list = list->next) {
    ...
    case STEP_STATE_WAITING:
      lives_printerr("Step status is waiting");
      break;
...
static void calc_node_sizes(lives_nodemodel_t *nodemodel, inst_node_t *n) {
  ...
  if (nins > 1 && rmaxw > 0. && rmaxh > 0.) {
    ...
    if (!svary) {
      int zwidth = width, zheight = height;
      for (no = 0; no < nouts; no++) {
        out = n->outputs[no];
        if (out->flags & NODEFLAGS_IO_SKIP) continue;
        if ((out->maxwidth && width > out->maxwidth)
            || (out->maxheight && height > out->maxheight)) {
          zwidth = out->maxwidth;
          zheight = out->maxheight;
          if (!zwidth) zwidth = zheight * op_ar;
          if (!zheight) zheight = zwidth / op_ar;
          calc_maxspect(zwidth, zheight, &width, &height);
        }
        if ((out->minwidth && width < out->minwidth)
            || (out->minheight && height < out->minheight)) {
          zwidth = out->minwidth;
          zheight = out->minheight;
          if (!zwidth) zwidth = zheight * op_ar;
          if (!zheight) zheight = zwidth / op_ar;
          calc_minspect(zwidth, zheight, &width, &height);
        }
      }
    }
    ...
    if (letterbox && has_non_lb_layers) {
      ...
      calc_minspect(width, height, &xopwidth, &xopheight);
      d_print_debug("some layers do not letterbox, adjusted size to  %d X %d\n", opwidth, opheight);
      if (prefs->pb_quality == PB_QUALITY_HIGH) {
        if (xopwidth < opwidth || xopheight < opheight) {
          xopwidth = opwidth;
          xopheight = opheight;
        }
      } else if (xopwidth > opwidth || xopheight > opheight) {
        xopwidth = opwidth;
        xopheight = opheight;
      }

      if (!svary) {
        int zwidth = xopwidth, zheight = xopheight;
        for (no = 0; no < nouts; no++) {
          out = n->outputs[no];
          if (out->flags & NODEFLAGS_IO_SKIP) continue;
          if ((out->maxwidth && zwidth > out->maxwidth)
              || (out->maxheight && zheight > out->maxheight)) {
            zwidth = out->maxwidth;
            zheight = out->maxheight;
            if (!zwidth) zwidth = zheight * op_ar;
            if (!zheight) zheight = zwidth / op_ar;
          ...
          if ((out->minwidth && zwidth < out->minwidth)
              || (out->minheight && zheight < out->minheight)) {
            zwidth = out->minwidth;
            zheight = out->minheight;
            if (!zwidth) zwidth = zheight * op_ar;
            if (!zheight) zheight = zwidth / op_ar;
    ...
    xopwidth = (xopwidth >> 2) << 2;
    xopheight = (xopheight >> 1) << 1;
    ...
    for (ni = 0; ni < n->n_inputs; ni++) {
      ...
      in->inner_width = in->width = xopwidth;
      in->inner_height = in->height = xopheight;
    ...
    for (no = 0; no < n->n_outputs; no++) {
      ...
      case 2:
        ...
        if (!out->width || !out->height) {
          ...
          if (out->width < out->minwidth) out->width = out->minwidth;
          if (out->height < out->minheight) out->height = out->minheight;
...
static void set_missing_sizes(inst_node_t *n) {
  // ascend filling in missing sizes
...
static inst_node_t *desc_and_do_something(int do_what, inst_node_t *n, inst_node_t *p,
  ...
  if (do_what == 5) {
    ...
    int key = n->model_idx;
    weed_instance_t *inst = rte_keymode_get_instance(key + 1, rte_key_getmode(key + 1));
    if (inst) {
    ...
    // 'virtuals' represent analyser instances with no connected outputs
    // hence they are not really part of the nodemodel
    // nevertheless, they may need to be reinited, so we check fot that
    for (LiVESList *list = n->virtuals; list; list = list->next) {
      n = (inst_node_t *)list->data;
      key = n->model_idx;
...
static void _make_nodes_model(lives_nodemodel_t *nodemodel) {
    ...
    if (mainw->rte) {
      ...
      for (i = 0; i < FX_KEYS_MAX_VIRTUAL; i++) {
        if (rte_key_valid(i + 1, TRUE)) {
          if (rte_key_is_enabled(i, FALSE)) { // soft deinits not applied
  ...
  } else if (!(mainw->vpp->capabilities & VPP_LOCAL_DISPLAY)) nsinks = 2;
...
void _get_costs(lives_nodemodel_t *nodemodel, boolean fake_costs) {
  ...
  // phase 18 - descend, creating tuples and prio list. We can "spend" slack to reduce tcost at the sink node
  do {
    for (list = nodemodel->node_chains; list; list = list->next) {
      nchain = (node_chain_t *)list->data;
      n = nchain->first_node;
...
void find_best_routes(lives_nodemodel_t *nodemodel, double * thresh) {
  ...
  // pass 2 descending - set sizes for known objects
  ...
  // pass 3 ascending - set missing sizes
...
lives_result_t run_next_cycle(void) {
  ...
  if (!mainw->plan_runner_proc
    ...
    g_print("plan runner cancelled, not running next cycle !\n");
    return LIVES_RESULT_INVALID;
  }
