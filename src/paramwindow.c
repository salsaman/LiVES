...
void transition_add_in_out(GtkBox *vbox, lives_rfx_t *rfx, boolean add_audio_check) {
  ...
  gtk_box_pack_start (GTK_BOX (vbox), hbox, FALSE, FALSE, widget_opts.packing_width);
...
boolean make_param_box(GtkVBox *top_vbox, lives_rfx_t *rfx) {
  ...
  if (!chk_params) {
    if (!has_param) {
      hbox = lives_hbox_new (FALSE, 0);
      gtk_box_pack_start (GTK_BOX (param_vbox), hbox, FALSE, FALSE, widget_opts.packing_height*2);
      add_fill_to_box(GTK_BOX(hbox));
...
boolean add_param_to_box (GtkBox *box, lives_rfx_t *rfx, gint pnum, boolean add_slider) {
  ...
  case LIVES_PARAM_BOOL :
	...
	gtk_box_pack_start (GTK_BOX (hbox), dlabel, FALSE, FALSE, widget_opts.packing_width);
