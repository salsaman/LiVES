...
_fx_dialog *on_fx_pre_activate(lives_rfx_t *rfx, boolean is_realtime, LiVESWidget *pbox) {
  ...
  if (pbox == NULL) {
    ...
    fx_dialog[didx]->dialog = lives_standard_dialog_new(_(rfx->menu_text[0] == '_' ? rfx->menu_text + 1 : rfx->menu_text), FALSE, scrw,
                              RFX_WINSIZE_V);
...
static boolean fmt_match(char *fmt_strings, int i, int n_fmt_strings, boolean state) {
  ...
  while (++i < n_fmt_strings) {
    const char *xfmt = &fmt_strings[i * FMT_STRING_SIZE];
    xlen = strlen(xfmt);
    all_fill = TRUE;
    if (xlen == 0) return FALSE;
    for (j = 0; j < xlen; j++) {
...
boolean make_param_box(LiVESVBox *top_vbox, lives_rfx_t *rfx) {
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    for (i = 0; list != NULL; i++) {
      ...
      layout_mode = FALSE;
      if (i < (pass == 0 ? MAX_FMT_STRINGS : n_fmt_strings - 1)) {
      ...
      for (j = 0; j < num_tok; j++) {
        ...
        if (!strncmp(array[j], "p", 1) && (pnum = atoi((char *)(array[j] + 1))) >= 0 && (pnum = pnum + poffset) < rfx->num_params && !used[pnum]) {
          ...
          if (needs_spacer) {
            ...
            if (pass == 0) {
              if ((fmtlen = strlen((const char *)format)) < FMT_STRING_SIZE - 1) format[fmtlen] = 255;
            } else {
              int wopw = widget_opts.packing_width;
          ...
          for (k = 0; k < length; k++) {
            ...
            } else if ((fmtlen = strlen((const char *)format)) < FMT_STRING_SIZE) format[fmtlen] = 255;
          }
          needs_spacer = FALSE;
          ...
          needs_spacer = FALSE;
          if (pass == 0) {
            if ((fmtlen = strlen((const char *)format)) < FMT_STRING_SIZE) format[fmtlen] = 254;
    ...
    for (i = 0; i < rfx->num_params; i++) {
      olayout_mode = layout_mode;
      layout_mode = FALSE;
      if (i + c_fmt_strings < (pass == 0 ? MAX_FMT_STRINGS : n_fmt_strings - 1)) {
      ...
      if (!used[i]) {
        if (!chk_params) {
          if (pass == 0) {
            if ((fmtlen = strlen((const char *)format)) < MAX_FMT_STRINGS) format[fmtlen] = (unsigned char)(rfx->params[i].type);
          } else {
            if (layoutx != NULL) {
  ...
  case LIVES_PARAM_NUM:
    ...
    if (add_scalers) {
      if (add_slider && !param->wrap) {
        ...
        //double val = *(double *)(param->def);
        //lives_scale_add_mark(LIVES_SCALE(scale), val, LIVES_POS_TOP, NULL);
        //lives_scale_add_mark(LIVES_SCALE(scale), val, LIVES_POS_BOTTOM, NULL);
    ...
    if (inst != NULL && weed_get_int_value(inst, WEED_LEAF_TYPE, &error) == WEED_PLANT_FILTER_INSTANCE) {
      ...
      if (mainw->multitrack != NULL && is_perchannel_multi(rfx, param_number)) {
        if (weed_plant_has_leaf(wparam, WEED_LEAF_IGNORE)) {
          int nvals = weed_leaf_num_elements(wparam, WEED_LEAF_IGNORE);
  ...
  if (rfx->status == RFX_STATUS_WEED) {
    ...
    if (inst != NULL && weed_get_int_value(inst, WEED_LEAF_TYPE, &error) == WEED_PLANT_FILTER_INSTANCE)  {
      update_weed_color_value(inst, param_number,
                              new_red, old_value.green, old_value.blue, 0, rfx);
      copyto = set_copy_to(inst, param_number, TRUE);
