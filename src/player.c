...
lives_result_t video_sync_ready(void) {
  ...
  if (lives_aplayer_get_seek_state(aplayer) == seek_approximate) {
    pthread_mutex_unlock(aplayer_seek_mutex);
    ...
    if (rndr) {
      ...
      if (!mainw->preview || lives_file_test(fname_next, LIVES_FILE_TEST_EXISTS)) {
        ...
        if (mainw->preview && !mainw->frame_layer
          ...
          frames_t delta_a = 0, delta_l = 0, delta_r = 0, delta_c = 0;
          ...
          // if it is behind sfile->frameno, then we discard it (set mainw->pred_frame == 0)
          ...
          if (mainw->cached_frame) {
            ...
            if (cclip == mainw->playing_file) {
              ...
              delta_c = dir * (ccframe - lframe);
            }
            if (delta_c < 1) evict_cache = TRUE;
          }
          ...
          if (mainw->pred_frame && mainw->frame_layer_preload) {
            ...
            if (delta_a > MAX_JMP_THRESH || delta_r > MIN_JMP_THRESH) {
              ...
              cache_misses++;
              if (delta_c) {
                if ((pframe - ccframe) * dir > 0 && delta_c > 0 && delta_c <= MIN_JMP_THRESH) {
                  use_cache = TRUE;
                  cache_pre = TRUE;
                }
              } else cache_pre = TRUE;
...
no_precache:
          if (!pframe && delta_c > 0 && delta_c <= MIN_JMP_THRESH) use_cache = TRUE;
          ...
          if (pframe) {
            ...
            if (delta_l > 1 && delta_a <= MAX_JMP_THRESH) {
              cache_hits++;
...
skip_precache:
    ...
    if (!mainw->frame_layer) {
      ...
      lives_layer_set_frame(mainw->frame_layer, frame);
    }
  }
...
void reset_old_frame_layer(void) {
  ...
  weed_layer_t *layer = STEAL_POINTER(old_frame_layer);
  ...
  if (!mainw->foreign) {
    ...
    if (!mainw->preview_rendering && (!((was_preview = mainw->preview) || mainw->is_rendering))) {
      ...
      if (LIVES_UNLIKELY(mainw->nervous) && clip_can_reverse(mainw->playing_file)) {
        // nervous mode
        frames_t xframe = frame;
        if ((xframe += (-10 + (int)(21.*rand() / (RAND_MAX + 1.0)))) <= cfile->frames && xframe >= 1)
          frame = xframe;
      ...
      if (LIVES_IS_RECORDING) {
        ...
        if (!mainw->elist_eom) {
          /* TRANSLATORS: rec(ord) */
          framecount = lives_strdup_printf((tmp = _("rec %9d / %d")), frame,
                                           cfile->frames > frame
                                           ? cfile->frames : frame);
        } else {
    ...
    if (!(mainw->preview || mainw->is_rendering)) {
      ...
      for (int i = 0; i < FX_KEYS_MAX_VIRTUAL; i++)
        if (rte_key_is_enabled(i, TRUE)) {
          boolean needs_reinit = pconx_chain_data(i, rte_key_getmode(i), FALSE);
          if (needs_reinit) fx_key_defs[i].flags |= FXKEY_NEEDS_REINIT;
        }
    ...
    if (prefs->audio_src == AUDIO_SRC_INT && mainw->aplayer) {
      ...
      if (skstate == seek_approximate) video_sync_ready();
    }
...
frames_t clamp_frame(int clipno, frames_t nframe) {
  ...
  if (!(sfile = RETURN_NORMAL_CLIP(clipno))) return 0;
    ...
    if (nframe >= first_frame && nframe <= last_frame) return nframe;
      ...
      ndir = dir = LIVES_DIRECTION_SIG(fps);
...
frames_t calc_new_playback_position(int clipno, ticks_t otc, ticks_t *ntc) {
  ...
  if (fps >= 0.)
    nframe = cframe + (frames_t)((double)dtc / TICKS_PER_SECOND_DBL * fps + .01);
  else
...
frames_t reachable_frame(int clipno, lives_decoder_t *dplug, frames_t stframe, frames_t enframe,
  ...
  // (or subtracting from) base (depending on dir(fps)) [sing of delta and fp should always produce a =ve value,
...
static frames_t find_best_frame(lives_decoder_t *dplug, frames_t requested_frame, frames_t dropped,
  ...
  if (sfile) {
    ...
    if (requested_frame < 0) {
      ...
      /* g_print("FRAMEVALS %d %d %d %d %d %d\n", best_frame, sfile->last_frameno, sfile->frameno, dir, */
...
// TODO - needs some rewriting, in particular:
// fixed_frame, sync_delta, SCRATCH_JUMP, SCRATCH_JUMP_NORESYNC, SCRATCH_REALIGN
...
static int process_one(void) {
  ...
  // sfile->last_frameno - the last frame number shown
...
player_loop:
  ...
  //  - check if any changes require a rebuild of the nodemodel
...
close_clip:
    ...
    if (mainw->playing_file != old_playing_file
      ...
      if (mainw->playing_file != old_playing_file) {
        ...
        if (!(prefs->audio_opts & AUDIO_OPTS_IS_LOCKED)) avsync_force(mainw->pulsed->inst);

#ifdef ENABLE_JACK
        if (prefs->audio_player == AUD_PLAYER_JACK) {
          /* if (sfile->arate) */
          /*   g_print("HIB2 %d %d %d %d %ld %f %f %ld %ld %d %f\n", sfile->frameno, sfile->last_req_frame, */
  ...
  if (init_timers) {
    ...
    sfile->last_req_frame = sfile->last_frameno = sfile->frameno;
  ...
  // handle jumps and trickplay first, as this may force the next frame

  if (sfile->next_frame > 0) {
    // external (to the player) controllers can force the next frame by
    // setting next_frame to a value > 0
    // this has the following results:-
    // next frame to be played is set, force show is set
    // next_frame is reset
    // mainw->scratch is set to SCRATCH_JUMP - this disarms any accounting based on
    // continuous flux, e.g any preload frame will be flagged as invalid, frame drop counter will be suspended
    // effort estimation will be bypassed, instant fps counter will be reset, CPU load threshold
    // will be ignored
    // if audio is set to resync on video jump that will also be triggered
  ...
  if (mainw->scratch == SCRATCH_NONE) {  
    ...
    // or to pb_fps, this is done by adjusting sfile->fps_scale, a muliplier
  ...
  if (fixed_frame > 0) {
    if (mainw->frame_layer_preload) {
      delta = dir * (mainw->pred_frame - fixed_frame);
  ...
  if (sfile->pb_fps * sfile->fps_scale != 0.) {
    dir = LIVES_DIRECTION_SIG(sfile->pb_fps * sfile->fps_scale);
    // TODO - move delivery and target_fps into clip_srcs
    ...
    if (sfile->delivery == LIVES_DELIVERY_PUSH) {
      ...
      // clips can set a target_fps, for example a generator
      // or webcam (
      ...
      if (LIVES_UNLIKELY(sfile->play_paused)) {
        mainw->startticks = mainw->currticks;
        lives_obj_instance_t *aplayer = get_aplayer_instance(prefs->audio_src);
        if (lives_aplayer_get_seek_state(aplayer) != not_seeking) mainw->video_seek_beacon = -1;
      }
	...
	if (fixed_frame > 0) {
	  ...
	  if (fixed_frame > 0) {
        ...
        if (mainw->force_show) show_frame = TRUE;
        else {
    ...
    if (IS_PHYSICAL_CLIP(mainw->playing_file)) {
      ...
      if (mainw->frame_layer_preload && mainw->pred_frame > 0) {
        ...
        lagged = (requested_frame - sfile->last_frameno) * dir;
      }
    ...
    if (show_frame) {
      ...
      if (mainw->foreign) {
        if (requested_frame >= sfile->frameno) {
          sfile->last_frameno = load_frame_image(requested_frame);
...
update_effort:
    ...
    if (show_frame) {
      ...
      skipped = (sfile->frameno - sfile->last_frameno) * dir - 1;
      ...
      lives_printerr("\nPLAY %d %d %d %d %ld %ld %d %d %.4f\n", sfile->frameno, requested_frame, sfile->last_frameno,
                     ...
                     mainw->video_seek_beacon,
                     lives_aplayer_get_seek_state(mainw->aplayer), cpuloadval);
#endif
#ifndef REC_IDEAL
      can_rec = TRUE;
#endif
      ...
      if (cpu_load_thresh) {
        if (cpuloadval > cpu_load_thresh) cpu_load_thresh += 1.;
      ...
      if (!cpu_load_thresh || cpuloadval <= cpu_load_thresh || mainw->force_show
        ...
        frame = load_frame_image(sfile->frameno);
        if (frame > 0) sfile->last_frameno = sfile->frameno = frame;
        ...
        // on return, we have:
        // sfile->last_frameno == the frame that was actually played, this may differ from
    ...
    if (LIVES_IS_RECORDING) {
      ...
      mainw->record_frame = sfile->last_frameno;
#endif
...
#define REC_IDEAL
    ...
    if (scratch == SCRATCH_NONE && IS_PHYSICAL_CLIP(mainw->playing_file)) {
      if (!mainw->multitrack && !mainw->refresh_model
        ...
        if (getahead <= 0 && can_precache && !showed_frame) {
          lives_decoder_t *dplug = NULL;
          if (sfile->clip_type == CLIP_TYPE_FILE) {
            lives_clipsrc_group_t *srcgrp = get_srcgrp(mainw->playing_file, 0, SRC_PURPOSE_PRECACHE);
            if (srcgrp && srcgrp->n_srcs) {
...
boolean begin_playback(void) {
  ...
  if (mainw->event_list || !CLIP_HAS_VIDEO(mainw->playing_file)) mainw->video_seek_beacon = -1;
