...
static lives_hash_store_t *plugin_store = NULL;

const lives_plugin_id_t *plugin_from_store(uint64_t uid) {
  return (const lives_plugin_id_t *)get_from_hash_store_i(plugin_store, uid);
}


const char *lives_plugin_get_name(uint64_t uid) {
  const lives_plugin_id_t *plug_id = plugin_from_store(uid);
  if (uid) return plug_id->name;
  return NULL;
}


char *get_pltype(uint64_t ptype, boolean plural, boolean caps, boolean full) {
  int nitems = 1;
  char *tname;
  if (plural) nitems = 2;
  if (!full) {
    if (!caps) switch (ptype) {
      ...
      case PLUGIN_TYPE_FILTER: tname = lives_strdup(P_("filter", "filters", nitems)); break;
    ...
    if (!caps)
      ...
      case PLUGIN_TYPE_FILTER: tname = lives_strdup(P_("filter plugin", "filter plugins", nitems)); break;
...
char *get_full_plname(uint64_t uid) {
  const lives_plugin_id_t *plug_id = plugin_from_store(uid);
  char *plugtype = get_pltype(plug_id->type, FALSE, FALSE, TRUE);
  char *plver = plverstring(plugtype, plug_id);
  lives_free(plugtype);
  return plver;
}

// e,g "openGL_player engine version x.y"
LIVES_LOCAL_INLINE char *make_plverstring(_vid_playback_plugin *vpp, const char *extra) {
  if (vpp->id) {
    char *text = lives_strdup_printf("%s%s", vpp->id->name, extra);
    char *vstring = plverstring(text, vpp->id);
    lives_free(text);
    return vstring;
  }
...
_vid_playback_plugin *open_vid_playback_plugin(const char *name, boolean in_use) {
  ...
  plugin_store = add_to_hash_store_i(plugin_store, vpp->id->uid, (void *)vpp->id);
...
const lives_decoder_sys_t *find_decoder(uint64_t dec_uid, const char *decplugname) {
...
static void decoder_plugin_move_to_first(const lives_decoder_sys_t *dpsys) {
  LiVESList *list;
  load_decoders();
  list = lives_list_find_by_data(capable->plugins_list[PLUGIN_TYPE_DECODER], (void *)dpsys);
  capable->plugins_list[PLUGIN_TYPE_DECODER] =
    lives_list_move_to_first(capable->plugins_list[PLUGIN_TYPE_DECODER], list);
}


static LiVESList *disable_decoders(LiVESList **dlistp) {
  ...
  const char *decid;
  uint64_t uid;
  if (!dlistp) return NULL;
  for (dlist = *dlistp; dlist; dlist = dlist->next) {
...
void load_decoders(void) {
  if (ARE_UNCHECKED(decoder_plugins)) {
    ...
    if (dlist) {
      LiVESList *ddlist = get_list_pref(PREF_DISABLED_DECODERS);
      capable->plugins_list[PLUGIN_TYPE_DECODER] = dlist;
      if (dlist) prefs->disabled_decoders = disable_decoders(&ddlist);
      ...
      d_print(msg);
      lives_free(msg);
      capable->has_decoder_plugins = MISSING;
    }
  }
}
...
static void set_cdata_memfuncs(lives_clip_data_t *cdata) {
  ...
  if (!cdata) return;
  cdata->ext_funcs.malloc  = &ext_malloc;
  cdata->ext_funcs.free    = &ext_free;
  cdata->ext_funcs.memcpy  = &ext_memcpy;
  cdata->ext_funcs.memset  = &ext_memset;
  cdata->ext_funcs.memmove = &ext_memmove;
  cdata->ext_funcs.realloc = &ext_realloc;
  cdata->ext_funcs.calloc  = &ext_calloc;
}
  ...
  if (sfile && decoder && !pthread_mutex_lock(&decoder->mutex)) {
    ...
    if (parent) {
      ...
      if (parent->relations.offspring) {
        parent->relations.offspring = lives_list_remove_data(parent->relations.offspring, decoder, FALSE);
      }
    }
    for (LiVESList *list = decoder->relations.offspring; list; list = list->next) {
      lives_decoder_t *child = (lives_decoder_t *)list->data;
      pthread_mutex_lock(&child->mutex);
      child->relations.parent = parent;
      if (parent)
        parent->relations.offspring = lives_list_append(parent->relations.offspring, child);
...
void propogate_timing_data(lives_decoder_t *dplug) {
  if (!dplug || !dplug->cdata) return;
  else {
    static boolean orig = TRUE;
    boolean was_orig = orig;
    boolean need_unlock = FALSE;
    if (!pthread_mutex_trylock(&dplug->mutex)) need_unlock = TRUE;
    if (orig) {
      // copy upwards
      lives_decoder_t *parent = (lives_decoder_t *)dplug->relations.parent;
      if (parent) {
        if (parent->cdata) lives_memcpy(&parent->cdata->adv_timing, &dplug->cdata->adv_timing,
                                          (char *) & (dplug->cdata->adv_timing.xvals)
                                          - (char *) & (dplug->cdata->adv_timing));
        // copy downwards
        propogate_timing_data(parent);
        if (need_unlock) pthread_mutex_unlock(&dplug->mutex);
        return;
      }
      orig = FALSE;
    }
    for (LiVESList *offspring = dplug->relations.offspring; offspring; offspring = offspring->next) {
      lives_decoder_t *child = (lives_decoder_t *)offspring->data;
      if (!pthread_mutex_trylock(&child->mutex)) {
        if (child->cdata) {
          lives_memcpy(&child->cdata->adv_timing, &dplug->cdata->adv_timing,
                       (char *) & (dplug->cdata->adv_timing.xvals)
                       - (char *) & (dplug->cdata->adv_timing));
          propogate_timing_data(child);
        }
        pthread_mutex_unlock(&child->mutex);
      }
    }
    if (was_orig) orig = TRUE;
    if (need_unlock) pthread_mutex_unlock(&dplug->mutex);
  }
}
...
lives_decoder_t *add_decoder_clone(int nclip, int track, int purpose) {
  if (IS_NORMAL_CLIP(nclip)) {
    ...
    if (dplug) {
      add_clip_src(nclip, track, purpose, CLASS_UID_VIDEO_DECODER,
...
lives_clip_src_t *add_ext_decoder_clone(int dclip, int sclip, int track, int purpose) {
  ...
  if (IS_NORMAL_CLIP(sclip)) {
    ...
    if (dplug) dsource = add_clip_src(dclip, track, purpose, CLASS_UID_VIDEO_DECODER,
                                        (void *)dplug, LIVES_SRC_TYPE_DECODER,
                                        dplug->dpsys->id->uid, NULL, dplug->cdata->URI);
  }
  return dsource;
...
static lives_decoder_t *try_decoder_plugins(char *xfile_name, LiVESList * disabled,
  ...
  for (; decoder_plugin; decoder_plugin = decoder_plugin->next) {
    ...
    defer_sigint_rcpt = lives_hook_cb_append(NULL, THREAD_EXIT_HOOK, 0,
    ...
    lives_hook_cb_remove(defer_sigint_rcpt);
...
const lives_clip_data_t *get_decoder_cdata(int clipno, const lives_clip_data_t *fake_cdata) {
  ...
  *decplugname =  0;
  ...
  xdisabled = lives_list_copy(prefs->disabled_decoders);

  dec_uid = sfile->old_dec_uid;
...
lives_decoder_sys_t *open_decoder_plugin(const char *plname) {
  ...
  plugin_store = add_to_hash_store_i(plugin_store, dpsys->id->uid, (void *)dpsys->id);
  return dpsys;
}
...
static void dpa_ok_clicked(LiVESButton * button, LiVESWidget * dlg) {
  ...
  while (cb) {
    if (!lives_toggle_button_get_active(LIVES_TOGGLE_BUTTON(cb)))
      future_prefs->disabled_decoders = lives_list_append_unique(future_prefs->disabled_decoders, list->data);
    list = list->next;
...
void on_decplug_advanced_clicked(LiVESButton * button, livespointer user_data) {
  ...
  for (; decoder_plugin; decoder_plugin = decoder_plugin->next) {
    const lives_decoder_sys_t *dpsys = (const lives_decoder_sys_t *)decoder_plugin->data;
    char *pltype = get_full_plname(dpsys->id->uid);
    ...
    // TODO -show warnign if user enables a dodgy plugin
    //lives_widget_object_set_data(LIVES_WIDGET_OBJECT(last_cb), LINK_KEY, checkbutton);
    last_cb = checkbutton;
  }
