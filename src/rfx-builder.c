...
void on_table_swap_row (LiVESButton *button, livespointer user_data) {
  ...
  case RFX_TABLE_TYPE_PARAM_WINDOW:
    ...
    lives_entry_set_text (LIVES_ENTRY (rfxbuilder->entry[found]),lives_entry_get_text (LIVES_ENTRY (rfxbuilder->entry[rfxbuilder->table_swap_row2])));
    ...
    lives_entry_set_text (LIVES_ENTRY (rfxbuilder->entry2[found]),lives_entry_get_text (LIVES_ENTRY (rfxbuilder->entry2[rfxbuilder->table_swap_row2])));
...
LiVESWidget * make_param_dialog (int pnum, rfx_build_window_t *rfxbuilder) {
  ...
  rfxbuilder->spinbutton_param_max = lives_standard_spin_button_new (_("Ma_ximum value: "),TRUE, RFX_DEF_NUM_MAX, -LIVES_MAXINT, LIVES_MAXINT, 1., 1., 0,
								     LIVES_BOX(dialog_vbox),NULL);
  ...
  rfxbuilder->spinbutton_param_step = lives_standard_spin_button_new ((tmp=lives_strdup(_("     _Step size:   "))),TRUE,1., 1., LIVES_MAXINT, 1., 1., 0,
  ...
  rfxbuilder->param_wrap_checkbutton = lives_standard_check_button_new ((tmp=lives_strdup(_("_Wrap value"))),TRUE,LIVES_BOX(rfxbuilder->param_wrap_hbox),
...
boolean rfxbuilder_to_script (rfx_build_window_t *rfxbuilder) {
  ...
  lives_free (script_name);
  msg=lives_strdup_printf (_ ("Writing script file %s..."),script_file);
  d_print (msg);
  lives_free (msg);
...
void on_delete_rfx_activate (LiVESMenuItem *menuitem, livespointer user_data) {
  ...
  if (strlen (script_name)) {
    ...
    msg=lives_strdup_printf (_ ("Deleting rfx script %s..."),rfx_script_file);
    d_print (msg);
    lives_free (msg);
...
void on_promote_rfx_activate (LiVESMenuItem *menuitem, livespointer user_data) {
  ...
  if (strlen (script_name)) {
    ...
    msg=lives_strdup_printf (_ ("Promoting rfx test plugin %s to custom..."),script_name);
    d_print (msg);
    lives_free (script_name);
    lives_free (msg);
...
void on_export_rfx_activate (LiVESMenuItem *menuitem, livespointer user_data) {
  ...
  msg=lives_strdup_printf(_ ("Copying %s to %s..."),rfx_script_from,filename);
  d_print(msg);
  lives_free(msg);
#ifndef IS_MINGW
...
void on_import_rfx_activate (LiVESMenuItem *menuitem, livespointer user_data) {
  ...
  msg=lives_strdup_printf(_ ("Copying %s to %s..."),filename,rfx_script_to);
  d_print(msg);
  lives_free(msg);
#ifndef IS_MINGW
...
char *prompt_for_script_name(const char *sname, lives_rfx_status_t status) {
  ...
  do {
	...
	if (name !=NULL&&lives_list_find ((nmlist=get_script_list (status)),xname)!=NULL) {
	  ...
	  d_print ((tmp=lives_strdup_printf (_ ("Renaming RFX test script %s to %s..."),from_name,xname)));
	  lives_free(tmp);
	  lives_free (from_name);
