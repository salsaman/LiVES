  ...
  if (aligned_span_count > span_count) {
    ...
    if (reserved_count > _memory_heap_reserve_count) {
      ...
      if (_memory_global_reserve) {
        _rpmalloc_span_mark_as_subspan_unless_master(_memory_global_reserve_master, _memory_global_reserve,
...
static uint32_t
free_list_partial_init(void **list, void **first_block, void *page_start, void *block_start, uint32_t block_count,
                       uint32_t block_size) {
...
_rpmalloc_heap_cache_insert(heap_t *heap, span_t *span) {
  ...
  if (span_count == 1) {
    ...
    if (span_cache->count == cache_limit) {
      ...
      const size_t transfer_count = (THREAD_SPAN_LARGE_CACHE_TRANSFER <= transfer_limit ? THREAD_SPAN_LARGE_CACHE_TRANSFER :
                                     transfer_limit);
...
_rpmalloc_heap_allocate_new(void) {
  ...
  size_t heap_span_count = ((aligned_heap_size * request_heap_count) + sizeof(span_t) + _memory_span_size - 1) /
                           _memory_span_size;
...
rpmalloc_initialize_config(const rpmalloc_config_t *config) {
  ...
  if ((_memory_page_size >= _memory_span_size) && ((_memory_span_map_count * _memory_span_size) % _memory_page_size))
    _memory_span_map_count = (_memory_page_size / _memory_span_size);
  _memory_heap_reserve_count = (_memory_span_map_count > DEFAULT_SPAN_MAP_COUNT) ? DEFAULT_SPAN_MAP_COUNT :
...
_memory_heap_dump_statistics(heap_t *heap, void *file) {
  ...
  fprintf(file,
          "Spans  Current     Peak Deferred  PeakMiB  Cached  ToCacheMiB FromCacheMiB ToReserveMiB FromReserveMiB ToGlobalMiB FromGlobalMiB  MmapCalls\n");
  for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
  ...
  for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
    ...
    if (global_cache || global_overflow_cache || cache->insert_count || cache->extract_count)
      fprintf(file, "%4zu: %8zuMiB (%8zuMiB overflow) %14zu insert %14zu extract\n", iclass + 1, global_cache / (size_t)(1024 * 1024),
              global_overflow_cache / (size_t)(1024 * 1024), cache->insert_count, cache->extract_count);
  }
#endif
