...
// functions for first time setup
...
reloop:
  ...
  if (response == LIVES_RESPONSE_RETRY) {
    if (!do_setup_tests(FALSE)) response = LIVES_RESPONSE_CANCEL;
    else {
      txt0 = lives_strdup("");
      radiobutton_group = NULL;
      goto reloop;
...
boolean do_setup_tests(boolean tshoot) {
  LiVESWidget *dialog;
...
boolean do_setup_interface_query(void) {
...
void on_troubleshoot_activate(LiVESMenuItem * menuitem, livespointer user_data) {do_setup_tests(TRUE);}
