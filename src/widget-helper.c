...
static void button_state_cb(LiVESWidget *button, LiVESWidgetState state, livespointer user_data) {
  if (widget_opts.apply_theme && LIVES_IS_BUTTON(button)) {
...
static void set_entry_colours(LiVESWidget *widget, LiVESWidgetState state, livespointer labelp) {
...
#endif
  if (labelp != NULL) set_label_state(widget, state, labelp);
}
...
LiVESWidget *lives_standard_spin_button_new(const char *labeltext, double val, double min,
  ...
  lives_signal_connect_after(LIVES_GUI_OBJECT(spinbutton), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                             ...
                             (livespointer)label);
  set_entry_colours(spinbutton, lives_widget_get_state(spinbutton), (livespointer)label);
...
LiVESWidget *lives_standard_combo_new(const char *labeltext, LiVESList *list, LiVESBox *box, const char *tooltip) {
  ...
  set_entry_colours(LIVES_WIDGET(combo), lives_widget_get_state(combo), (livespointer)label);
...
WIDGET_HELPER_GLOBAL_INLINE LiVESWidget *lives_standard_lock_button_new(boolean is_locked, int width, int height, const char *tooltip) {
  ...
  _on_lock_button_clicked(LIVES_BUTTON(lockbutton), NULL);
  return lockbutton;
}
...
LiVESWidget *lives_standard_color_button_new(LiVESBox *box, const char *name, boolean use_alpha, lives_colRGBA64_t *rgba,
  ...
  if (box != NULL) {
    if (labelcname != NULL) lives_box_pack_start(LIVES_BOX(hbox), labelcname, FALSE, FALSE, (int)(4.*widget_opts.scale));
...
void funkify_dialog(LiVESWidget *dialog) {
  if (prefs->funky_widgets) {
    ...
    gtk_widget_set_margin_top(action, widget_opts.packing_height);
