...
static void edit_state_cb(LiVESObject *object, livespointer pspec, livespointer user_data) {
...
static void widget_state_cb(LiVESObject *object, livespointer pspec, livespointer user_data) {
...
WIDGET_HELPER_GLOBAL_INLINE LiVESWidget *lives_image_new_from_stock(const char *stock_id, LiVESIconSize size) {
  return lives_image_new_from_stock_at_size(stock_id, size, get_real_size_from_icon_size(size), get_real_size_from_icon_size(size));
}
...
static void default_changed_cb(LiVESObject *object, livespointer pspec, livespointer user_data) {
...
static void sens_insens_cb(LiVESObject *object, livespointer pspec, livespointer user_data) {
  LiVESWidget *widget = (LiVESWidget *)object;
...
LiVESWidget *lives_standard_spin_button_new(const char *labeltext, double val, double min,
  ...
  if (widget_opts.apply_theme) {
    ...
    lives_signal_connect_after(LIVES_GUI_OBJECT(spinbutton), LIVES_WIDGET_NOTIFY_SIGNAL "sensitive",
                               LIVES_GUI_CALLBACK(widget_state_cb),
                               NULL);
    widget_state_cb(LIVES_WIDGET_OBJECT(spinbutton), NULL, NULL);
#else
    set_child_colour(spinbutton, TRUE);
    set_child_dimmed_colour(spinbutton, BUTTON_DIM_VAL); // insens, themecols 1, child only
    lives_widget_apply_theme2(LIVES_WIDGET(spinbutton), LIVES_WIDGET_STATE_NORMAL, TRUE);
    lives_widget_apply_theme_dimmed(spinbutton, LIVES_WIDGET_STATE_INSENSITIVE, BUTTON_DIM_VAL);
#endif
  }
  return spinbutton;
}
...
LiVESWidget *lives_standard_entry_new(const char *labeltext, const char *txt, int dispwidth, int maxchars,
  ...
  if (widget_opts.apply_theme) {
#if GTK_CHECK_VERSION(3, 0, 0)
    lives_signal_connect_after(LIVES_GUI_OBJECT(entry), LIVES_WIDGET_NOTIFY_SIGNAL "editable",
...
LiVESWidget *lives_dialog_add_button_from_stock(LiVESDialog *dialog, const char *stock_id, const char *label, int response_id) {
  ...
  if (dialog != NULL) {
    if ((first_button = (LiVESWidget *)lives_widget_object_get_data(LIVES_WIDGET_OBJECT(dialog), "first_button")) == NULL) {
      ...
      lives_button_uncenter(first_button, LIVES_POINTER_TO_INT(lives_widget_object_get_data(LIVES_WIDGET_OBJECT(first_button), "normal_width")));
    }
  }
  return button;
}
