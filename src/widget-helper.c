...
static void force_widget_colours(LiVESWidget *widget, LiVESWidgetState state, livespointer user_data) {
  int colscheme = LIVES_POINTER_TO_INT(user_data);
  ...
  if (LIVES_IS_COMBO(widget)) {
    ...
    force_widget_colours(widget, state, user_data);
  }
  if (LIVES_IS_TOOL_BUTTON(widget)) {
    widget = gtk_tool_button_get_icon_widget(LIVES_TOOL_BUTTON(widget));
    if (widget != NULL) force_widget_colours(widget, state, user_data);
  }
...
static LiVESWidget *prettify_button(LiVESWidget *button) {
  if (!widget_opts.apply_theme || button == NULL) return button;

  lives_widget_apply_theme2(button, LIVES_WIDGET_STATE_NORMAL, TRUE);
  lives_widget_object_set_data(LIVES_WIDGET_OBJECT(button), "default_grabbed", LIVES_INT_TO_POINTER(FALSE));
...
WIDGET_HELPER_GLOBAL_INLINE boolean lives_object_ref(livespointer object) {
#ifdef GUI_GTK
  if (LIVES_IS_OBJECT(object)) g_object_ref(object);
  else {
...
#ifdef GUI_GTK
  if (LIVES_IS_OBJECT(object)) g_object_unref(object);
  else {
...
#ifdef GUI_GTK
#if GTK_CHECK_VERSION(3, 0, 0)
WIDGET_HELPER_GLOBAL_INLINE boolean lives_object_ref_sink(livespointer object) {
  if (!LIVES_IS_OBJECT(object)) {
...
static boolean lives_widget_color_mix(LiVESWidgetColor *c1, const LiVESWidgetColor *c2, float mixval) {
...
WIDGET_HELPER_GLOBAL_INLINE LiVESWidget *lives_image_new_from_stock(const char *stock_id, LiVESIconSize size) {
  ...
  if (lives_has_icon(stock_id, size)) {
    ...
    LiVESPixbuf *pixbuf = lives_pixbuf_new_from_file(fnamex, &error);
    lives_free(fnamex);
    lives_free(fname);
    if (pixbuf != NULL) {
      LiVESPixbuf *new_pixbuf = pixbuf;
      if (size != LIVES_ICON_SIZE_CUSTOM) {
        new_pixbuf = lives_pixbuf_scale_simple(pixbuf, get_real_size_from_icon_size(size), get_real_size_from_icon_size(size),
                                               LIVES_INTERP_BEST);
...
WIDGET_HELPER_GLOBAL_INLINE LiVESToolItem *lives_tool_button_new(LiVESWidget *icon_widget, const char *label) {
  ...
  prettify_button(LIVES_WIDGET(button));
#endif
#ifdef GUI_QT
...
WIDGET_HELPER_GLOBAL_INLINE boolean lives_toolbar_insert(LiVESToolbar *toolbar, LiVESToolItem *item, int pos) {
  ...
  if (widget_opts.apply_theme) {
    lives_signal_connect_after(LIVES_GUI_OBJECT(item), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                               LIVES_GUI_CALLBACK(force_widget_colours),
                               LIVES_INT_TO_POINTER(1));
    force_widget_colours(LIVES_WIDGET(item), lives_widget_get_state(LIVES_WIDGET(item)), LIVES_INT_TO_POINTER(1));

    if (GTK_IS_TOOL_BUTTON(item)) {
      LiVESWidget *icon = gtk_tool_button_get_icon_widget(LIVES_TOOL_BUTTON(item));
      if (icon != NULL) {
        lives_signal_connect_after(LIVES_GUI_OBJECT(icon), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                                   LIVES_GUI_CALLBACK(force_widget_colours),
                                   LIVES_INT_TO_POINTER(1));
        force_widget_colours(LIVES_WIDGET(icon), lives_widget_get_state(LIVES_WIDGET(icon)), LIVES_INT_TO_POINTER(1));
      }
    }
  }
#endif
...
WIDGET_HELPER_GLOBAL_INLINE boolean lives_image_scale(LiVESImage *image, int width, int height, LiVESInterpType interp_type) {
  LiVESPixbuf *pixbuf = lives_image_get_pixbuf(image);
  if (pixbuf != NULL) {
    height = width = 16; // TODO !!!
    ...
    //if (LIVES_IS_OBJECT(pixbuf)) lives_object_unref(pixbuf);
    //if (new_pixbuf != NULL && LIVES_IS_OBJECT(new_pixbuf)) lives_object_unref(new_pixbuf);
  }
  return TRUE;
}
...
boolean lives_widget_grab_default_special(LiVESWidget *widget) {
  ...
  if (widget_opts.apply_theme && LIVES_IS_BUTTON(widget)) {
    set_child_alt_colour(widget, TRUE);
    lives_widget_object_set_data(LIVES_WIDGET_OBJECT(widget), "default_grabbed", LIVES_INT_TO_POINTER(TRUE));
  }
  return TRUE;
}
...
WIDGET_HELPER_GLOBAL_INLINE LiVESWidget *lives_layout_new(LiVESBox *box) {
  ...
  if (LIVES_IS_VBOX(box)) {
    lives_box_pack_start(box, layout, widget_opts.expand == LIVES_EXPAND_EXTRA, TRUE, widget_opts.packing_height);
  } else {
    lives_box_pack_start(box, layout, widget_opts.expand == LIVES_EXPAND_EXTRA, TRUE, widget_opts.packing_width);
  }
...
#else
  LiVESWidget *alignment = lives_alignment_new(widget_opts.justify == LIVES_JUSTIFY_CENTER ? 0.5 : widget_opts.justify == LIVES_JUSTIFY_RIGHT
                           ? 1. : 0. , .5, 0., 0.);
...
LiVESWidget *lives_standard_check_button_new(const char *labeltext, boolean active, LiVESBox *box,
  ...
  if (box != NULL) {
    ...
    lives_box_pack_start(LIVES_BOX(hbox), checkbutton, expand, FALSE,
...
LiVESWidget *lives_standard_spin_button_new(const char *labeltext, double val, double min,
  ...
  if (widget_opts.apply_theme) {
    ...
    force_widget_colours(spinbutton, lives_widget_get_state(spinbutton), LIVES_INT_TO_POINTER(0));
  }
#endif
...
LiVESWidget *lives_standard_combo_new(const char *labeltext, LiVESList *list, LiVESBox *box, const char *tooltip) {
  ...
  if (widget_opts.apply_theme) {
    lives_signal_connect_after(LIVES_GUI_OBJECT(combo), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                               LIVES_GUI_CALLBACK(force_widget_colours),
                               LIVES_INT_TO_POINTER(0));
    force_widget_colours(combo, lives_widget_get_state(combo), LIVES_INT_TO_POINTER(0));
    lives_signal_connect_after(LIVES_GUI_OBJECT(entry), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                               LIVES_GUI_CALLBACK(force_widget_colours),
                               LIVES_INT_TO_POINTER(0));
    force_widget_colours(LIVES_WIDGET(entry), lives_widget_get_state(LIVES_WIDGET(entry)), LIVES_INT_TO_POINTER(0));
  }
#endif
...
LiVESWidget *lives_standard_entry_new(const char *labeltext, const char *txt, int dispwidth, int maxchars,
  ...
  if (widget_opts.apply_theme) {
    lives_signal_connect_after(LIVES_GUI_OBJECT(entry), LIVES_WIDGET_STATE_CHANGED_SIGNAL,
                               LIVES_GUI_CALLBACK(force_widget_colours),
                               LIVES_INT_TO_POINTER(0));
    force_widget_colours(entry, lives_widget_get_state(entry), LIVES_INT_TO_POINTER(0));
  }
#endif
...
LiVESWidget *lives_standard_dialog_new(const char *title, boolean add_std_buttons, int width, int height) {
  ...
  if (add_std_buttons) {
    ...
    LiVESWidget *okbutton = lives_standard_button_new_from_stock(LIVES_STOCK_OK, NULL);
...
static void _on_lock_button_clicked(LiVESButton *button, livespointer user_data) {
  ...
  if (locked) image = lives_image_new_from_stock(LIVES_LIVES_STOCK_LOCKED, LIVES_ICON_SIZE_CUSTOM);
  else image = lives_image_new_from_stock(LIVES_LIVES_STOCK_UNLOCKED, LIVES_ICON_SIZE_CUSTOM);
...
WIDGET_HELPER_GLOBAL_INLINE LiVESWidget *lives_standard_lock_button_new(boolean is_locked, int width, int height, const char *tooltip) {
...
LiVESWidget *lives_standard_color_button_new(LiVESBox *box, const char *name, boolean use_alpha, lives_colRGBA64_t *rgba,
  ...
  if (box != NULL) {
    ...
    if (parent != NULL && LIVES_IS_TABLE(parent) && lives_widget_object_get_data(LIVES_WIDGET_OBJECT(parent), "widgets_added") != NULL) {
  ...
  if (box != NULL) {
    if (labelcname != NULL) {
      lives_box_pack_start(LIVES_BOX(hbox), labelcname, FALSE, FALSE, (int)(4.*widget_opts.scale));
      if (parent_is_layout) {
...
WIDGET_HELPER_GLOBAL_INLINE boolean toggle_button_toggle(LiVESToggleButton *tbutton) {
...
LiVESWidget *add_fill_to_box(LiVESBox *box) {
  ...
  if (LIVES_IS_HBOX(box)) {
    ...
    } else lives_box_pack_start(box, widget, FALSE, TRUE, widget_opts.filler_len / 2);
    //lives_widget_set_size_request(widget, 1, -1);
  } else {
    if (widget_opts.expand == LIVES_EXPAND_EXTRA) {
      lives_box_pack_start(box, widget, TRUE, TRUE, widget_opts.filler_len / 2);
    } else lives_box_pack_start(box, widget, FALSE, TRUE, widget_opts.expand == LIVES_EXPAND_NONE ? 0 : widget_opts.packing_height);
    //lives_widget_set_size_request(widget, -1, 1);
  }
