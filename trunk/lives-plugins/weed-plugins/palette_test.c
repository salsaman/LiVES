...
int ptest_process (weed_plant_t *inst, weed_timecode_t timestamp) {
  ...
  if (src[1]!=dst[1]) weed_memcpy(dst[1],src[1],width*height/4);
  if (src[2]!=dst[2]) weed_memcpy(dst[2],src[2],width*height/4);
...
weed_plant_t *weed_setup (weed_bootstrap_f weed_boot) {
  ...
  if (plugin_info!=NULL) {
    int palette_list[]={WEED_PALETTE_YUV420P,WEED_PALETTE_END};

    weed_plant_t *in_chantmpls[]={weed_channel_template_init("in channel 0",0,palette_list),NULL};
    weed_plant_t *out_chantmpls[]={weed_channel_template_init("out channel 0",WEED_CHANNEL_CAN_DO_INPLACE,palette_list),NULL};
    weed_plant_t *filter_class=weed_filter_class_init("Palette testing plugin","salsaman",1,WEED_FILTER_HINT_IS_POINT_EFFECT,NULL,&ptest_process,NULL,in_chantmpls,out_chantmpls,NULL,NULL);
