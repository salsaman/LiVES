...
nine_fill(unsigned char *new_data, int rowstride, unsigned char *old_data) {
  ...
  new_data[-rowstride - 3] = new_data[-rowstride] = new_data[-rowstride + 3] = new_data[-3] = new_data[0] = new_data[3] = new_data[rowstride -
                             3] =
                               new_data[rowstride] = new_data[rowstride + 3] = old_data[0];
  new_data[-rowstride - 2] = new_data[-rowstride + 1] = new_data[-rowstride + 4] = new_data[-2] = new_data[1] = new_data[4] =
                               new_data[rowstride - 2] =
                                 new_data[rowstride + 1] = new_data[rowstride + 4] = old_data[1];
  new_data[-rowstride - 1] = new_data[-rowstride + 2] = new_data[-rowstride + 5] = new_data[-1] = new_data[2] = new_data[5] =
                               new_data[rowstride - 1] =
...
weed_plant_t *weed_setup(weed_bootstrap_f weed_boot) {
  ...
  if (plugin_info != NULL) {
    ...
    weed_plant_t *filter_class = weed_filter_class_init("graphic novel", "salsaman", 1, 0, NULL, &xeffect_process, NULL, in_chantmpls,
                                 out_chantmpls, NULL,
                                 NULL);
