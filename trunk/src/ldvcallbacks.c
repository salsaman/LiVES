...
static void dvgrab_set_cursor_style(gint cstyle) {
...
void on_camgrab_clicked (GtkButton *button, gpointer user_data) {
  ...
  dvgrab_set_cursor_style(LIVES_CURSOR_BUSY);
...
void on_camstop_clicked (GtkButton *button, gpointer user_data) {
  ...
  if (cam->format==CAM_FORMAT_DV) dummyvar=system("pkill -9 dvgrab 2>/dev/null");
