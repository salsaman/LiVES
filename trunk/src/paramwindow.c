...
void transition_add_in_out(LiVESBox *vbox, lives_rfx_t *rfx, boolean add_audio_check) {
  ...
  lives_signal_connect_after (LIVES_GUI_OBJECT (radiobutton_in), "toggled",
  ...
  lives_signal_connect_after (LIVES_GUI_OBJECT (radiobutton_out), "toggled",
...
static void add_gen_to(LiVESBox *vbox, lives_rfx_t *rfx) {
  ...
  lives_signal_connect_after (LIVES_GUI_OBJECT (radiobutton), "toggled",
  ...
  if (top_dialog_vbox!=NULL) {
	...
	if (rfx->status==RFX_STATUS_SCRAP)
	  ...
	  lives_signal_connect (LIVES_GUI_OBJECT (okbutton), "clicked",
			    ...
			    rfx);
	  if (resetbutton!=NULL) {
	    lives_signal_connect_after (LIVES_GUI_OBJECT (resetbutton), "clicked",
	...
	lives_signal_connect_after (LIVES_GUI_OBJECT (okbutton), "clicked",
...
boolean add_param_to_box (LiVESBox *box, lives_rfx_t *rfx, int pnum, boolean add_slider) {
  ...
  case LIVES_PARAM_STRING:
	...
	param->special_type!=LIVES_PARAM_SPECIAL_TYPE_FILEREAD) {
      ...
      lives_signal_connect_after (G_OBJECT (textbuffer), "changed", LIVES_GUI_CALLBACK (after_param_text_buffer_changed), 
			      (gpointer) rfx);
	...
	lives_signal_connect_after (G_OBJECT (entry), "changed", LIVES_GUI_CALLBACK (after_param_text_changed), 
				(gpointer) rfx);
      }
  ...
  case LIVES_PARAM_STRING_LIST:
    ...
    lives_signal_connect_after (G_OBJECT (combo), "changed", 
...
void add_param_label_to_box (LiVESBox *box, boolean do_trans, const gchar *text) {
  LiVESWidget *label;
...
void after_param_text_changed (LiVESWidget *textwidget, lives_rfx_t *rfx) {
  ...
  if (rfx->status==RFX_STATUS_WEED) {
	...
	if (copyto!=-1) rec_param_change(inst,copyto);
      ...
      if (disp_string!=NULL) {
	if ((int)param->max>RFX_TEXT_MAGIC||param->max==0.) {
	  lives_text_buffer_set_text (LIVES_TEXT_BUFFER (textbuffer), (gchar *)param->value, -1);
	}
	else {
